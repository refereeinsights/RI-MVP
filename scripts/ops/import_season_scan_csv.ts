...
async function main() {
  ...
  if (!file) throw new Error("Missing --file=tmp/season_scan_2027_....csv");
  const filePath = path.resolve(process.cwd(), file);
  if (!fs.existsSync(filePath)) throw new Error(`File not found: ${file}`);

  const scannedAt = clean(argValue("scanned_at")) ?? asIsoFromMtime(filePath);
  ...
  if (rows.length === 0) {
    console.log(JSON.stringify({ ok: true, apply: APPLY, file, rows: 0 }, null, 2));
    return;
  }
