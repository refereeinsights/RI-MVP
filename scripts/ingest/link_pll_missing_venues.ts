...
async function fetchHtml(url: string) {
  try {
    const resp = await fetch(url, {
      ...
      redirect: "follow",
      cache: "no-cache",
      headers: { "user-agent": "RI-PLL-VenueLinker/1.0" },
    });
...
function isPllUrl(value: string | null | undefined) {
  return /(?:^https?:\/\/)?(?:www\.)?premierlacrosseleague\.com\//i.test(String(value ?? ""));
}

function parseStreetCityStateZip(input: string): { address: string; city: string; state: string; zip: string | null } | null {
  const v = clean(input);
  if (!v) return null;
  // Typical format: "14787 Academy Pkwy, Herriman, UT 84096"
  const m = v.match(/^(.+?),\s*(.+?),\s*([A-Z]{2})\s*(\d{5}(?:-\d{4})?)?$/);
  if (!m) return null;
  const address = clean(m[1]);
  const city = clean(m[2]);
  const state = (clean(m[3]) ?? "").toUpperCase();
  const zip = clean(m[4]) ?? null;
  if (!address || !city || !state) return null;
  return { address, city, state, zip };
}

function parsePllVenues($: cheerio.CheerioAPI): ParsedVenue[] {
  const out: ParsedVenue[] = [];

  $("table.event-location-table tbody tr").each((_, tr) => {
    const row = $(tr);
    const name = clean(row.find('td[data-title="Location"]').text().replace(/^Location\s*/i, ""));
    const addrRaw = clean(row.find('td[data-title="Address"]').text().replace(/^Address\s*/i, ""));
    const directions = clean(row.find('td[data-title="Directions"] a[href]').attr("href"));
    if (!name || !addrRaw) return;
    const parsed = parseStreetCityStateZip(addrRaw);
    if (!parsed) {
      out.push({ name, address: addrRaw, city: "", state: "", zip: null, venue_url: directions });
      return;
    }
    out.push({
      name,
...
async function run() {
  ...
  const { data: tournamentsRaw, error: tournamentErr } = await supabase
    ...
    .or("source_url.ilike.%premierlacrosseleague.com%,official_website_url.ilike.%premierlacrosseleague.com%")
