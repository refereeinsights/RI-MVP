...
function looksLikeDetailLink(url: string) {
  return /soccerwire\.com\/tournaments\//i.test(url) || /soccerwire\.com\/events\//i.test(url);
}
...
export default async function crawlSoccerWire(
  seed: CrawlSeed,
  ctx: RunContext
  ...
  for (const link of detailLinks) {
    try {
      ...
      const title =
        ...
        $$("title").text().trim() ||
        "SoccerWire Tournament";

      const summary =
        $$("meta[name='description']").attr("content") ||
        $$("meta[property='og:description']").attr("content") ||
        extractText($$, ".entry-content p") ||
        null;

      const locationText =
        extractText($$, ".event-location") ||
        extractText($$, ".location") ||
        extractText($$, ".meta-location");
      ...
      appendRunLog(
        ctx,
        `Failed to parse SoccerWire detail ${link}: ${(error as Error).message}`
      );
    }
  }
