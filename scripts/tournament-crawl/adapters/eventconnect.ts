...
function looksLikeDetailLink(url: string) {
  return /eventconnect\.io\/events\/[\w-]+/i.test(url);
}
...
export default async function crawlEventConnect(
  seed: CrawlSeed,
  ctx: RunContext
  ...
  for (const link of detailLinks) {
    try {
      ...
      appendRunLog(
        ctx,
        `Failed to parse EventConnect detail ${link}: ${(error as Error).message}`
      );
    }
  }
