...
function looksLikeDetailLink(url: string) {
  if (!/infosports\.com/i.test(url)) return false;
  const parsed = new URL(url);
  if (/index\.cgi$/i.test(parsed.pathname)) {
    const search = parsed.search.toLowerCase();
    return (
      search.includes("tournament") ||
      search.includes("tourny") ||
      search.includes("event")
    );
  }
  if (/\/tournaments?\//i.test(parsed.pathname)) return true;
  if (parsed.pathname.endsWith(".htm") || parsed.pathname.endsWith(".html")) return true;
  return false;
}
...
function extractSummary($$: cheerio.CheerioAPI) {
...
export default async function crawlInfoSports(
  seed: CrawlSeed,
  ctx: RunContext
  ...
  for (const link of detailLinks) {
    try {
      ...
      appendRunLog(
        ctx,
        `Failed to parse InfoSports detail ${link}: ${(error as Error).message}`
      );
    }
  }
