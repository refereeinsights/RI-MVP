...
function looksLikeDetailLink(url: string) {
  return /exposureevents\.com\/[a-z-]+\/event\/\d+/i.test(url);
}
...
export default async function crawlExposureEvents(
  seed: CrawlSeed,
  ctx: RunContext
  ...
  for (const link of detailLinks) {
    try {
      ...
      const title =
        ...
        $$("title").text().trim() ||
        "Exposure Basketball Tournament";
      ...
      const locationBlock =
        ...
        extractText($$, ".address");
      const { city, state } = locationBlock ? extractCityState(locationBlock) : { city: null, state: null };
      ...
      appendRunLog(
        ctx,
        `Failed to parse ExposureEvents detail ${link}: ${(error as Error).message}`
      );
    }
  }
