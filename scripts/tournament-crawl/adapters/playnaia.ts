...
function looksLikeDetailLink(url: string) {
  return /playnaia\.com\/youth-events\/[\w-]+/i.test(url);
}
...
export default async function crawlPlayNAIA(
  seed: CrawlSeed,
  ctx: RunContext
  ...
  for (const link of detailLinks) {
    try {
      ...
      appendRunLog(
        ctx,
        `Failed to parse PlayNAIA detail ${link}: ${(error as Error).message}`
      );
    }
  }
