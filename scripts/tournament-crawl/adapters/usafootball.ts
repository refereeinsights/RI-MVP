...
function looksLikeDetailLink(url: string) {
  return /usafootball\.com\/tournaments-events\/.+/i.test(url);
}
...
export default async function crawlUSAFootball(
  seed: CrawlSeed,
  ctx: RunContext
  ...
  for (const link of detailLinks) {
    try {
      ...
      appendRunLog(
        ctx,
        `Failed to parse USA Football detail ${link}: ${(error as Error).message}`
      );
    }
  }
