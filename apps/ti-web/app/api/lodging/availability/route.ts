...
const AVAILABILITY_ENDPOINT = "/api/lodging/availability";
...
function asTrackingString(body: AvailabilityRequestBody, keys: Array<keyof AvailabilityRequestBody>): string | null {
...
function normalizeTracking(body: AvailabilityRequestBody) {
  return {
...
async function insertStartedSession(input: {
  sessionId: string;
  provider: string;
  availabilityQuery: Record<string, unknown>;
  ...
  try {
    await (supabaseAdmin.from("lodging_search_session" as any) as any).insert({
      ...
      search_query: input.availabilityQuery,
      status: "started",
...
export async function POST(request: Request) {
  const body = (await request.json().catch(() => null)) as AvailabilityRequestBody | null;
  ...
  try {
    ...
    childCount = parseNonNegativeInteger(body.children ?? body.childrenPerRoom ?? body.childCount);
  } catch (error: unknown) {
  ...
  await insertStartedSession({
    ...
    availabilityQuery: {
      ...
      locale: providerInput.locale || null,
      currency: providerInput.currency || null,
    },
  ...
  try {
    const result = await provider.getHotelAvailability(providerInput);

    const latencyMs = Date.now() - startedAt;
    await updateSessionLifecycle({
      sessionId,
      status: "succeeded",
      resultCount: result.roomOptions.length,
      latencyMs,
    });
    ...
    return NextResponse.json({ sessionId, provider: providerName, propertyId, error: errorMessage }, { status: statusCode });
  }
}
