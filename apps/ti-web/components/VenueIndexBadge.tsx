import { computeVenueIndex, VenueIndexInput } from "@/lib/venueIndex";
    ...
    <div className={styles.badge}>
      ...
      <div className={styles.meter} aria-hidden="true">
        {[0, 1, 2, 3, 4].map((bar) => (
          <span
            key={bar}
            className={`${styles.bar} ${bar < result.bars ? styles.barFilled : ""}`}
          />
        ))}
      </div>
      ...
      {count > 0 && count < 5 ? (
        <div className={styles.muted}>Early data — score stabilizes as reviews increase.</div>
      ) : null}
    </div>
  );
}
