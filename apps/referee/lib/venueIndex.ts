...
function freshnessScoreFromDate(reviewsLastUpdatedAt: string | Date | null | undefined, now: Date): number {
  if (!reviewsLastUpdatedAt) return 40;
  const date = reviewsLastUpdatedAt instanceof Date ? reviewsLastUpdatedAt : new Date(reviewsLastUpdatedAt);
...
export function computeVenueIndex(input: VenueIndexInput): VenueIndexResult {
  ...
  const presentComponents = Object.values(normalized).filter((value): value is number => typeof value === "number");
  ...
  const weightedParts: Array<{ score: number; weight: number }> = [{ score: freshnessScore, weight: DEFAULT_WEIGHTS.freshness }];
  if (normalized.restrooms != null) weightedParts.push({ score: normalized.restrooms, weight: DEFAULT_WEIGHTS.restrooms });
  if (normalized.parking != null) weightedParts.push({ score: normalized.parking, weight: DEFAULT_WEIGHTS.parking });
  if (normalized.shade != null) weightedParts.push({ score: normalized.shade, weight: DEFAULT_WEIGHTS.shade });
  if (normalized.vendors != null) weightedParts.push({ score: normalized.vendors, weight: DEFAULT_WEIGHTS.vendors });

  const totalWeight = weightedParts.reduce((sum, part) => sum + part.weight, 0);
  const base = totalWeight > 0 ? weightedParts.reduce((sum, part) => sum + (part.score * part.weight) / totalWeight, 0) : 0;
