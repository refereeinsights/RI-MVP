...
type StateSeoProps = {
  ...
  intro: string;
  basePath: string;
  searchParams?: StateSeoSearchParams;
};
...
export default async function SportHubPage(props: Props) {
  ...
  const reviewedOnly = parseToggle(props.searchParams?.reviewed);
  const includePast =
    props.mode === "state-seo"
      ? parseToggle(props.searchParams?.past)
  ...
  const heading =
    props.mode === "state-seo"
      ? `Soccer tournaments in ${props.stateName}`
        ...
        <div className="headerBlock brandedHeader">
          ...
          {props.mode === "state-seo" ? (
            <div className="subtitle" style={{ marginTop: 12, maxWidth: 860, fontSize: 14, lineHeight: 1.6 }}>
              <p style={{ marginTop: 0 }}>{props.intro}</p>
            </div>
          ) : (
            <div className="subtitle" style={{ marginTop: 12, maxWidth: 860, fontSize: 14, lineHeight: 1.6 }}>
              <p style={{ marginTop: 0 }}>
        ...
        {error ? (
          ...
          <>
            {props.mode === "state-seo" ? (
              ...
              >
                <div>
                  ...
                  <div className="input" style={{ display: "grid", gap: 4, alignItems: "center" }}>
                    <strong>{`${totalCount} soccer tournaments in ${props.stateName}`}</strong>
                    <span style={{ fontSize: 12, color: "#6b7280" }}>
                      Updated automatically from RefereeInsights listings
                    </span>
                  </div>
                </div>
                ...
                <div className="actionsRow">
                  ...
                  <a className="smallBtn" href={props.basePath}>Reset</a>
                </div>
              </form>
            ) : (
              <form className="filters" method="GET" action={props.basePath}>
                <div>
            ...
            {tournamentsSorted.length === 0 && (
              <p className="empty">
                {props.mode === "state-seo"
                  ? `No soccer tournaments are listed for ${props.stateName} yet.`
