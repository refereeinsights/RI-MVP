...
async function runRpc(limitPerTournament: number, dryRun: boolean) {
  return (supabaseAdmin as any).rpc("apply_inferred_venue_candidates_for_published", {
    limit_per_tournament: limitPerTournament,
    dry_run: dryRun,
  });
}
...
export async function GET(request: Request) {
  ...
  const { data: rows, error: rpcErr } = await runRpc(limit, true);
  ...
  const [{ data: tournaments, error: tErr }, { data: venues, error: vErr }] = await Promise.all([
    tournamentIds.length
      ? supabaseAdmin
          .from("tournaments" as any)
          .select("id,name,city,state,sport,start_date,updated_at,source_url,official_website_url")
          .in("id", tournamentIds)
      : Promise.resolve({ data: [] as any[], error: null }),
    venueIds.length
      ? supabaseAdmin.from("venues" as any).select("id,name,address,city,state,zip,venue_url").in("id", venueIds)
      : Promise.resolve({ data: [] as any[], error: null }),
  ]);
  ...
  for (const row of candidates) {
    ...
    if (!tid) continue;
    const entry = grouped.get(tid) ?? { tournament: tournamentById.get(tid) ?? { id: tid }, candidates: [] };
    entry.candidates.push({ ...row, venue: venueById.get(row.venue_id) ?? { id: row.venue_id } });
