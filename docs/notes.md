...
- **Corralio Slice 4.0A children and teams foundation implemented locally.** The authenticated home page now exposes the already-secured Slice 2 family model: owners can add and rename active children, add private child-owned teams, and edit team name/sport. Children receive the next existing six-token color automatically. V1 cardinality remains one child to many private teams and one child per private team; siblings can use separate same-named rows. Every mutation resolves the active owner household through the existing authenticated idempotent RPC, uses the authenticated Supabase client plus existing RLS/composite FKs, and accepts no browser household ID. Delete/archive/restore/reorder/color editing, shared-team junctions, team-child reassignment, collaboration, and schedule assignment remain deferred. An unapplied preflight migration narrows optional team sport to the existing nine-value taxonomy without coercion, with read-only catalog and rollback-only verification scripts prepared for manual production use. Schedule ingestion, source/event assignment, refresh, imported events, analytics, and weekend rendering are unchanged. Offline validation passed 47 tests, Corralio TypeScript/lint/build, and diff checks; no production SQL, push, or deployment occurred.
