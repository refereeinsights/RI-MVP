...
- **Corralio Slice 4.0A.1 broader sports taxonomy implemented locally.** Corralio's application-local planning list now adds Tennis, Swimming, Gymnastics, Track & Field, Golf, Wrestling, Cheer, and Dance for 17 bounded tokens total. All selectors continue sharing one constant; icons are presentation-only, `other` remains sports-only, and imported events still derive sport through schedule-source identity. No runtime TI taxonomy or matching dependency was found, and Corralio support remains separate from future TI eligibility. The unapplied preflight migration expands the team/source constraints and two existing source-sport RPC validators while preserving RLS, grants, locked search paths, ownership, and private URL controls. Catalog, rollback-only, and cleanup verification scripts are prepared. Canonical roadmap, ADR, and founder handoff documents now state the sports-first/TI-optional boundary. No Activity entity, event sport field, schedule assignment, conflict, leave-by, matching, weather, travel, manual-event, TI, or RI behavior was added. Offline validation passed 53 Corralio tests, TypeScript, lint, production build, and diff checks.
