...
- **Corralio Slice 3.1.1 password authentication and recovery implemented locally.** Returning users can sign in with email/password through the existing Supabase browser-client/session boundary, while Magic Link remains the new-user and passwordless fallback. Authenticated users can set or update the password on the same shared Supabase identity without changing household/RLS data. Forgot/reset routes use a server-only, validated `CORRALIO_SITE_URL`; recovery callbacks accept `type=recovery`, preserve existing email/Magic Link handling, and use bounded relative 303 redirects to avoid host-header and `0.0.0.0` regressions. Invalid credentials and recovery requests remain non-enumerating, raw provider errors are not exposed, and no password is stored or logged by Corralio. A manual shared Supabase Recovery template reference and allowlist/env instructions were added; no remote Auth setting, email template, deployment, production data, schema, or TI/RI recovery code changed. Validation passed 21 focused tests, Corralio TypeScript/lint/build, diff checks, and local HTTP route rendering. Live UAT with the designated `+parent.ryan` identity remains pending the manual env/template configuration.
