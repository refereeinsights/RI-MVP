...
- **Corralio Slice 4.0B schedule family assignment database-verified; browser UAT pending.** The manually applied narrow RPC derives the authenticated active owner household before locking its connected ICS source, validates active child/team ownership, persists only the mutually exclusive unassigned/child/team shape, and atomically propagates it to exact household/source imported events. The authenticated server action performs one RPC call with no admin client or household input; connected-source controls expose only active family choices, and unavailable historical assignments receive neutral copy. Existing weekend cards now show child context or `Child · Team`, while unassigned events retain source context. Canonical ingestion already copies the locked source assignment, so refresh, valid-empty recovery, and validated replacement remain unchanged; assignment itself preserves URL, sport, health, failure/claim state, and stable event/source identity. No table, column, index, trigger, remote request, cron work, analytics, TI/RI surface, or manual-event assignment was added. The corrected read-only catalog audit passed and confirmed the ICS-only imported identity index; rollback-only Household A/B verification reached `ROLLBACK` with all six final fixture counts zero, and the separate cleanup query returned zero for all seven categories including memberships. Offline validation passed all 63 Corralio tests, TypeScript, lint, production build, and diff checks. No retained synthetic data, private feed request, cron invocation, push, or deployment occurred.
