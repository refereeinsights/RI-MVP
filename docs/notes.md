...
- **Corralio brand asset handoff — partial.** Copied approved brand files from the external design source into the repo: `docs/brand/BRAND-SPEC.md`, 4 production-safe mark SVGs (`corralio-mark.svg`, `corralio-mark-light.svg`, `corralio-mark-mono-black.svg`, `corralio-mark-mono-white.svg`), 11 PNG app icons, and 1 social avatar PNG. The 6 wordmark and horizontal-logo SVGs contain live `<text font-family="Manrope">` and cannot ship as production assets without text-to-path conversion; they are staged under `apps/corralio/public/brand/source/` (non-shipping) pending conversion. **Blocker:** Inkscape CLI with Manrope font is required to complete conversion — neither is available in this environment.
