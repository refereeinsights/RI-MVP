...
- **Corralio Slice 4.4C applied-migration repair prepared.** The first rollback-only behavioral run exposed a PL/pgSQL ambiguity between the V2 RPC output variable and evidence-table column in its column-list `ON CONFLICT` target. The base migration now uses the named unique constraint, and `20260825_corralio_slice44c_provisional_lifecycle_evidence_fix.sql` safely repairs an already-installed function without data mutation. Catalog and architecture checks guard against recurrence. The repair and both Stage 2 verifiers remain to be run.
