...
- **Corralio Slice 4.4 location foundation Stage 1 is ready.** Corralio now ports TI's conservative venue normalization/matching behavior locally, but rejects the inherited capped global fallback as evidence of uniqueness: only complete, deterministically paginated state scopes with normalized-city filtering can produce matched or authoritative unmatched results. Household-origin classification uses exact normalized complete/base address equality before any venue query, never loose substrings, and private/insufficient locations retain their Slice 4.3 geocodes. A prepared, unapplied service-role-only forced-RLS match table stores one household-safe provenance row per event, a salted SHA-256 location fingerprint, matcher version, timestamps, and a 30-day unmatched recheck without duplicating raw locations, coordinates, routes, or canonical venue fields. Canonical venue UUIDs intentionally have no cross-domain FK; missing current rows trigger safe reconsideration. Matching runs only after successful initial/replacement/refresh persistence in a separately caught boundary, so it cannot change connection success or refresh health. Event geocodes and leave-by semantics remain unchanged; `venues_public` required no coordinate extension. Candidate reads reuse complete state scopes in 500-row pages and use bounded 100-ID event/result batches. Both Stage 2 SQL verifiers are prepared but unrun. All 158 Corralio/shared tests, TypeScript, Corralio lint, diff checks, and all four production builds passed; usage review found zero vendor calls, no new Vercel function/cron/render-time work, and only post-persistence database activity. Stage 2 still requires human migration application, both SQL runs, disposable browser/database UAT, and cleanup-zero proof. No migration, database mutation, provider request, cron, push, or deployment occurred. Verdict: **SLICE 4.4 READY AFTER LISTED FIXES**.
