...
- **TI past-tournament venue maps now offer explicit hotel-date recovery.** A selected venue whose tournament dates are completed or otherwise unavailable now shows blank check-in/check-out controls and clear date-required copy. The initial `no_dates` resolution still avoids HotelPlanner; submitting a valid future range refreshes the existing hotel pool, rates, pins, property/team-block dates, and attributed full-results path. Client and server checks reject past dates before provider access and retain the existing 730-day horizon. Local no-booking UAT used two HotelPlanner searches: completed Florida Elite/Earl Johnson Park recovered with Sep 18–20 (115 results/10 pins), while upcoming Autumn Ambush retained its automatic Sep 26–28 search (71 results/10 pins). No-selection, direct venue, and the new venue-hotel SEO pilot behaviors remain unchanged. The TI build also exposed and included a one-line repair for `7d5fb800`'s incorrect SEO-page stylesheet import. Focused tests passed 19/19, TI TypeScript and zero-warning lint passed, the TI production build passed with only existing displayed warnings, and diff validation passed. Report: `docs/reports/ti-past-tournament-hotel-date-recovery-2026-09-04.md`.

- **TI selected-venue HotelPlanner searches now remain centered on the venue.** Browser verification showed that when TI supplied both precise Dwight Merkel venue coordinates and `city=Spokane, WA 99205`, HotelPlanner replaced the venue point with its Spokane city centroid (`47.658889,-117.425`) and prioritized downtown inventory. The same request without `city` resolved near the intended venue at 5701 N Assembly St (`47.7100434,-117.4776833`). `/go/hotels` now omits only HotelPlanner's `city` parameter when a selected venue has coordinates, while preserving the coordinate destination, dates, attribution/Custom fields, Hotel Program routing, and city/locality fallback for searches without coordinates. This applies consistently to direct venue, tournament venue-card, and selected tournament-map `near this venue` handoffs; tournament-wide and generic searches remain unchanged. The 137-test focused hotel suite, explicit TI TypeScript validation, zero-warning TI lint, all four production builds, and diff validation passed; the full TI/RI builds displayed only their pre-existing warning backlogs. No database/provider write, booking, push, or deployment occurred.

- **TI venue travel actions are now grouped around the venue map.** On venue detail pages, the existing hotel action moved from the hero into the venue card immediately below the map; the existing directions row follows it, and the existing camping/RV action now sits beneath directions. Direct SEO entry retains the undated, attributed `Find hotels near this venue` action, while explicit tournament context retains `See hotels & rates on map`. No destination, attribution, eligibility, provider, tournament-date, schema, RI, or Corralio behavior changed. Focused tests and local port-3001 browser UAT passed without following an external travel link or initiating a booking.

- **TI direct SEO venue entry no longer silently switches into tournament context.** A venue page without an explicit valid `?tournament=` selection now keeps its static location preview neutral instead of making the preview overlay link to the first upcoming or merely linked tournament map. Its attributed `Find hotels near this venue` handoff also omits inferred tournament IDs and dates. This prevents a field-address/SEO visitor from inheriting unrelated tournament dates and dated hotel pins. Explicit tournament context still enables the existing tournament planning-map link and dated hotel behavior, and named upcoming tournaments remain available through their own links. Directions are unchanged; no provider, attribution, schema, tournament-map, RI, or Corralio behavior changed. Focused tests and local port-3001 no-booking UAT passed. Report: `docs/reports/ti-direct-venue-map-context-correction-2026-09-04.md`.

- **TI tournament venue-map hotel dates are now visible and explicitly editable.** The selected venue's hotel panel displays its active stay range and exposes native check-in/check-out controls behind `Change dates`. Input changes alone do not search; `Update hotels` validates checkout-after-check-in and refreshes the existing HotelPlanner results, prices, pins, property handoffs, team-block dates, and attributed `View all nearby hotels` link. Tournament dates remain the default, and direct venue visits without explicit tournament context still use broad attributed search rather than inferred hotel pins. Local no-booking UAT passed with two bounded HotelPlanner searches: 71 results/10 pins for Sep 26–28 and 72 results/10 pins after updating to Sep 27–29; an equal-date range was rejected without refresh. Seven focused tests, TI TypeScript, zero-warning TI lint, all four production builds, and diff validation passed. Report: `docs/reports/ti-venue-map-hotel-date-controls-2026-09-04.md`.

- **TI direct-venue hotel entry restored to attributed broad search unless tournament context is explicit.** Review of `7d5f0212` confirmed that an ordinary direct venue visit had begun silently selecting the earliest upcoming linked tournament, entering its dated map, and making hotel-marker clicks open one specific property. Supplied-page inspection confirmed that the property page technically permits date changes but makes them and the all-nearby-hotel escape less discoverable than broad results. Direct venue traffic without an explicit `?tournament=` now retains the existing `Find hotels near this venue` → attributed `/go/hotels` broad-results journey; explicit tournament context still uses the dated map and existing hotel pins. Broad search retains `Custom3` attribution, so this does not exchange measurement for usability. Tournament pages/maps, provider parameters, Hotel Program/commercial routing, schemas, RI, and Corralio are unchanged. Report: `docs/reports/ti-direct-venue-hotel-entry-correction-2026-09-04.md`.

## 2026-09-03

- **TI HotelPlanner booking-sync heartbeat implemented locally; database verification remains gated.** Audit confirmed the daily `15 5 * * *` admin-email cron is the ordinary sync caller and intentionally returns HTTP success when booking sync fails but email succeeds. Booking-row `synced_at` could not prove zero-row success, cancellation failure, or job health; the historical freshness-gap cause remains `UNPROVEN`. An unapplied migration adds a forced-RLS, service-role-RPC-only operational run table with database-clock start/finalize, terminal-once state, closed error stages, and aggregate provider-call/row counts. Sync now durably distinguishes succeeded/partial/failed, including zero-row success, while the admin email separately alerts on 30-minute stale-running and 36-hour stale-success states. A read-only aggregate operator report was added. No analytics, cron cadence, provider call, database access, revenue-producing hotel path, attribution, commercial routing, push, or deployment changed. HotelPlanner/revenue-path coverage passed 40/40, Referee TypeScript and zero-warning lint passed, the Referee production build passed with only its pre-existing displayed warning backlog, and diff checks passed; migration/catalog/rollback-only behavioral verification remains human-gated. Report: `docs/reports/ti-hotelplanner-booking-sync-heartbeat-2026-09-03.md`. Verdict: **HOTELPLANNER SYNC HEARTBEAT READY FOR DATABASE VERIFICATION**.
...
- **TI HotelPlanner August 2026 backfill applied to production.** Authorized Aug 1–30 window (narrowed from Aug 1–31 after a --end 2026-08-31 dry run returned 46 rows — 6 Sep 1 bleed-through from the half-open end boundary). Corrected --end 2026-08-30 dry run returned exactly 40 rows (matching baseline). Apply: 5 provider calls, 0 errors, parsed=40, persisted=40 to ti_hotel_bookings by upsert. Post-apply DB: 40 rows / confirmed=29 / cancelled=11 (acceptance criteria exact match). Financial: all-status booking value $19,192.14, confirmed-only $13,923.71, expected commission (all-status) $1,264.89. Aug 1–23: 8 rows (4c/4x), confirmed-only $1,270.70; Aug 24–30: 32 rows (25c/7x), confirmed-only $12,653.01. Attribution: 7 MATCHED, 0 ORPHANED, 22 MISSING_TOKEN. RI-attributed: 2 (custom8=app:refereeinsights). Aug 31 bookings: 0. Sep 1+ captured by daily rolling cron. Boundary fix and tests committed locally in cbaaf61f; not yet pushed.

## 2026-08-31

- **TI HotelPlanner revenue-safe synchronization and reporting hardened and post-implementation validated locally.** One authorized aggregate-only, read-only seven-day cancellation diagnostic used exactly one provider report call and established exact Cancelled/source/key behavior without provider/database writes or retained/printed customer data, booking identifiers, payloads, or signed URLs. The downstream sync now isolates cancellation refresh and preserves existing economics; commercial reporting uses the exact normalized TournamentInsights Source cohort, conservative status mappings, confirmed-only booking value/expected commission, separately labeled all-status provider-paid commission, confirmed-cohort attribution, `UNPROVEN` room nights, and `UNOBSERVABLE` HotelPlanner arrival. Historical backfill tooling is dry-run-first and bounded to the five documented sequential August chunks; report downloads are host/HTTPS/timeout/size/archive/row bounded. Search/property Custom3 is explicit; checkout's opaque provider-bundle continuity remains a documented attribution gap and was not changed. Offline post-implementation validation passed 134 HotelPlanner/revenue-path tests, Referee TypeScript/lint/build, and diff checks. The separately authorized August dry run then completed all five read-only provider calls, parsed 35 aggregate rows, persisted zero, and returned zero errors; its wrapper required a local async-`main()` compatibility repair before any provider call occurred. Because 35 differs from the captured 40-row August baseline, production persistence remains held pending aggregate variance review. The existing daily rolling-seven-day September scorecard is ready. No customer-facing HotelPlanner path, migration, cron, production write, deployment, or push changed. Detailed report: `docs/reports/ti-hotel-monetization-reporting-2026-08-31.md`.

## 2026-08-28

- **Removed the redundant RI ten-minute TI static-map proxy schedule.** A read-only aggregate check found 8,552 published canonical tournaments: 8,404 static maps are `ready`, 148 are `missing`, and zero are currently queued, processing, errored, or eligible for the worker's seven-day non-ready retry. TI already schedules the same lock-protected, hash-deduplicated worker hourly with a 50-tournament batch capacity, while RI's `*/10` proxy added 144 cross-project function invocations per day without a documented ten-minute map-readiness requirement or current backlog. The RI proxy route remains available for bounded manual/admin use, and TI's hourly cron remains unchanged. This reduces RI's configured cadence from 170 to 26 invocations/day and the currently deployed monorepo configuration baseline from 232 to 88/day after the next authorized push/deploy. Historical throughput remains unproven because the repository's `cron_job_results` migration is not present in any configured database; no schema change was made merely to satisfy this audit. No Mapbox call, tournament/map mutation, push, or deployment occurred.
- **RefereeInsights Travel MVP implemented and verified locally.** Added a focused `/travel` hotel-search surface for officials with generic and canonical-venue-anchored modes, privacy-safe same-origin POST submission, inline sanitized HotelPlanner results, responsive cards, calm retry/no-results/date/rate-limit handling, canonical SEO metadata, and sitemap coverage. Raw destination text stays out of RI URLs, analytics, logs, and new RI persistence. Existing RI venue inline results remain unchanged; venue fallback and tournament broader-search links now enter `/travel?venue_id=...`. Every property action uses the existing TI `/go/hotels/property` boundary with `referee_travel` request/flow context, canonical `referee` source-page attribution, `Custom8=app:refereeinsights`, and no direct provider URL. The generic TI search allowlist was expanded only to exact `referee_travel`; arbitrary sources remain denied. The shared-server-IP limiter remains unchanged (5/5 seconds and 30/minute): safe for expected launch volume but a documented capacity ceiling, with no spoofable client-IP trust or global weakening added.
- Restored the previously established deterministic booking-summary boundary: confirmed bookings count as matched only after a bounded-batch `Custom3` attribution-ID join to `ti_outbound_clicks`; orphaned-valid, missing, and invalid tokens remain separate, while lookup failure preserves booking/money/freshness/tournament totals and suppresses matched conversion. Focused RI/TI hotel tests passed 86/86, RI and TI TypeScript/lint checks passed, all four application production builds passed, and `git diff --check` passed. Bounded browser UAT used exactly two HotelPlanner searches for generic Spokane and anchored Dwight Merkel plus one existing venue regression request, and one Mapbox generic geocode; no booking was completed. Both searches returned 20 bounded results, the mobile anchored flow had no horizontal overflow, the existing venue fallback led to `/travel`, and browser checks had no application errors. Local analytics correctly skipped persistence under the existing development safeguard. No migration, venue write, new attribution ID, provider/commercial change, push, or deployment occurred. Detailed report: `docs/reports/refereeinsights-travel-mvp-2026-08-28.md`.

## 2026-08-27

- **TI Hotel Booking Attribution Reconciliation completed locally.** The current-branch audit classified all six named gaps as `STILL PRESENT` (tournament property-click outbound identifiers, tournament-map event shape fragmentation, `hotel_checkout_handoff` and `hotel_pin_click` persistence, Weekend hotel placement, and generic Book Travel lodging-search propagation); none were `ALREADY RESOLVED` or `NO LONGER APPLICABLE`. The narrow fixes now carry existing outbound/search IDs into the affected events and handoffs, preserve legacy map event names while giving them consistent source/placement context, persist the two previously rejected map events, and validate group-request `source_page_type` against the unchanged canonical set. Confirmed HotelPlanner bookings are reconciled in bounded batches against persisted `ti_outbound_clicks`: `MATCHED`, `ORPHANED_VALID_TOKEN`, `MISSING_TOKEN`, and `INVALID_TOKEN` partition confirmed bookings only when reconciliation is available. Lookup failure leaves matched/orphaned unavailable, preserves all status/money/freshness/tournament totals, raises an admin alert, and suppresses matched conversion. No identifier, source enum, Clarity integration, commercial/provider logic, migration, database call, HotelPlanner call, broader privacy cleanup, Corralio behavior, or unrelated RI product behavior changed. Verification passed 19 focused offline tests, explicit TypeScript checks and zero-warning lint for both affected workspaces, both production builds, and `git diff --check`; build-time warning backlogs are pre-existing. No push or deployment occurred.
- **TI HotelPlanner Stage 2 UAT completed and a local date-horizon guardrail was added.** Exactly three authorized calls confirmed recognized hotel/availability shapes for contextual Starfire 2026 (183 results) and generic Spokane 2026 (57), while a controlled Starfire 2028 search approximately 739 days ahead returned valid HTTP-200 JSON without a recognized result/success shape and was correctly rejected by the unchanged adapter as `UPSTREAM_REQUEST_FAILURE` / `upstream_rejected_response`. No raw payload, hotel list, credential, secret-bearing URL, customer data, or booking was retained. With no documented booking-search contract maximum, TI now applies an explicitly non-contractual 730-day UTC safety boundary to check-in and checkout. Later searches are classified `unsupported_date_horizon` before geocoding/provider invocation or outbound persistence, retain contextual/destination UI state, show calm booking-too-early copy, and cannot loop through a HotelPlanner fallback. Attribution, Hotel Program/commercial routing, provider response handling, zero-inventory semantics, RI, and Corralio are unchanged. Focused hotel coverage passed 94/94 plus TI TypeScript, zero-warning lint, the TI production build, and `git diff --check`; build-time warnings are pre-existing and outside this task. Historical `200` semantic failures may share this reproduced cause, but existing diagnostics cannot prove all did. No additional provider call, push, or deployment occurred.
- **TI hotel reliability and conversion Stage 1 implemented locally.** One authorized aggregate-only read inspected the 500 most recent `/api/lodging/search` sessions from 2026-07-28 through 2026-08-27 using only safe lifecycle/category fields: 492 succeeded with results, zero were successful zero-result sessions, eight failed, four successful sessions requested low-inventory fallback, and seven failures carried error code `200`. No prohibited/private field, row-level output, provider payload, write, configuration change, HotelPlanner call, or booking attempt was involved. The proven UI defect is fixed: contextual failure/empty states now present a positive attributed `/go/hotels` action; insufficient context remains `/book-travel`; team requests remain distinct. Search telemetry now uses bounded failure classifications and no longer persists provider-derived failure messages/response snapshots. The homepage adds one secondary travel module and `/book-travel` clarifies family versus 5+ room use without changing attribution, commercial routing, provider architecture, schemas, navigation, RI, or Corralio. Validation passed 99/99 focused hotel tests (including a deterministic clock repair for one pre-existing Team Hotel reporting fixture), TI TypeScript/lint/build, and intercepted desktop/mobile browser checks with zero HotelPlanner traffic. Canonical prompt: `docs/prompts/ti-hotel-reliability-conversion.md`; detailed report: `docs/reports/ti-hotel-reliability-conversion-2026-08-27.md`. The Stage 1 verdict at that checkpoint required approved provider UAT; the completed Stage 2 result and follow-up guardrail are recorded immediately above. No push or deployment occurred; all four builds remain required before any eventual push.

## 2026-08-25

- **Corralio Slice 4.4C provisional lifecycle and evidence is complete locally.** The applied base migration and additive named-conflict repair passed their exact repair, strengthened catalog, and rollback-only behavioral sentinels, covering the complete lifecycle/evidence/security/idempotency/atomicity/reconciliation contract and rollback cleanup zero. Database concurrency safety is enforced by the verified unique observation constraint and named-constraint `ON CONFLICT` without manufacturing an undeletable production tombstone. The aggregate report returned the correct clean pre-launch state with zero venue/evidence/lifecycle/eligibility counts. Independent usage and cleanup reads returned zero provider-call rows, quota rows/reservations, provisional venues, evidence, and transitions. Claude's signed-in local browser regression passed four schedules, the existing conflict, correct conditional leave-by absence for an empty origin, Family/Upcoming behavior, product-surface isolation, and console health without credentials or mutation. Stage 1's 168 tests, TypeScript, zero-warning lint, diff checks, and all four builds remain green. No provider/source-feed call, canonical write, backfill, cron, push, deployment, or retained fixture occurred. Verdict: **SLICE 4.4C COMPLETE LOCALLY**.
- **Corralio Slice 4.4C database verification passed.** The user applied the base migration and additive evidence-conflict repair. The repair, strengthened owner/catalog verifier, and rollback-only behavioral verifier all returned their exact pass sentinels, including `ROLLBACK CLEANUP ZERO`. This completes the migration/catalog/rollback SQL portion of Stage 2 with no retained verifier fixture.
- **Corralio Slice 4.4C applied-migration repair prepared.** The first rollback-only behavioral run exposed a PL/pgSQL ambiguity between the V2 RPC output variable and evidence-table column in its column-list `ON CONFLICT` target. The base migration now uses the named unique constraint, and `20260825_corralio_slice44c_provisional_lifecycle_evidence_fix.sql` safely repairs an already-installed function without data mutation. Catalog and architecture checks guard against recurrence. The repair and both SQL verifiers subsequently passed as recorded above.
- **Corralio Slice 4.4C provisional lifecycle and evidence Stage 1 is ready.** Audit classified TI's anonymous, subjective quick-check as not strong enough, so production evidence is exactly low-trust `ics_observation`, there is no strong-evidence writer, and zero promotion-eligible provisional venues is the correct expected state. The unapplied migration adds coherent active/suppressed/merged/reconciled state, immutable transitions, idempotent typed evidence using server-only versioned keyed HMAC fingerprints, exact and narrowly trusted merge authority, one-hop redirects, suppression, and reconciliation to existing canonical truth without canonical writes. Generic ICS volume never becomes strong; raw/private source material is not stored and anonymized evidence survives source deletion. Machine-failing catalog and rollback-only behavioral verifiers plus an aggregate-only report are prepared. Stage 1 passed 168 Corralio tests, TypeScript for the app and report, zero-warning lint, diff checks, and all four production builds; existing RI/TI warnings remain unrelated. No SQL was applied and no provider/source-feed call, database mutation, evidence backfill, cron, canonical/public venue write, push, or deployment occurred. The dirty canonical ADR/roadmap files were preserved, with proposed amendments recorded in the report. Verdict: **SLICE 4.4C READY AFTER MIGRATION AND STAGE 2 VERIFICATION**.
- **Corralio Slice 4.4B shared provisional venues is complete locally.** After the user applied the migration, the exact owner-level catalog verifier and rollback-only behavioral verifier passed, including forced RLS/grants/RPC security, state coherence, manual rejection, cross-household reuse, suppression, authenticated denial, `ROLLBACK`, and cleanup zero. Live probes confirmed anon/authenticated denial and service-only access; a disposable canonical-conflict check created no provisional data and cascade-cleaned to zero. Two genuinely simultaneous household RPC calls returned exactly one `created` and one `reused`, converged both associations on one identity, and were suppressed/detached; the user removed the exact synthetic tombstone and an independent check returned zero for that key, the provisional table, and the external-call ledger. Aggregate coverage/duplicate/orphan metrics were all zero on the clean pre-launch dataset. Browser regression returned 200 with meaningful content, expected controls, and no framework overlay; temporary artifacts and server were removed. Stage 2 passed 156 current Corralio tests, TypeScript, zero-warning Corralio lint, diff checks, and all four production builds; existing RI/TI warnings remain unrelated. There were zero provider calls, canonical/public writes, retained fixtures, backfills, crons, pushes, or deployments. Verdict: **SLICE 4.4B COMPLETE LOCALLY**.
- **Corralio Slice 4.4B shared provisional venues Stage 1 is ready.** The final product decision retires 4.4B.1: a generic successfully persisted ICS event may contribute low-trust preliminary place evidence, while manual events remain ineligible and provisional identity can never authorize canonical/public data. The repaired v3 prompt and implementation add a structurally isolated, forced-RLS, service-only shared table plus a mutually exclusive typed ID on the existing private event-match row. Conservative parsing requires a named place but no sports-keyword allowlist; bare addresses, private/logistical values, and orphan sublocations remain usable without creating shared data. Shared rows contain no household/event/source ID, complete event text, URL, credential, note, child/team, or route data. The transactional invoker RPC validates ICS origin, persisted geocode equality, canonical-first state and race checks, complete provisional candidates, deterministic name/locality locking, idempotent reuse, and durable suppression tombstones. A bounded separately caught post-geocode pass reuses Slice 4.3 coordinates and adds zero external calls. Catalog, rollback-only behavioral, and aggregate coverage/duplicate/orphan tooling are prepared. Stage 1 passed 205 regressions, Corralio TypeScript/lint, diff checks, and all four production builds; existing RI/TI warnings remain unrelated. No SQL was applied, database/provider call made, historical row processed, public/canonical venue changed, cron added, push made, or deployment performed. Exact ADR-008/030/033 amendments are reported but not applied over unrelated working-tree edits. Verdict: **SLICE 4.4B READY AFTER MIGRATION AND STAGE 2 VERIFICATION**.
- **Corralio Slice 4.4B stopped correctly at its source-trust gate.** The complete reviewed prompt now lives at `docs/prompts/corralio-slice-4.4b-provisional-venue-foundation.md`. Audit found that Corralio persists only generic `source_type='ics'`; its display name and sport are user-controlled, URLs are protected credential-bearing inputs, and the shared fetcher/normalizer proves safe valid ICS rather than publisher identity. ADR-019 also deliberately defers direct sports-platform integrations. The system therefore cannot distinguish qualifying team/league/platform evidence from arbitrary ICS using existing trusted non-secret metadata. The audit report defines the smallest prerequisite: a narrow server-derived, versioned, non-client-writable source classifier that defaults to ineligible, clears on URL replacement, performs no automatic historical backfill, and requires separately approved trust evidence. The documentation-only audit passed `git diff --check`; application tests/builds were not rerun because the mandatory stop gate prohibited schema/runtime work. Per the prompt, no provisional schema/runtime implementation, venue/evidence write, migration, verifier, provider call, database mutation, ADR change, cron, push, deployment, backfill, or production enablement occurred. Existing ICS planning and Slice 4.3/4.4 behavior remain unchanged. Verdict: **SOURCE CLASSIFICATION PREREQUISITE NOT MET — SLICE 4.4B NOT READY**.
- **Corralio Slice 4.4 location foundation is complete locally.** Corralio ports TI's conservative matching behavior locally while requiring complete, deterministically paginated state scopes with normalized-city filtering; it never infers uniqueness or authoritative unmatched status from TI's capped global fallback. The applied service-role-only forced-RLS provenance table stores no raw location, coordinates, routes, or canonical fields; advisory canonical UUIDs have no cross-domain FK, missing venues re-evaluate safely, unmatched results recheck after 30 days, and private origins are excluded by exact normalized complete/base equality before any canonical query. Matching remains separately caught after successful initial/replacement/refresh persistence and cannot alter connection success or source health. The strengthened catalog verifier passed the exact schema/RLS/grant/no-coordinate-view contract, and the rollback-only behavioral verifier passed all four states, authorization, invalid-state, cascade, and post-rollback cleanup-zero assertions. Disposable UAT produced exactly one matched, one private-skipped, and one unmatched event using a real read-only canonical candidate; preserved all three event geocodes; retained estimated leave-by on the two routed cards; and rendered cleanly at desktop and 390px mobile with the existing navigation chooser, no new venue UI, overflow, overlay, page error, or console error. Post-render usage remained zero external API calls. Exact cleanup returned zero fixture Auth, household, membership, source, event, match, weekly-engagement, API-audit, and quota records; no existing or canonical venue row was written, and all temporary sensitive artifacts were removed. Stage 1 passed 158 tests, TypeScript, lint, diff checks, and all four production builds; Stage 2 changed verifier assertions and notes only. No provisional/canonical venue creation, Overture, Nearby, provider request, cron, push, or deployment occurred. Verdict: **SLICE 4.4 COMPLETE LOCALLY**.
- **Corralio Slice 4.3 is complete locally.** The user reported the migration and both provider keys applied. Live verification corrected Geocodio v2's request country to `USA` (responses remain `US`) and repaired a proven claim bug: PostgREST UPDATE `.or(...is.null...)` silently matched zero rows, so origin/event claims now use atomic unclaimed-then-stale compare-and-set updates. The final disposable UAT audit recorded exactly 4 Geocodio plus 2 OpenRouteService calls, two persisted/routable event estimates, one graceful definitive geocode failure, labeled estimated leave-by UI, unchanged four-choice directions, clean desktop/mobile layouts, and zero browser/network errors. Exact cleanup returned zero household/member/source/event/quota rows, removed the Auth identity and temporary artifacts, and preserved the smoke identity. Two earlier post-claim-fix runs each used the same deterministic six-call provider phase before exposing fixture/harness-only problems, making the explicitly user-authorized session total 12 Geocodio plus 6 ORS calls; pre-fix claim skips were nonbillable. The catalog verifier passed all columns/defaults, constraints/FKs, RLS/grants, trigger, and function contracts after correcting its alias sort and making trigger lookup role-independent. The rollback-only verifier reached `ROLLBACK` with all storage, claim, quota, constraint, and isolation assertions passing; a separate read-only check returned zero fixed-ID Auth/household/membership/event/audit/quota rows. Final validation passed 133 Corralio/shared tests, TypeScript, Corralio lint, diff checks, and production builds for all four workspaces; the final audit ledger also satisfied the usage/cost review. Existing RI/TI build warnings remain unrelated and non-blocking. Verdict: **SLICE 4.3 COMPLETE LOCALLY**. No push or deployment occurred.
...
- **TI admin dashboard email + HotelPlanner booking sync implemented locally.** Audited the HotelPlanner `getReport` endpoint using a live call against production credentials (`purchasedDateStart`/`purchasedDateEnd` in `mm/dd/yyyy`, POST with JSON body). Downloaded and parsed the real xlsx response to verify all column names — confirmed 74 columns with real booking data: `Itinerary`, `Confirmation`, `Status`, `Purchased`/`Check-In`/`Check-Out` (Excel serial dates), `Rm.s`, `Hotel`/`Hotel City`/`Hotel State`/`Hotel Country`, `Avg Rate`, `Total`, `Exp Comm USD`, `Comm. USD`, `Paid`, `Custom1–8`, `IsMobile`, `Currency`, and `Cancel Date`. Attribution join uses `Custom3` (format: `attr:{outbound_attribution_id}`). Added three new files: `supabase/migrations/20260824_ti_hotel_bookings.sql` (new table, unique on `itinerary_number`, RLS locked to service_role only; not applied — production DB is live), `apps/referee/lib/hotelPlannerBookingSync.ts` (fetches getReport, downloads xlsx from S3, parses inline XML without external library, maps using real column names, upserts to `ti_hotel_bookings`), `apps/referee/lib/tiAdminDashboardEmail.ts` (6-section compact email: Business Snapshot via `get_admin_dashboard_email_tiles` RPC, Traffic from `ti_outbound_clicks`, Hotels from `ti_hotel_bookings` summary, Planning from `ti_map_events` activations, Tournament Partners from pending contacts/reviews, Alerts). New cron route `apps/referee/app/api/cron/ti-admin-dashboard-email/route.ts` runs booking sync then sends email. TypeScript clean. Recipients read from `TI_ADMIN_DASHBOARD_EMAILS` (falls back to `RI_ADMIN_DASHBOARD_EMAILS`/`RI_ADMIN_EMAIL`). Migration must be applied manually before cron runs first production sync.

## 2026-08-24

- **Corralio Slice 4.2A core usage measurement complete locally after applied-schema and browser/database UAT.** The user manually applied both reviewed migrations. Live probes and the SQL catalog run confirmed the new table/column, RLS/role boundary, exact RPC signatures, postgres ownership, definer/search-path configuration, updated owner-RPC overload, and acquisition trigger boundary. The rollback verification initially found a fixture-only role bug—its timestamp assertion read the intentionally admin-only table before `RESET ROLE`; the corrected script resets for the assertion, restores `authenticated` for later RPC calls, then passed all validation, no-op, UTC week, accumulation, immutable-provenance, isolation, and final `ROLLBACK` checks. Exactly two disposable, confirmed Auth identities were created via the admin API in separate fresh browser contexts; raw creation responses were suppressed, credentials/emails stayed in memory, and only IDs entered a mode-0600 `/tmp` cleanup ledger. Their first real Family mutations used the locally rejected `ftp:` input, created no source, made zero fixture-domain browser requests, and produced the expected safe UI error. The direct context stored `direct`; the invited context's exact 30-day HttpOnly/Lax, local-HTTP non-Secure cookie produced `ti_weekend_planner_opt_in` specifically through `supabaseStore.ts`, empirically confirming nested request-scope cookie access. Direct provenance mutation was rejected with `23514`. Two active inert sources plus two overlapping events produced one verified conflict and one weekly row on real navigation; hover caused no request or row/timestamp change, while click navigation preserved `first_viewed_at` and updated `last_viewed_at`. Adding 198 controlled rows exercised the exact 200-candidate unavailable state, and the same row correctly retained verified `true/1` while adding the unavailable flag. The browser reported no console/page errors. Exact household deletion and independent queries returned zero household/membership/child/team/source/event/weekly rows; both exact Auth users were then deleted and independently absent, the established smoke identity remained unchanged, and the ledger/screenshot/harness/server were removed. Stage 1's 124-test, TypeScript, lint, build, and diff results remain green. No feed DNS/HTTP, cron, push, deployment, credential disclosure, dashboard/vendor/event stream, or Slice 4.3 signal was added. Verdict: **SLICE 4.2A COMPLETE LOCALLY**.

- **Corralio Slice 4.2A core usage measurement Stage 1 implemented; manual schema application and UAT remain gated.** Prerequisite `25a5ba9e` and the completed Slice 4.2 notes were verified. A live read-only schema probe confirmed the existing household/source/child/team timestamps and activation inputs and confirmed no acquisition column exists; activation remains a current-state report query with no new activation events, and neither RI's analytics table nor PostHog was adopted as cross-product infrastructure. Two unapplied migrations add an RLS-locked, service-role-readable household/week aggregate plus a postgres-owned owner-derived RPC, and an immutable default/direct-or-TI-planner household provenance tag plus trigger and correctly dropped/recreated two-argument owner-household RPC. The weekly RPC safely no-ops without an active source, explicitly validates all verified inputs, preserves unavailable/verified overlap, first-view time, maximum conflict count, and OR-accumulated weekly observations, and uses a UTC ISO week distinct from the unchanged browser-local weekend. Data retains for the household lifetime and cascade-deletes. Exact invite attribution is captured in a 30-day HttpOnly/Lax transport-secure cookie; both independent owner-household call sites pass its pure validated value. A ref-guarded client effect records the rendered browser-local result once per real mount through a separately testable orchestration module; RPC failures never affect UI and produce only one constant payload-free log line, while real viewer-resolution null/no-household outcomes remain silent. The manual report uses current >=2-source activation, labels the current week partial, computes return across the two latest completed UTC weeks, and exposes overlapping verified/conflict/unavailable sets. Catalog and rollback-only behavioral scripts are prepared but unrun. All 124 Corralio/shared tests passed, including proof that the prescribed `ftp:` first-mutation input reaches neither DNS nor fetch; TypeScript, lint, and the Next 14.2.35 production build passed. Source review confirmed force-dynamic `/`, no `loading.tsx`, `/` revalidation after mutations, and source-gated mounting. Stage 2 still requires a human to apply the weekly migration then the acquisition migration (no schema dependency; documented operational order), run both SQL checks, and perform the authorized two-identity browser/DB UAT, including hover/prefetch zero-row verification, nested schedule-store cookie access, raw Auth-response suppression, exact cleanup, independent absence checks, and smoke-user preservation. Slice 4.3 signals, dashboard/vendor/event-stream scope, feed/cron, push, and deployment remain deferred. Verdict: **SLICE 4.2A READY AFTER LISTED FIXES**.

- **Corralio Slice 4.2 basic family conflict detection complete locally.** Prerequisite commits `f6fc4e0d` and `d5efbef6` plus the completed 4.1B notes baseline were verified. No migration was needed. One shared pure pipeline now selects the exact browser-local Friday-through-exclusive-Monday event set before deriving deterministic canonical pairwise overlaps from distinct stable event IDs and positive valid absolute-time intervals; touching boundaries and missing/invalid/zero/negative ends do not participate, no duration or all-day state is guessed, and the count means unique pairs. Same-child classification uses the private resolved active owning-child UUID, including team ownership, never labels/colors; unassigned events retain source fallback. `This Weekend` adds a compact factual pair/overlap summary and restrained non-color-only card badges. An exactly-200-row server candidate result preserves events but suppresses definitive conflict claims with bounded unavailable copy. The signed-out preview remains deferred. Approved browser UAT used one disposable household/membership with 2 children, 3 teams, 6 inert unfetched `.invalid` sources, and 7 events. Database preflight proved the disconnected overlapping control existed but was excluded, leaving six active events that produced exactly one same-child, one different-child, and one assigned/unassigned pair plus an unrelated Sunday event. Playwright passed 1280×900 and 375×812 in light/dark: exact summary/badges/times, fallback, disconnected exclusion, directions, readability, no overflow/private URL, and zero browser/HTTP errors. A pre-hydration harness click also exposed an existing native password-form GET risk; the password action is now inert until hydration with explicit click/Enter handlers, and coverage prevents regression. No external destination received the local smoke form data. Exact service cleanup and an independent readback returned zero fixture household/membership/child/team/source/event rows while preserving the smoke auth user and original no-household state. Final validation passed 102 tests, TypeScript, lint, production build, diff checks, and React/Next.js review. Travel feasibility, leave-by, routing/traffic, resolution/recommendations, venue/tournament/travel, collaboration, notifications, analytics, persistent conflict history, cron, deployment, and push remain deferred. Verdict: **SLICE 4.2 COMPLETE LOCALLY**.

- **Corralio Slice 4.1B family and schedule lifecycle complete locally.** After the user applied `20260823_corralio_slice41b_family_schedule_lifecycle.sql` from Stage 1 commit `f6fc4e0d`, consolidated catalog verification passed all function ownership/security/grant, RLS, source-delete removal, archive-column denial, ordinary edit, source-URL privacy, service cleanup, and no-suppression-column checks. Rollback-only Household A/B verification passed lifecycle semantics, connected/disconnected assignment cleanup, manual-event preservation, bounded stale/foreign behavior, privilege denial, normal editing, isolation, and forced-failure atomicity; independent cleanup returned zero verification rows. The approved smoke identity had no Corralio household, so browser UAT created a wholly disposable synthetic household/membership with one child, Team A, still-active Team C, Sources A/B/C, an unrelated control, and four weekend events (`1/1/1/2/4/4` household/membership/child/teams/sources/events); all source strings used unfetched `.invalid` hosts. Native confirmation dialogs passed at 375×812 light and 1280×900 dark without mutation. At 1280×900 light, existing Source B unassign/reassign passed; Source A disconnect hid its active event while preserving its rows/assignment until subsequent cleanup; Team A removal archived it, unassigned connected Source B plus disconnected Source A, preserved Team C, and produced source fallback; child removal archived the child and Team C, unassigned connected Source C, removed archived choices, and produced Source C fallback. The unrelated control remained unchanged. Final 1280×900 light and 375×812 dark states had no overflow, browser error, failed response, source URL exposure, or retained private input. A harness-only strict-locator ambiguity stopped after Team A removal; exact database inspection proved the safe partial state and the resume performed only the pending child removal. Post-UAT SQL confirmed both teams/child archived, Source A disconnected/unassigned/claim-invalidated and preserved, Sources B/C connected/unassigned, control unchanged, and all four events retained/unassigned. Controlled service-role deletion of only the disposable household cascaded the fixture; both immediate and independent exact-ID checks returned zero household/membership/child/team/source/event rows, while the smoke auth user remained and returned to its original no-household baseline. Stage 1’s 89 tests, TypeScript, lint, diff checks, isolated production build, and React/Next.js review remain green. No real family data, feed/cron, permanent erasure/history/reconnect, manual-event lifecycle, conflicts, leave-by, maps/routing, matching, travel/HotelPlanner, collaboration, notifications, providers, analytics, push, or deployment changed. Verdict: **SLICE 4.1B COMPLETE LOCALLY**.

## 2026-08-23

- **Corralio Slice 4.1B family and schedule lifecycle Stage 1 implemented; migration/UAT remain separately gated.** After verifying prerequisite commits `85588cee`, `4a7782ae`, and `a99ef055` plus the completed notes baseline, the unapplied `20260823_corralio_slice41b_family_schedule_lifecycle.sql` removes authenticated source hard-delete access, reserves child/team `archived_at` for trusted RPCs while retaining approved name/sport editing, and adds separate owner-authorized transactional boundaries for non-destructive disconnect, team removal, and child removal. Disconnect preserves the private URL, source/event rows, assignment, stable IDs, sync/failure history, and clears only active claim state; team removal archives then unassigns all connected/disconnected team sources and their imported events; child removal archives the child and active teams then unassigns direct/team sources and imported events, including disconnected sources. Manual events remain unchanged. Missing/foreign/stale/repeated targets share bounded false without metadata leakage. The existing assignment RPC retains behavior but adopts family-before-source locking to avoid lifecycle deadlocks. Active planning now applies its manual-or-active-source predicate in the database before chronological ordering and the 200-row limit, with child/team reads still concurrent and no event suppression column or client filter. `/family` gains native accessible confirmation dialogs for Disconnect schedule, Remove team, and Remove child with accurate consequence copy and no source URL exposure. Read-only catalog, rollback-only Household A/B/forced-failure, and exact cleanup scripts are prepared but not run because SQL was not applied. All 89 Corralio tests, TypeScript, lint, diff checks, an isolated production build, and React/Next.js review passed. Read-only Playwright authenticated successfully, but its approved smoke household has no family/source records, so it could not render populated controls; no mutation occurred. Stage 2 requires separate approval for migration application, database verification, an approved disposable fixture containing one child, Team A, a second active Team C, Sources A/B/C, and an unrelated control, lifecycle browser UAT, cleanup, and independent zero-row confirmation. No feed/cron, real UAT mutation, permanent erasure/history/reconnect, manual-event lifecycle, conflicts, leave-by, routing/maps, matching, travel/HotelPlanner, collaboration, notifications, providers, analytics, push, or deployment occurred. Verdict: **SLICE 4.1B READY AFTER LISTED FIXES**.

- **Corralio Slice 4.1A landing and onboarding clarity complete locally.** The baseline signed-out page led with an unsupported `Private pilot` badge, weekend/single-schedule copy, and a login panel; account creation was secondary helper text, Family setup led with iCal/ICS and import terminology, and no shipping legal/support destination existed. After verifying `85588cee` and `4a7782ae` as ancestral Slice 4.1 implementation/UAT evidence, signed-out `/` now leads with the canonical sports-family positioning, multi-kid/multi-team problem, one-plan outcome, and team-app differentiation. A server-rendered static Saturday/Sunday example uses synthetic family/team/activity/location copy only, performs no household query/fetch or persistence, and makes no conflict claim because conflicts remain Slice 4.2. Get Started is primary and reaches the established Magic Link account path; Sign in reaches unchanged password access, with recovery and shared auth semantics preserved. The unverified pilot badge was removed, a bounded private-by-default reassurance reflects existing household/RLS behavior, no nonexistent legal/support link was fabricated, and `noindex, nofollow` remains unchanged. Family setup now leads with Calendar link and Connect schedule/team schedule, moves iCal/ICS into secondary help, and uses Change assignment while preserving Connected, Refresh delayed, Schedule needs attention, and Replace calendar link semantics. Local Playwright replaced the unavailable `agent-browser` executable and passed signed-out `/` plus read-only authenticated `/` and `/family` at exact 375×812 and 1280×900 in actual emulated system light/dark preference, including comprehension, CTA destinations, 3px focus, brand/preview treatment, private empty URL inputs, no overflow, and no console/page/network error. The smoke projection had no connected source, so populated lifecycle controls were not manufactured or live re-tested; no Magic Link email, family/schedule form, feed, retained row, or cron was submitted. Four focused tests, all 79 Corralio tests, TypeScript, lint, diff checks, isolated production build, and React/Next.js review passed. No About/marketing expansion, legal placeholder, pricing/Pro, social proof, interactive demo, conflicts, tournament/travel/HotelPlanner, analytics, auth provider, backend/migration/RPC, push, or deployment was added. Verdict: **SLICE 4.1A COMPLETE LOCALLY**.

- **Corralio Slice 4.1 family weekend plan complete locally after populated-state UAT.** The dedicated UAT household now demonstrates the three required connected-schedule states without disturbing Casey/Owls: Avery/Strikers renders child plus team, Drew/Wolves renders Drew only with no team, and the connected but unassigned Emerson/Ice Owls schedule renders its source fallback. Four children and four connected schedules produced three simultaneous Saturday cards spanning all assignment treatments plus a Sunday card, satisfying the minimum for distinguishable children, team assignment, child-only assignment, unassigned fallback, two populated weekend days, navigable locations, and scanning/wrapping density. Claude in Chrome passed `/` at 375×812, 768×900, and 1280×900 in light and dark, including navigation, chronological non-empty day grouping, identity treatment, contrast, wrapping, and absence of clipping or horizontal overflow. The constrained responsive passes used a same-origin exact-size iframe because the extension could not resize its real window; dark screenshots temporarily applied the app's own `prefers-color-scheme` rules because no in-app theme toggle exists, and both non-destructive harness changes were removed afterward. The directions chooser passed at mobile and desktop widths; `/upcoming` and `/family` shell regressions passed; browser inspection found no application console/network error, overlay, rendered ICS URL, secret parameter, or `.ics` reference; and anonymous credential-omitted requests redirected from `/family`, `/upcoming`, and `/account` while `/` retained its public landing behavior. No direct database edit, source deletion/disconnection, private URL disclosure, push, or deployment occurred during the sweep. Verdict: **SLICE 4.1 COMPLETE LOCALLY**. Slice 4.2 conflict work and every other recorded deferral remain unchanged.

## 2026-08-21

- **Corralio team-scoped calendar connection completed locally.** Expanded Edit team now contains a reset-on-success private iCal/ICS URL form. Its server action accepts only team ID plus URL, resolves the authenticated owner household and active team, derives child/name/sport server-side, and reuses the existing SSRF-safe importer and canonical persistence boundary. New sources are created already assigned through the existing validated source-creation RPC; no migration or parallel ingestion path was added. Already-connected URLs are not silently reassigned and instead direct the user to the established assignment control. Private URLs remain out of props, rendered retained values, messages, and logs. A browser-feedback follow-up made the whole team header the native disclosure target, underlined the team name, retained sport context, and added a rotating chevron rather than leaving a disconnected `Edit team` target. The interaction keeps a 44px target, visible focus, and native pointer/keyboard behavior. All 75 Corralio tests, TypeScript, lint, diff checks, an isolated production build, and a read-only runtime smoke passed; the follow-up additionally passed eight focused family/team tests, TypeScript, lint, diff checks, and an isolated production build. Populated `/family` UAT passed at 390×844 and 1280×900 in both system modes for two retained teams, full-header/name pointer toggling, Enter/Space toggling, chevron state, conditional calendar-form visibility, 3px focus, and absence of overflow, failed requests, overlays, console errors, and page errors. No form was submitted, real feed fetched, retained data mutated, schema/RPC/cron changed, pushed, or deployed.

## 2026-08-20

- **Corralio Slice 4.1 family weekend plan implemented locally; populated-state UAT remains.** The unchanged broad server query still returns at most 200 household events in ascending `starts_at` order and the browser still applies the exact local Friday-through-exclusive-Monday boundary. A pure presentation helper now derives bounded child/team/source/unavailable identity from authorized family rows and groups applicable events into chronological, non-empty browser-local day sections; displayed times still use each valid event timezone, preserving the documented mixed-timezone day-heading/time limitation. Cards communicate child or `Child · Team` in text plus the owning child's existing semantic color, while schedule fallback, `Unassigned event`, and `Previous assignment unavailable` remain neutral. Source-derived sport, title, time, authorized location/field, directions behavior, stable identity, assignment meaning, and private URL boundaries remain intact. The family and event reads now run concurrently without changing columns, scope, window, order, or limit. Read-only browser checks passed signed-out `/` at 375×812 light and all three authenticated routes at 375×812, 768×900, and 1280×900 in both modes, including exact navigation, active state, keyboard navigation, responsive layout, and no overlay/console/page error; the smoke projection had no sources/events, so populated cards and directions were not exercised. The user's newly added child/events were not touched, and no synthetic fixture was created without the prompt's separate approval. Eight focused tests, all 70 Corralio library/schedule tests, TypeScript, lint, production build, and diff checks passed. No roadmap/ADR, migration, RPC, schema, cron, feed, retained data, external integration, push, or deployment changed. Slice 4.2 retains overlap/conflict UI, prioritization, and engagement analytics; 4.3 retains origin, routes, buffers, leave-by, caching/staleness/cost/traffic; 4.4 retains TI/tournament/official-link/weather and venue matching/provenance/writes; 4.5 retains travel inference, HotelPlanner/hotels, commercial attribution, and booking. Later approved work still owns filters/preferences, manual events, collaboration/roles/transport coordination, notifications, analytics infrastructure, providers, remaining disconnect/suppression, theme preferences, marketing demo, and Pro pricing/paywalls. Verdict: **SLICE 4.1 READY AFTER LISTED FIXES** (populated-state browser UAT only).

- **Corralio Slice 4.0C branded product shell complete locally.** After repository-evidence gates confirmed the completed Slice 4.0B UAT and production-ready outlined brand/font handoff, Corralio now loads checked-in Manrope 4.504 through `next/font/local`, uses only approved lockups/icons/social artwork, and has semantic light/dark tokens driven solely by `prefers-color-scheme`. The shared authenticated consumer shell exposes exactly This Weekend (`/`), the intentional query-free Upcoming placeholder (`/upcoming`), and Family (`/family`); Family exclusively owns the existing family and schedule-management presentation, while Account/Sign out remain secondary. Signed-out, account, forgot-password, and reset-password surfaces now use the approved adaptive lockup. Existing event query/window/rendering, family and assignment semantics, ingestion/refresh/replacement, stable identity, RLS, and private URL boundaries were preserved; only the presentation/data-loading boundary and dual-route revalidation changed. Browser UAT passed signed-out `/` plus all three authenticated routes at 375×812, 768×900, and 1280×900 in both system modes, with correct active/lockup states, local font, pointer and keyboard navigation, 3px focus, no overflow, overlays, console errors, or page errors. Recovery routes passed at 375px in both modes; protected redirects and branded `/account` passed. The authenticated smoke identity had an empty Corralio projection, so no retained data was mutated and mutation-dependent family/source/event states were explicitly left to prior UAT plus the passing regression suite. All 65 Corralio tests, TypeScript, lint, production build, and diff checks passed. No migration, real feed, cron, production-data mutation, push, or deployment occurred; Slice 4.1+ conflict, leave-by, tournament, venue, routing, travel/hotel, collaboration, analytics, provider, and event-card-refactor work remains deferred.

- **Corralio brand asset handoff complete.** Converted the six approved Manrope ExtraBold wordmark/horizontal-lockup sources to self-contained vector paths with Inkscape 1.4.4, completing all 10 production SVGs under `apps/corralio/public/brand/`. Mechanical validation found no live text, font dependency, embedded raster data, script, or external dependency; all SVGs parse and retain their locked viewboxes and approved colors. All six source/production pairs rendered byte-identically at 1200px, and light/dark lockups passed visual checks on warm-white/navy at large and 120px minimum width. Live-text design sources now live outside the deployable public tree at `docs/brand/source/`. Added the official Google Fonts Manrope 4.504 variable TTF (weights 200–800), OFL license, font log, and metadata under `apps/corralio/app/fonts/`; Slice 4.0C must use the checked-in font through `next/font/local`, never a build-time Google Fonts fetch. `docs/brand/BRAND-SPEC.md` records asset mappings, provenance, checksums, and validation. The Slice 4.0C artwork blocker is resolved.

- **Corralio Slice 4.0B schedule family assignment complete.** The manually applied narrow RPC derives the authenticated active owner household before locking its connected ICS source, validates active child/team ownership, persists only the mutually exclusive unassigned/child/team shape, and atomically propagates it to exact household/source imported events. The authenticated server action performs one RPC call with no admin client or household input; connected-source controls expose only active family choices, and unavailable historical assignments receive neutral copy. Existing weekend cards show child context or `Child · Team`, while unassigned events retain source context. Canonical ingestion already copies the locked source assignment, so refresh, valid-empty recovery, and validated replacement remain unchanged; assignment itself preserves URL, sport, health, failure/claim state, and stable event/source identity. The corrected catalog audit passed; rollback-only Household A/B verification and separate cleanup returned zero retained fixture rows. Browser UAT passed at `390px` and `1280px` for unassigned fallback, child-only assignment/reload, child-scoped teams, team assignment, reassignment, unassignment, event propagation, manual-event exclusion, URL secrecy, responsive layout, and error-free requests. The unrelated existing source and all 151 events remained unchanged, and independent cleanup restored one source, 151 events, and no synthetic family rows. No real feed, browser replacement/refresh, cron, push, or deployment occurred; canonical refresh/replacement assignment preservation remains covered by rollback SQL. Offline validation passed all 63 Corralio tests, TypeScript, lint, production build, and diff checks.

- **Corralio Slice 4.0A.1 broader sports taxonomy complete.** Corralio's application-local planning list adds Tennis, Swimming, Gymnastics, Track & Field, Golf, Wrestling, Cheer, and Dance for 17 bounded tokens total. All selectors share one constant; icons are presentation-only, `other` remains sports-only, and imported events derive sport through schedule-source identity. No runtime TI taxonomy or matching dependency was found, and Corralio support remains separate from future TI eligibility. The migration was manually applied with zero incompatible team/source values. Catalog verification confirmed exact enforcement, function signatures, trusted ownership, `SECURITY DEFINER`, locked search paths, intended grants, RLS, and calendar URL protection. Rollback verification passed broader/existing sports, bounded rejection, optional-team behavior, and household isolation; cleanup returned `0/0/0/0/0/0` for synthetic auth users, households, children, teams, sources, and events. No Activity entity, event sport field, schedule assignment, conflict, leave-by, matching, weather, travel, manual-event, TI, or RI behavior was added. Offline validation passed 53 Corralio tests, TypeScript, lint, production build, and diff checks.

- **Corralio Slice 4.0A children and teams foundation complete.** The authenticated home page exposes the already-secured Slice 2 family model: owners can add and rename active children, add private child-owned teams, and edit team name/sport. Children receive the next existing six-token color automatically. V1 cardinality remains one child to many private teams and one child per private team; siblings can use separate same-named rows. Every mutation resolves the active owner household through the existing authenticated idempotent RPC, uses the authenticated Supabase client plus existing RLS/composite FKs, and accepts no browser household ID. Delete/archive/restore/reorder/color editing, shared-team junctions, team-child reassignment, collaboration, and schedule assignment remain deferred. The preflight migration narrowing optional team sport to the existing nine-value taxonomy was manually applied without coercion. The catalog audit confirmed RLS, authenticated-only SELECT/INSERT/UPDATE, no DELETE access, the composite household FK, and the exact sport constraint. The corrected rollback-only verification passed family mutations, same-named sibling teams, invalid-sport rejection, cross-household isolation, and reassignment denial; cleanup confirmed zero synthetic auth users, households, children, and teams. Schedule ingestion, source/event assignment, refresh, imported events, analytics, and weekend rendering are unchanged. Offline validation passed 47 tests, Corralio TypeScript/lint/build, and diff checks; no push or deployment occurred.

## 2026-08-19

- **Corralio Slice 3.3 persistent refresh failure and recovery complete.** The existing connected `error` state is extended by an exact three-failure saturated counter and safe pause timestamp rather than a new status. Only service-role finalization of a live owned claim using one of the eight established bounded failure codes increments atomically; stale/expired/mismatched claims return false. Counts one and two retry after the normal 23-hour window, while count three preserves the URL, events, household, and sport but excludes the source from cron. Canonical successful ingestion resets count/pause/error for scheduled and valid-empty success. Paused sources recover only through the existing validated replacement path, which also updates attempt freshness in the same transaction; failure rolls all replacement state back. The source UI distinguishes `Refresh delayed` from `Schedule needs attention` and exposes only the safe pause marker, never the counter, claim metadata, raw error, or URL. No table, index, queue, second cron, polling, or history writes were added. The migration was manually applied; all 13 consolidated catalog checks passed, the complete rollback-only behavioral verification returned cleanup counts `households=0`, `sources=0`, and `events=0`, and a separate read-only check preserved the existing UAT family at one household, one source, and 151 events. Offline validation passed 40 tests, Corralio TypeScript/lint/build, and diff checks; no live feed, cron invocation, push, or deployment occurred.

- **Corralio Slice 3.2 secure scheduled ICS refresh complete.** One daily Corralio Vercel cron (`17 11 * * *`) calls a no-store Node route authenticated only by the exact `Authorization: Bearer ${CRON_SECRET}` contract and using only the service-role Supabase client. The sequential worker claims at most 10 active ICS sources, uses a 23-hour attempt freshness window, prioritizes never-attempted/oldest sources deterministically, recovers abandoned claims after 10 minutes, reports `batch_full` without a count query, and isolates each source failure. It reuses the shared SSRF-safe fetch/normalization engine and delegates claimed persistence to `corralio_persist_ics_ingestion_v1`; valid empty feeds preserve unrelated events and apply only explicit cancellations, while every failure preserves the existing URL and event set. URL replacement invalidates active claims. The migration was manually applied and its catalog privilege contract passed; it adds only nullable operational columns and three service-role-only RPCs—no table or index—and retains authenticated `source_url`/claim denial. Persistent-failure disabling is deferred to Slice 3.3. The corrected rollback-only behavioral verification passed claim non-overlap, production isolation, abandoned-claim recovery, canonical persistence, bounded failure handling, and URL-replacement invalidation, reached `ROLLBACK`, and returned cleanup counts `households=0`, `sources=0`, and `events=0`. Validation also passed 28 tests, Corralio TypeScript/lint/build, and diff checks; no live feed fetch, cron invocation, push, or deployment occurred.

- **TI hotel measurement foundation implemented locally with approximately zero incremental Supabase load.** The existing first-party `ti_venue_hotel_session_id` UUID is now the canonical identity across Tournament Hotels hub views/property clicks, venue/map hotel actions, tournament-detail hotel intent, Book Travel/Weekend Planner hotel actions, Team Hotels, and all three `/go/hotels*` handoff routes. The prior Team Hotels-only session key is no longer created. A director pilot may use manually constructed tournament links with one bounded `distribution_source` value (`director_email`, `team_manager_email`, `tournament_website`, `qr_code`, `social`, or `other_director`); the value is validated and retained in a separate session-scoped key, and the outbound row records `director:<value>` in the existing unbounded `traffic_source text` column. Director source deliberately takes precedence over an existing traffic-source label rather than concatenating values. Missing tournament context remains valid for generic Book Travel/Planner traffic.
- Existing analytics and outbound writes are enriched in place; no table, migration, index, database lookup, event type, dashboard, aggregation job, or passive-interaction write was added. The analytics API now validates session UUIDs and distribution enums before storing them, including Team Hotels events that previously lost the incoming session field during normalization. `/go/hotels/checkout` now carries the canonical session into its existing outbound row and preserves both measurement values on its safe first-party fallback. Distribution context remains TI-only and is not added to HotelPlanner Custom1–Custom8; Custom3 reconciliation, the immutable Hotel Program snapshot, fee/beneficiary resolution, tournament-end behavior, and Standard fail-safe routing are unchanged.
- The volume audit found ordinary UI links do not establish a separate prefetch persistence path, while direct/scanner invocation can still reach outbound routes. All three HotelPlanner routes now skip persistence for a small shared set of recognizable crawler/link-preview user agents while continuing the safe redirect/fallback behavior. Missing or unrecognized user agents are not labeled human, and no expensive bot service or database classification was added. A normal direct hotel journey still produces the same existing intent analytics rows and one canonical outbound row; expected incremental database rows/day and synchronous reads/journey are both zero, with fewer known crawler rows expected. Focused offline tests passed (22), along with TI typecheck and lint; no production analytics, outbound row, database mutation, live HotelPlanner click, build, push, or deployment was performed.

- **Corralio Slice 3.1.1 password authentication and recovery implemented and local-browser verified.** Returning users can sign in with email/password through the existing Supabase browser-client/session boundary, while Magic Link remains the new-user and passwordless fallback. Authenticated users can set or update the password on the same shared Supabase identity without changing household/RLS data. Forgot/reset routes use a server-only, validated `CORRALIO_SITE_URL`; recovery callbacks accept `type=recovery`, preserve existing email/Magic Link handling, and use bounded relative 303 redirects to avoid host-header and `0.0.0.0` regressions. Invalid credentials and recovery requests remain non-enumerating, raw provider errors are not exposed, and no password is stored or logged by Corralio. Validation passed 21 focused tests, Corralio TypeScript/lint/build, diff checks, and local HTTP rendering. Complete mobile/desktop UAT then passed password setup, password login, Magic Link regression, recovery email/callback, password reset, token single-use, exact `localhost:3002` origin retention, and household/schedule/event preservation with no duplicate onboarding. The pre-restart recovery `503` was traced to an unsaved local `CORRALIO_SITE_URL`; the fail-closed response is intentional for invalid infrastructure configuration and passed after the environment was saved and the server restarted. The remaining source-read console warning referenced a present tracked file and was classified as a non-blocking Next.js development source-map artifact. No production defect remains open; Vercel still requires `CORRALIO_SITE_URL=https://corralio.com` before deployment.

- **Corralio Slice 3.1 complete.** The production-only migration was manually applied and its catalog contract verified: source-level sport and constraint exist, authenticated clients can read sport but not `source_url`, V2 source creation and sport editing are present, atomic replacement is service-role only, and the legacy authenticated direct-replacement grant is removed. The complete rollback-only production verification passed successful persistence delegation, failed-replacement rollback, household isolation, and privilege checks, then confirmed zero remaining synthetic auth users, households, sources, and events. Final browser UAT passed Corralio-branded Magic Link sign-in twice after the same-origin callback repair, connected-source status, URL secrecy, persisted sport/event icon, blocked replacement preservation, navigation choices/copy/external handoff, and responsive layouts at 375/390/430px with no console errors, failed requests, or overflow. Successful private-feed replacement and a location-less event were not exercised in browser data, but atomic replacement/rollback has focused application and SQL coverage and location-less navigation is conditionally omitted. No reproducible Slice 3.1 defect remains. Slice 3.2 remains reserved for secure scheduled ICS refresh.

## 2026-08-18

- Corralio local auth callback origin defect repaired after browser UAT isolated a correct Corralio OTP request/template followed by an incorrect `0.0.0.0:3002` callback redirect. The route previously based success and failure destinations on `request.url`, which can expose the internal listener origin when `next dev` runs with `--hostname 0.0.0.0`. All Corralio auth-result responses now use bounded relative 303 `Location` headers, preserving the browser-facing localhost/LAN/preview/production origin without trusting forwarded host values. Focused coverage verifies relative success/error destinations and cookie retention on the redirect response. No Supabase dashboard, allowlist, database, authorization, push, or deployment setting changed.
- Corralio Slice 3.1 secure schedule connections implemented locally. Optional sport now lives on `corralio_schedule_sources`, stays outside the shared parser, and reaches event cards through source identity. The connection form clears bearer-like URLs after success; safe connected-source cards expose only name, sport, and status and support later sport edits. Replacement validates and normalizes a fresh URL before a service-role-only atomic RPC updates the secret and delegates event persistence to the existing canonical ingestion function, so failures preserve the working URL and events. The legacy authenticated direct URL-replacement grant is revoked by the unapplied migration. Event locations open a user-initiated Apple Maps/Google Maps/Waze/copy chooser using existing authorized address text only; no geocoding, coordinates, routing, traffic, SDK, or provider preference was added. Daily/background ICS refresh remains explicitly deferred to a separately reviewed Slice 3.2; current imports happen only on connection or link replacement. Added a rollback-only post-migration verification script; no production SQL, live feed request, push, or deployment was performed.
- Shared Supabase Confirm Signup and Magic Link callback hardening implemented locally across Corralio, RI, and the TI auth smoke path. Corralio now sends a presentation-only `brand=corralio` callback; RI's previously clean signup and admin-resend callbacks use the non-authoritative `auth_callback=1` sentinel; TI's production signup, resend, claim, and Magic Link callbacks retain their existing safe `next` query parameters. The TI auth-email smoke utility makes only its Magic Link callback query-bearing while leaving recovery and email-change callback behavior unchanged. All three handlers continue to ignore branding/sentinel parameters for authorization and accept `type=email` token-hash verification.
- Replaced the stale token-hash documentation with the shared callback contract: application-supplied Confirm/Magic `RedirectTo` values must already have a query parameter, while both manually managed Supabase templates must fall back to `.ConfirmationURL` when no RedirectTo exists and append `&token_hash=...&type=email` only inside a RedirectTo guard. Dashboard subjects remain neutral and Corralio detection uses exact URL equality rather than a nonstandard template helper. Focused contract tests cover callback construction, credential exclusion, handler non-reliance on sentinels, and the documented fallback. No Supabase dashboard setting, live email, authorization rule, recovery/invitation/email-change/OAuth flow, push, or deployment was changed.
- TI tournament-only HotelPlanner trust implemented locally. Tournament-specific no-venue links now carry a six-hour server-issued HMAC context bound only to the trusted tournament UUID; `/go/hotels` verifies its signature, expiry, and tournament match before allowing the existing Hotel Program resolver to consider Tournament Support. The shared link builder covers the main directory, sport/state/metro hubs, tournament-detail destination fallback, and dated June landing page; the canonical Tournament Hotels page passes the same proof to its no-venue `View all hotels` handoff. Existing confirmed tournament+venue trust is unchanged, while a bare `tournamentId`, invalid/expired/mismatched proof, missing signing configuration, generic Weekend Planner/Book Travel, and arbitrary venue traffic all remain Standard/no-fee. Tokens contain no program, rate, beneficiary, or fee-target data; `Custom3` and immutable outbound snapshots remain authoritative. The resolver now also enforces the documented inclusive Tournament Support end window using the already loaded tournament end date, reverting support routing to Standard after the final day without changing TI Revenue behavior. No database/configuration writes or live outbound clicks were made. Focused tests passed (25/25), as did TI typecheck, lint, and the production build; build warnings are pre-existing.
- TI Tournament Hotel Support director enrollment UX/terms v2 implemented locally on the applied enrollment foundation. The private form now uses concise partnership copy, a restrained mobile headline, trusted tournament benefit naming, an exact UTC expiration cutoff, a protected new-tab hotel-page link, four organization-only recipient choices, three direct confirmations, and the director-facing `Enroll my tournament` activation-review message. Canonical `tournament_hotel_support_v2` terms are deterministic UTF-8 text with trusted $5/$10 economics and fixed SHA-256 hashes; v1 constants remain unchanged.
...
- RI invitation-expiration rendering defect fixed separately: the admin component no longer combines incompatible `Intl.DateTimeFormat` style and timezone-name options. Validation for the combined work passed with 23/23 offline Hotel Support tests, both app typechecks, both linters, diff checks, and the TI production build; build warnings are pre-existing and unrelated.

## 2026-08-17

- TI/RI optional behavioral analytics write hardening implemented locally. Both browser clients now share a bounded 20-event, two-second batcher, flush on page hide with `sendBeacon` when available, and suppress only repeatable `*_viewed`, `*_impression`, `*_loaded`, and legacy `Opened` events when the event plus page/tournament/venue/planner/placement context already occurred in the same browser session. TI’s persisted activation, temporary-event, auth-completion, anonymous-claim, team-hotel conversion, travel-share, and saved-tournament terminal events remain immediate; queued earlier funnel events flush first to preserve ordering. Both `/api/analytics` handlers remain backward-compatible with single-event payloads, accept no more than 20 events, perform one multi-row insert per destination table, fail open for product UX, and emit one structured duration/count log per request instead of logging full event properties. Revenue-critical HotelPlanner/camping/tournament `ti_outbound_clicks` persistence is unchanged, individual, and unsampled.
- Added `scripts/analysis/analytics_write_volume_and_index_audit.sql`, a read-only, timeout-limited post-recovery diagnostic for seven-day/24-hour event volume, hourly peaks, table/index footprint, index usage, table churn/vacuum state, stats age, and separate critical outbound volume. This phase intentionally does not sample general events, delete history, add/drop indexes, or establish retention without production evidence. Use the results to choose exact high-volume events for sampling, confirm retention needs, and replace rather than stack indexes only when reporting-query evidence supports it.
- TI/RI public tournament read-path hardening implemented locally for the two primary visitor entry surfaces. Each app now has a versioned, server-only public tournament-by-slug loader that validates the app-specific public row, caches successful records for one hour, and uses React request memoization so `generateMetadata` and page rendering share the same lookup. Confirmed not-found results throw inside `unstable_cache` and are translated outside it, preventing cross-request cached 404s; invalid rows and Supabase failures remain retryable errors instead of being mislabeled as missing tournaments. Viewer identity, claims, entitlements, saved state, and private contact fields stay outside the public cache, and RI now starts auth plus series lookup together only after the public record resolves.
- TI/RI tournament directories no longer walk large result sets: TI’s previous 200-row loop with a 600-row ceiling and RI’s unbounded 1,000-row loop were replaced by one deterministic page query for 50 visible rows plus one lookahead row, ordered by start date and ID, with page inputs capped at 100. Normal state, sport, month, upcoming-date, league/AYSO, and configured RI metro filters are applied before transfer; accessible Previous/Next links preserve active filters. TI ZIP/radius calls now pass `p_limit=51` and bounded offsets directly into the existing RPC; radius-only state/sport/league refinement remains application-side because the existing RPC has no such parameters, but it never receives more than 51 rows. Existing set-based Owl's Eye, review-score, series, and venue enrichment runs only for the visible page. RI reviewed-only remains series-aware and is evaluated against the bounded visible page rather than triggering a catalog scan.
- Controlled default/single-state/single-sport directory shapes use five-minute app-specific caches; arbitrary text, ZIP/radius, multi-state/multi-sport, configured city-metro, impossible-filter, and deep-page shapes remain uncached to prevent cache-key explosion. TI’s `get_public_directory_tournament_counts_by_state_sport` heatmap aggregate now also uses the shared five-minute directory cache. Static filter options remain available with bounded reads; TI state counts use the cached aggregate, while visible result counts and RI’s contextual counts describe only the current bounded page. Added production-safe non-executing query plans in `scripts/analysis/tournaments_public_read_path_diagnostic.sql` plus a separately labeled read-only, timeout-limited low-traffic/staging ANALYZE script. No index or migration was added because production catalog/plan evidence has not yet been reviewed; a timed-out slug lookup during general saturation is not treated as proof of a missing index.
- Remaining `tournaments_public` follow-up opportunities, in likely traffic/cost order, are TI metro hub catalog batching, tournament map/hub directory reads, Weekend Planner/weekend routes, lodging/affiliate redirect lookups, and lower-volume public APIs/review checks. Post-deployment measurement should compare Supabase CPU, disk I/O, statement timeouts, 504/522 volume, slug/directory calls and total time, cache-refresh frequency, transferred rows, and page latency after 24 hours, 72 hours, and seven complete days; interpret results together with the sitemap and RI admin caching shipped in the same release. Focused tournament/sitemap tests, both app typechecks, both linters, and diff checks passed. A production-backed build was intentionally skipped while recovering from Supabase saturation.
- TI/RI sitemap runtime-load hardening implemented locally without restoring build-time database work. Every sitemap route remains `force-dynamic` with `revalidate = 0`; shared exported `unstable_cache` loaders now cache TI and RI index counts for 24 hours, TI metro rows for 24 hours, and paged tournament/hotel/venue shard rows for 6 hours under separate versioned `ti:sitemap:*:v1` and `ri:sitemap:*:v1` namespaces. The TI metro sitemap and `SeoMetroHubChips` import the same loader, preventing duplicate cold-cache RPCs. Loader failures, invalid shapes, and invalid counts still produce retryable uncached 503 sitemap responses; valid zero/empty results remain valid XML with warning logs, and successful refreshes log duration and row/count totals. Tags are present for future invalidation, while Phase 1 intentionally relies on TTL because RI admin and TI public site are separate deployments. Static/hub sitemap XML uses 24-hour CDN caching, database-backed shard XML uses 6-hour CDN caching, and index/metro XML retains its existing one-hour CDN layer. Cache-key versions must be bumped when loader queries, arguments, filters, or result shapes change. No database migration or automated deployment warmer was added. TI and RI TypeScript checks, focused sitemap safety tests (14/14), and diff checks passed; a production-backed Next build was intentionally not run during the Supabase saturation investigation.
- Supabase privileged-RPC exposure repaired in production and captured as forward migration `20260817_lockdown_privileged_admin_rpcs.sql`. Read-only catalog verification showed `create_event_code(...)`, `process_assignor_crawl_run(uuid)`, and `refresh_city_state_sport_venue_clusters(boolean)` were all `SECURITY DEFINER` functions executable by `anon` and `authenticated`. Production privileges were changed to deny both client roles while retaining `service_role`, then independently verified as `false / false / true` for all three functions. The idempotent migration guards environments where a production-only RPC is absent and raises if any present function remains client-executable or loses service-role execution. No function bodies, application routes, data, or runtime behavior for authorized RI admin callers changed.
- RI admin dashboard database-load hardening implemented locally after a Supabase timeout event exposed the page's always-on query fan-out. Inventory and coverage metrics now use independent eight-hour server caches, operational queue counts use a five-minute cache, and the outbound-click total uses a one-hour cache; admin mutations invalidate the metric tags. Cached groups fail independently and render `—` instead of false zeroes or taking down the entire dashboard, while the UI shows metric generation times and cache expectations. The full tournament/venue coverage scan is no longer repeated on every tab load, and the Owl's Eye count no longer falls back to paging through as many as 200,000 run rows after an aggregate failure. Badge definitions now load only on the Badges tab. Root-layout and admin authorization share a request-scoped authenticated-user lookup, eliminating one duplicate Supabase Auth call without caching identity across requests. Validation passed with RI TypeScript, RI lint, the existing RI test suite (3/3), and `git diff --check`; a full production build was intentionally not run while Supabase was degraded because static generation can query the production-backed database.

## 2026-08-13

- TI dashboard-tiles timeout repair prepared locally after the successful manual resend proved section-level degradation but still showed `dashboard_tiles (statement_timeout)`. Read-only production timing found `get_admin_dashboard_email_tiles()` completes in roughly 0.37s alone, while the cron log showed 10.4s under the email's nested query fan-out, identifying database contention rather than a permanently slow standalone RPC. The cron now loads dashboard tiles before launching the remaining report sections. Migration `20260813_ti_admin_dashboard_tiles_v5_optimized.sql` preserves the v4 JSON contract but replaces roughly twenty sequential aggregates with a materialized tournament source plus one grouped aggregate per table, adds a partial `(completed_at, venue_id)` index for completed Owl's Eye reporting, keeps the service-role-only grant, and does not alter the cron schedule or timeout settings. Focused tests, TI typecheck, focused lint, and production build passed; the migration has not been applied and no additional production email was triggered.
- TI admin dashboard email reliability follow-up implemented locally after the August 13 production cron ran at 6:05 AM Pacific but failed before send/audit with PostgreSQL `57014` (`statement timeout`). Noncritical report loaders now resolve through an isolated, typed boundary with structured section start/completion/failure timing logs; a failed section renders as unavailable, is recorded in `payload.degraded_sections`, and no longer suppresses delivery or misclassifies a successfully delivered partial report. Email provider/preflight failures still record `ok=false`, while audit-write failure is logged without falsely treating the already delivered email as a send failure. First-game reporting now calls a database aggregate instead of downloading raw `ti_map_events`, removing the 1,000-row PostgREST cap while preserving strict `activation_flow="first_game_inline_v1"`, unique-session, and game-persistence rules. Migration `20260813_ti_first_game_activation_reporting.sql` adds the service-role RPC and a narrow partial reporting index. Pacific complete-day windows are now calendar-based across DST transitions. Focused tests, TI typecheck, focused lint, and production build passed. Production still requires applying the migration before deploying; no production email was triggered.

## 2026-08-12

- TI admin email now leads with an isolated Weekend Planner first-game activation report instead of blending the new flow into legacy Planner behavior. The new section and email subject filter strictly to `activation_flow="first_game_inline_v1"`, count unique `planner_session_id` values, use eligible prompt-ready sessions as the denominator, and show yesterday plus trailing seven complete days for prompt viewed, form started, submit, anonymous first-game persistence, failure, save/account prompt, auth start, and auth completion. The headline is persisted / eligible; the historical `<1%` baseline is labeled directional because its definition differs. The email explicitly notes that persistence currently measures anonymous local-storage success and omits second-action reporting until dedicated instrumentation exists. Existing broad Planner metrics remain in a separately labeled legacy/all-flow context section. Added pure reporting aggregation tests covering session deduplication, legacy exclusion, non-game exclusion, and missing session IDs.
- TI Weekend Planner anonymous cross-tournament context defect fixed after Claude UAT: seeded `source_type="tournament"` rows are now ephemeral display context, filtered from legacy snapshots on read, and excluded from every anonymous snapshot write, while real manual events continue across the active/session/tournament aliases. Schedule-conflict calculation now also ignores the current synthetic tournament seed but continues detecting overlaps between real events. Regression coverage verifies legacy cleanup, write filtering, synthetic-seed exclusion, and real-event conflicts. Focused tests passed (11/11), TI typecheck and focused lint passed, and 390px Playwright UAT with a deliberately contaminated active snapshot confirmed no leaked tournament, persisted seed, false conflict, overflow, failed request, or console error.
- TI Weekend Planner first-game activation Phase 1 implemented locally. Valid tournament entrants with no manual event for that exact tournament now see a focused, prebuilt tournament card instead of the dense planner/travel controls; the inline flow asks for a constrained tournament date (fixed for single-day events), required game time, and optional field/court, then reveals a compact persisted-game summary, the normal planner, and secondary hotel/meal/travel actions. Venue context is resolved server-side and query-supplied venue IDs are accepted only when linked to the tournament; multiple venues remain unselected. Anonymous snapshots now return write success/failure and preserve `tournament_id`, trusted venue metadata, timezone, and `field_label`; authenticated create and anonymous claim paths also carry `field_label`.
- TI Weekend Planner activation measurement now carries `activation_flow="first_game_inline_v1"` through typed analytics and server sanitization, removes view toggles from first-action semantics, and enforces the persistence boundary in both the new inline flow and the existing generic event flow: `weekend_planner_first_meaningful_action` and `weekend_planner_activation_achieved` fire only after a successful local-storage or API write. Forced local-storage failure UAT confirmed the form remains open, a failure event fires, and neither success event fires. The finalized implementation prompt is `docs/prompts/ti-weekend-planner-activation-phase1-first-game-inline-v1.md`.
- Validation: focused planner tests passed (8/8), standalone TI typecheck passed, TI production build exited successfully with existing warnings and non-fatal restricted-network Supabase DNS messages, and desktop/mobile Playwright UAT verified hidden pre-activation competition, exact date bounds, post-activation progression, stored field/tournament context, success-event ordering/properties, failure suppression, and a clean final browser console. The broader planner glob still has four pre-existing harness/assertion failures (`calendarFeeds`, `guestShares`, and two ICS sanitizer assertions); none touch this change.
...
- TI Campspot Camping + RV affiliate MVP implemented locally: added secondary Campspot links below the primary hotel actions on eligible TI venue detail and selected tournament-map venue panels; added the TI-only `/go/camping` Awin boundary with canonical `clickref = outbound_attribution_id`, direct/wrapped URL persistence, stable shared lodging sessions, resilient insert handling, and localhost/bot write suppression; added durable `camping_cta_impression` ingestion plus an isolated TI daily-email section for yesterday/7-day impressions, clicks, sessions, CTR, breakdowns, and missing attribution fields. Canonical venue coordinates are required and reliable tournament dates are optional. No RI app/analytics code changed and no new migration was needed. Focused tests (7/7), TI typecheck, clean lint, production build, diff check, and local production-backed Playwright UAT passed. Report: `docs/reports/ti-campspot-camping-rv-affiliate-mvp-2026-08-10.md`.
...
- RI Owl’s Eye hotel-search shutdown (minimal canonical-category change): removed `hotel` from `CURRENT_OWL_CATEGORIES` in `apps/referee/src/owlseye/categories.ts` so default/admin Owl’s Eye runs no longer fetch hotels or require hotel completion in `categories_fetched`. This intentionally leaves the surrounding Owl’s Eye tooling in place and avoids a broader pipeline refactor; new canonical Owl’s Eye runs now target food, coffee, sporting goods, quick eats, and hangouts only. Validation passed with `npx tsc -p apps/referee/tsconfig.json --noEmit`.
- TI venue-detail weather accordion gap fix: followed up on the signed-out browser UAT blank-space report from the venue-detail page. Added a defensive collapsed-details rule in `apps/ti-web/app/tournaments/tournaments.css` so `.venueMobileAccordion` hides all non-summary children unless the native `details` element is open, preventing the closed `10-Day Weather Planner` content from reserving vertical space in the layout. This is a narrow presentation fix only; no hotel flow or entitlement behavior changed. Validation passed with `npx tsc -p apps/ti-web/tsconfig.json --noEmit`.
- TI HotelPlanner canonical hotel-layer UAT follow-up: browser UAT found one remaining TI inconsistency on the tournament venue map after commit `e8b340ed` — venue tiles were still rendering Owl’s Eye-sourced `🏨 N hotels` counts alongside coffee/food/quick-eats stats even though tournament detail and venue detail had already removed hotel counts from Owl’s Eye presentation. Patched `apps/ti-web/app/tournaments/[slug]/map/TournamentVenueMapClient.tsx` so the map venue-tile nearby summary no longer renders hotel counts from `v.counts.hotels` or the temporary live pin count; public HotelPlanner CTA/handoff behavior remains unchanged. Focused validation passed with `npx tsc -p apps/ti-web/tsconfig.json --noEmit` and `npm run lint --workspace ti-web -- --file app/tournaments/[slug]/map/TournamentVenueMapClient.tsx`.
...
- TI Team Travel eligibility UAT follow-up: patched the three proven local defects from the final eligibility browser pass without redesigning the feature. `apps/ti-web/lib/teamHotelBooking.ts` now preserves `tournament_slug` on Team Travel URLs and normalizes same-day `checkin/checkout` pairs to a next-day checkout so one-day tournaments do not prefill a 0-night stay; `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.tsx` and `apps/ti-web/app/tournaments/[slug]/page.tsx` now pass consistent `sport` and `tournament_slug` context from tournament detail into both the Team Travel destination URL and CTA analytics payloads; `apps/ti-web/app/venues/[venueId]/page.tsx` now also includes `tournament_slug` in venue-detail Team Travel URLs. Added focused test coverage in `apps/ti-web/lib/teamHotelBooking.test.ts`. Validation passed with targeted Team Travel tests, TI typecheck, TI lint, and TI production build.
- RI shared venue Phase B follow-up: removed the remaining visible `Owl's Eye™` label from RI tournament-detail venue tiles so the broader RI surface now consistently uses RI-facing copy. Tournament detail venue rows with nearby data now show `Nearby for officials` instead of the TI-branded Owl marker: `apps/referee/app/tournaments/[slug]/page.tsx`.
- TI + RI shared venue platform Phase B implemented as the next narrow shared extraction on top of Phase A. Added shared nearby-place and airport-adjacent venue contracts under `packages/lib/venue/` (`NearbyPlace`, `NearbyPlaceRow`, grouped nearby helpers, and hotel-search readiness reason wiring), then removed the TI-branded `OwlsEyeVenueCard` render from the RI venue detail page in favor of a new RI-only `Nearby for Officials` section that reuses `owls_eye_nearby_food` coffee/food/gear data without carrying TI copy, Owl’s Eye™ branding, airport summary, or demo-score review plumbing onto the RI surface. The RI venue page now keeps the live HotelPlanner block intact above the nearby section, suppresses hotel rows inside the nearby section, and drops the dead `venue_reviews` / `venues` demo-score queries that only existed to feed the old TI-branded component. Supporting updates: `apps/referee/components/venues/RiVenueNearbySection.tsx`, `apps/referee/components/OwlsEyeWeekendGuideAccordion.tsx`, `apps/referee/components/venues/OwlsEyeVenueCard.tsx`, `apps/referee/app/venues/[venueId]/page.tsx`, `packages/lib/venue/{nearby.ts,normalize.ts,types.ts,index.ts,index.test.ts}`, plus `CLAUDE.md` UAT guidance for the new RI venue-detail surface.
- TI + RI shared venue platform Phase A implemented as a narrow read-only extraction under `packages/lib/venue/` with shared venue types, normalization, deterministic linked-tournament shaping, legacy TI address-slug resolution support, and a server-side read service consumed by both venue detail routes. TI remains the functional baseline for shared venue-domain behavior, while TI/RI routes, metadata, structured data, analytics, Owl’s Eye presentation, hotel-provider ownership, and page composition remain product-specific. Migrated read paths: `apps/ti-web/app/venues/[venueId]/page.tsx` and `apps/referee/app/venues/[venueId]/page.tsx`. Validation passed with `node --import tsx --test packages/lib/venue/index.test.ts`, `npx tsc -p apps/ti-web/tsconfig.json --noEmit`, `npx tsc -p apps/referee/tsconfig.json --noEmit`, `npm run build --workspace ti-web`, and `npm run build --workspace referee-app`. TI build completed with existing repo warnings plus transient external DNS fetch failures during static generation but still exited successfully; RI build completed successfully with existing warnings only.

## 2026-08-06

- TI venue cluster internal linking: audited the shared venue model before implementation and confirmed the repo supports a conservative cluster only from existing signals — same current/upcoming tournament relationships plus same-city venues that independently have current/upcoming tournament activity. There is no reliable metro/travel-market abstraction in current code, so the implementation intentionally stops short of radius- or metro-driven clustering. Added shared cluster ranking under `packages/lib/venue/cluster.ts` and exported it through `packages/lib/venue/index.ts`; extended `packages/lib/venue/service.ts` with bounded `listSharedVenuesByIds` and `listSharedVenuesByCityState` readers; added focused coverage in `packages/lib/venue/index.test.ts`; and rendered a compact server-side `Other tournament venues nearby` module on `apps/ti-web/app/venues/[venueId]/page.tsx` with supporting styles in `apps/ti-web/app/venues/[venueId]/VenueDetail.module.css`. The module omits itself unless at least two strong candidates survive self-exclusion, duplicate removal, same-city-without-activity rejection, and deterministic ranking. No new analytics were added because existing venue navigation instrumentation is sufficient for this release. Validation passed with `node --import tsx --test packages/lib/venue/index.test.ts`, `npx tsc -p apps/ti-web/tsconfig.json --noEmit` (after build regenerated `.next/types`), `npm run lint --workspace ti-web`, and `npm run build --workspace ti-web` (build completed with existing unrelated warnings plus the known transient external DNS fetch warnings during static generation).
- TI venue detail UAT follow-up: fixed the two proven regressions from the local venue travel-ladder browser pass. `apps/ti-web/app/venues/[venueId]/page.tsx` now renders a single unified upcoming-tournament block for the one-tournament case instead of duplicating the same tournament in separate mobile/desktop DOM branches, and the venue-level `Find hotels near this venue` CTA now uses the same real handoff readiness basis as the HotelPlanner link itself by accepting valid venue coordinates in addition to ZIP-based eligibility. Supporting callers were aligned in `apps/ti-web/lib/booking/venueBooking.ts`, `apps/ti-web/lib/booking/venueBooking.test.ts`, `apps/ti-web/components/venues/VenueCard.tsx`, `apps/ti-web/components/venues/OwlsEyeVenueCard.tsx`, and `apps/ti-web/app/tournaments/[slug]/page.tsx` so coordinate-backed venues keep their hotel CTA surfaces consistent across tournament, venue, and venue-card flows. Validation passed with `node --import tsx --test apps/ti-web/lib/booking/venueBooking.test.ts`, `npx tsc -p apps/ti-web/tsconfig.json --noEmit`, `npm run lint --workspace ti-web`, and `npm run build --workspace ti-web` (build completed with existing unrelated warnings plus the known transient external DNS fetch warnings during static generation).
- TI venue detail travel-ladder refinement: promoted the existing `/go/hotels` action to a clearer page-level lodging CTA, moved upcoming/current tournaments earlier in the venue page flow, and enriched tournament rows with sport and tournament city/state facts without inventing schedules or venue logistics. `apps/ti-web/app/venues/[venueId]/page.tsx` now renders `Find hotels near this venue` before the Team Travel/share actions when the existing venue hotel CTA is allowed, shows upcoming/current tournaments above the score/review/Owl’s Eye blocks, and uses shared linked-tournament data to display name + sport + location + date in both the single-tournament mobile/desktop branches and the multi-tournament list. `apps/ti-web/components/venues/OwlsEyeVenueCard.tsx` now supports suppressing the duplicated in-card booking CTA when the page-level hotel action is already shown, and `apps/ti-web/components/venues/HotelBookingCta.tsx` now supports left-aligned rendering for that page-level use. Validation passed with `node --import tsx --test apps/ti-web/lib/teamHotelTournamentCallout.test.ts`, `npx tsc -p apps/ti-web/tsconfig.json --noEmit`, `npm run lint --workspace ti-web`, and `npm run build --workspace ti-web` (build completed with existing unrelated repo warnings plus the known transient external DNS fetch warnings during static generation).
- TI tournament-detail Team Travel CTA presentation refined without changing eligibility, routing, attribution, or analytics semantics. `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.tsx` now renders the Team Travel secondary action as a single compact left-aligned link row beneath the three primary CTAs instead of a boxed panel, keeping the existing new-tab behavior and one-click analytics path intact. `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.module.css` removes the heavy card treatment in favor of a light divider/top-border treatment with tighter spacing and clearer secondary typography, and `apps/ti-web/lib/teamHotelTournamentCallout.ts` now uses the quieter `Traveling with the whole team?` headline. Validation passed with `node --import tsx --test apps/ti-web/lib/teamHotelTournamentCallout.test.ts`, `npx tsc -p apps/ti-web/tsconfig.json --noEmit`, `npm run lint --workspace ti-web`, and `npm run build --workspace ti-web` (build completed with existing unrelated repo warnings plus the known transient external DNS fetch warnings during static generation).
- RI venue detail contrast/readability cleanup: tightened the dark-card text hierarchy on the RI venue page without changing layout or behavior. `apps/referee/components/venues/RiVenueAirportSection.tsx` now uses light-on-dark heading/name/location/distance colors for the airport block; `apps/referee/components/venues/RiVenueNearbySection.tsx` now uses readable near-white/light-gray heading and summary text for `Nearby for Officials`; `apps/referee/app/venues/[venueId]/page.tsx` now explicitly renders the live-hotel subhead, affiliate disclosure, linked-tournament metadata, notes, and lower descriptive copy in readable light neutrals instead of low-contrast dark grays or opacity-only text; and `apps/referee/app/tournaments/tournaments.css` slightly increases RI venue-card surface opacity, strengthens nearby-panel contrast, brightens nearby metadata text, and adds visible keyboard focus outlines for secondary links on dark cards. Validation passed with `npx tsc -p apps/referee/tsconfig.json --noEmit`, `npm run lint --workspace referee-app`, and `npm run build --workspace referee-app` (build finished with existing unrelated repo warnings only).
- TI Team Travel tournament-detail CTA visibility + new-tab UX: replaced the buried plain-text Team Travel link under the tournament-detail CTA cluster with a clearly secondary full-callout link that keeps group-lodging intent distinct from individual hotel search. `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.tsx` now renders a single clickable callout using the existing `/team-hotel-booking` destination and existing Team Travel analytics path, but opens it in a new tab (`target="_blank"`, `rel="noopener noreferrer"`), preserves current attribution/eligibility metadata, and adds screen-reader-only new-tab text. Styling lives in `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.module.css`; a small tested config helper lives in `apps/ti-web/lib/teamHotelTournamentCallout.ts` with coverage in `apps/ti-web/lib/teamHotelTournamentCallout.test.ts`. Validation passed with targeted Team Travel tests, `npm run lint --workspace ti-web`, `npm run build --workspace ti-web`, and a fresh standalone `npx tsc -p apps/ti-web/tsconfig.json --noEmit` after build regenerated `.next/types`.
- TI multi-venue tournament hotel search implemented on the above-the-fold tournament-detail hotel CTA without changing Team Travel or property handoff flows. `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.tsx` now renames the CTA to `Find tournament hotels` and uses a deterministic pre-HotelPlanner venue chooser when a tournament has multiple usable linked venues; single-venue tournaments go straight to `/go/hotels` with venue-level lat/lng and tournament dates, while tournaments with no usable linked venue context keep the existing generic `/book-travel` fallback. Server-side linked venue shaping is now passed from `apps/ti-web/app/tournaments/[slug]/page.tsx`, and the reusable selection logic lives in `apps/ti-web/lib/tournamentHotelSelection.ts`.
- TI tournament hotel search analytics were extended additively, not renamed: `tournament_detail_travel_search_clicked` now records direct/selector/fallback mode plus selected venue context where available, and two new selector-only events were added in `apps/ti-web/lib/tiAnalyticsEvents.ts` — `tournament_detail_hotel_selector_viewed` and `tournament_detail_hotel_selector_venue_selected`.
- TI hotel-search helper coverage was expanded so direct `/go/hotels` links can explicitly carry `checkin` / `checkout` when tournament dates are known: `apps/ti-web/lib/booking/venueBooking.ts`, `apps/ti-web/lib/booking/venueBooking.test.ts`, `apps/ti-web/lib/tournamentHotelSelection.test.ts`. Validation passed with focused TI tests, `npm run lint --workspace ti-web`, `npm run build --workspace ti-web`, and a fresh standalone `npx tsc -p apps/ti-web/tsconfig.json --noEmit` after build regenerated `.next/types`.
- TI tournament hotel search follow-up: aligned the new selector with the existing venue-map HotelPlanner contract by treating coordinate-only linked venues as usable hotel-search targets, and made direct HotelPlanner tournament hotel handoffs open in a new tab from tournament detail. `apps/ti-web/lib/tournamentHotelSelection.ts` now honors lat/lng-only venues even when no text search string is available, and `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.tsx` now opens direct and selector-driven `/go/hotels` handoffs in a new tab while keeping the generic `/book-travel` fallback same-tab. Validation passed with focused hotel-selection tests plus standalone TI typecheck.

## 2026-08-04

- TI team travel pre-launch measurement fixes: split the canonical `/team-hotel-booking` landing view away from CTA impressions (`team_hotel_landing_viewed` vs `team_hotel_cta_viewed`), added lightweight anonymous/session tracking for team-travel funnel events, moved `team_hotel_request_submitted` to the server-owned `/api/lodging/group-request` success path so accepted requests are authoritative, added a reversible `Team Travel` header link with separate header attribution, and updated the TI admin email to report landing sessions, header traffic, accepted requests, unique accepted requests, and qualified requests. Validation passed with targeted team-hotel tests, `npm run lint --workspace ti-web`, `npx tsc -p apps/ti-web/tsconfig.json --noEmit`, and `npm run build --workspace ti-web`. Browser UAT is still the remaining pre-push check. See `docs/notes-ti.md` for details.
- TI team hotel booking SEO and attribution slice: added the new indexable `/team-hotel-booking` landing page, kept the existing HotelPlanner group-request backend as the only submission path, wired contextual team-hotel links from tournament detail, venue detail, and Book Travel into that canonical route, added landing-page/team-hotel success-failure analytics plus admin email counts, and added the route to the TI static sitemap. Validation passed with `node --import tsx --test apps/ti-web/lib/teamHotelBooking.test.ts apps/ti-web/lib/hotelPlannerAttribution.test.ts apps/ti-web/lib/lodging/hotelPlannerProvider.test.ts`, `npx tsc -p apps/ti-web/tsconfig.json --noEmit`, `npm run lint --workspace ti-web`, and `npm run build --workspace ti-web`. See `docs/notes-ti.md` for details.
- TI team hotel booking UAT follow-up: fixed the duplicated landing-page title suffix, widened the venue-detail CTA render gate to any linked tournament with an id, and made successful team-hotel submissions show a forced visible success state instead of leaving the filled form on screen with no confirmation. Follow-up validation passed with `node --import tsx --test apps/ti-web/lib/teamHotelBooking.test.ts apps/ti-web/lib/hotelPlannerAttribution.test.ts apps/ti-web/lib/lodging/hotelPlannerProvider.test.ts` and `npm run build --workspace ti-web`; standalone `npx tsc -p apps/ti-web/tsconfig.json --noEmit` is again flaky because of transient `.next/types` includes in the TI tsconfig. See `docs/notes-ti.md` for details.
...
- RI tournament map: added the new noindex RI map route at `/tournaments/map` with a dedicated Mapbox CSS layout import, shared Phase 2-compatible filters (`q`, `state`, `city`, `month`, `sports`, `reviewed`, `includePast`), clustered tournament markers, synchronized list selection, RI-only tournament/venue CTAs, and separate RI PostHog map events (`ri_tournament_map_viewed`, `ri_tournament_map_filter_applied`, `ri_tournament_map_marker_clicked`, `ri_tournament_map_result_clicked`, `ri_tournament_map_view_details_clicked`, `ri_tournament_map_view_venue_clicked`): `apps/referee/app/tournaments/map/{layout.tsx,page.tsx,TournamentMapPageClient.tsx,TournamentMapPage.module.css}`, `apps/referee/lib/tournamentMapAnalytics.ts`.
- RI analytics baseline follow-up: added a TI-style first-party analytics path for the currently shipped RI action events. New ingestion route `apps/referee/app/api/analytics/route.ts` allowlists the existing `ri_*` tournament/venue/map events, suppresses localhost/non-production traffic unless explicitly enabled, and persists into `public.ri_analytics_events` via `supabase/migrations/20260803_ri_analytics_events.sql`. The client helper `apps/referee/lib/riAnalytics.ts` now posts to this route instead of relying on third-party transport, and `/admin/analytics` provides a small operator surface for the last 7 days.
- RI analytics operator email follow-up: added a daily RI admin dashboard email path backed by `ri_analytics_events`. New helper `apps/referee/lib/riAdminDashboardEmail.ts` builds a 7-day digest including the new venue-hotel metrics (`ri_venue_hotel_results_loaded`, `ri_venue_hotel_card_clicked`, `ri_venue_hotel_no_results`) plus tournament/venue engagement breakdowns; `apps/referee/app/api/cron/admin-dashboard-email/route.ts` sends the email to `RI_ADMIN_DASHBOARD_EMAILS` (fallback `RI_ADMIN_EMAIL`); `apps/referee/app/admin/dashboard-email/page.tsx` provides a manual send/preview surface; `apps/referee/components/admin/AdminNav.tsx` now links to that page; and `apps/referee/vercel.json` schedules the daily cron.
- RI admin dashboard email production hardening: after production `/admin/dashboard-email` hit the shared admin error boundary while `/admin/analytics` itself loaded correctly, `apps/referee/app/admin/dashboard-email/page.tsx` was patched to fail gracefully instead of crashing the entire admin route. The page now catches preview-load errors, logs them server-side, preserves recipient visibility, shows an inline admin warning, and redirects manual send failures to a visible `send_failed` notice instead of dropping into the generic admin error screen.
- RI tournament directory UX follow-up: made the `/tournaments` listing cards themselves open the tournament detail page in addition to the existing `View details` button, while preserving separate nested CTA behavior for `Official site`. Added a keyboard-accessible clickable card wrapper with nested-link suppression in `apps/referee/components/analytics/RiTournamentCardLink.tsx`, switched the main directory cards in `apps/referee/app/tournaments/page.tsx`, and added focus/cursor affordances in `apps/referee/app/tournaments/tournaments.css`.
- RI entry points: added `Open map view` links from the RI tournament directory and sport hubs and added a tournament-detail link to browse similar tournaments on the map while keeping all destination routes on RI only: `apps/referee/app/tournaments/page.tsx`, `apps/referee/app/tournaments/hubs/_components/SportHubPage.tsx`, `apps/referee/app/tournaments/[slug]/page.tsx`.
- Validation for the RI Phase 3 map patch passed locally: `node --import tsx --test packages/lib/tournament/filters.test.ts packages/lib/tournament-map/index.test.ts apps/referee/lib/tournamentMapAnalytics.test.ts`, `npx tsc -p apps/referee/tsconfig.json --noEmit`, `npm run lint --workspace referee-app`, and `npm run build --workspace referee-app`. Browser UAT was not run in this implementation pass.
- RI tournament map follow-up after Claude local UAT: fixed a real data-source bug where the new map route only used joined `venues.latitude/longitude`, causing `1000 tournaments · 1000 without map pins` and a blank placeholder map on `/tournaments/map` whenever venue rows lacked coordinates. The route now uses `tournaments_public.latitude/longitude` as the canonical marker source with venue-coordinate preference when available, while still preserving venue metadata and RI venue links: `apps/referee/app/tournaments/map/page.tsx`. Revalidated with clean `npm run lint --workspace referee-app` and successful `npm run build --workspace referee-app`; standalone `npx tsc -p apps/referee/tsconfig.json --noEmit` remains flaky in this repo because `apps/referee/tsconfig.json` includes generated `.next/types/**/*.ts` paths that are not stable outside the full Next build flow.

## 2026-07-31

- TI Weekend Planner analytics audit follow-up: confirmed the remaining Phase 6 measurement gaps were caused by application analytics plumbing, not a missing SQL migration. Added the missing rollout-critical planner events to the analytics API `PLANNER_EVENTS` allowlist in `apps/ti-web/app/api/analytics/route.ts` (`weekend_planner_ready`, `weekend_planner_activation_achieved`, `weekend_planner_save_prompt_viewed`, the anonymous-claim event family, and `weekend_planner_first_authenticated_action_after_claim`) and fixed the remaining `team_hotel_cta_viewed` emitter path in `apps/ti-web/app/book-travel/BookTravelTeamBlockForm.tsx` so it also carries `experiment_name`, `experiment_variant`, and `feature_flag_state`.
- TI Weekend Planner operator reporting follow-up: updated `apps/ti-web/app/api/cron/admin-dashboard-email/route.ts` so the daily admin email now reports the improved planner rollout metrics as well — planner-ready count, meaningful activation count, save-prompt views, anonymous-claim started/succeeded/failed/skipped counts, and first authenticated action after claim — instead of only the older loaded/manual-event proxies.
- TI Weekend Planner treatment-funnel instrumentation follow-up: added the first-action micro-funnel event family (`weekend_planner_first_action_cta_clicked`, `weekend_planner_manual_event_form_opened`, `weekend_planner_manual_event_form_started`, `weekend_planner_manual_event_submitted`, `weekend_planner_temporary_event_persisted`, `weekend_planner_manual_event_failed`, `weekend_planner_first_meaningful_action`) in `apps/ti-web/lib/tiAnalyticsEvents.ts`, `apps/ti-web/app/api/analytics/route.ts`, and `apps/ti-web/app/_components/planner/PlannerClient.tsx`; expanded planner analytics normalization to preserve `event_type`, `form_location`, `temporary_plan_id`, `device_type`, and `failure_reason`; corrected the admin email treatment/direct funnel math to use unique `planner_session_id` denominators; and replaced the prior capped HotelPlanner handoff query with uncapped pagination over `ti_outbound_clicks`. `npx tsc -p apps/ti-web/tsconfig.json --noEmit` passed.
- TI Weekend Planner rollout-assignment follow-up: fresh production signed-out tournament CTA sessions on Friday, July 31, 2026 still returned `experiment_variant=control` with `feature_flag_state=enabled` despite the intended `100` percent treatment config. Local code inspection did not reveal a direct logic failure in the signed-out 100% branch, so a defensive operator fallback was added: `NEXT_PUBLIC_ENABLE_WEEKEND_PLANNER_DIRECT_ENTRY=true` now acts as a hard treatment override even when the newer experiment env vars are also present. Added focused tests proving both signed-out 100% rollout treatment and hard-override treatment behavior; typecheck passed.
- TI Weekend Planner Phase 6 production telemetry follow-up: production browser UAT on Friday, July 31, 2026 showed the treatment UX path working end to end, but SQL verification for `planner_session_id = f30747f2-6b27-4455-9168-ea7c1cb2c073` exposed incomplete rollout analytics persistence. Fixed locally by adding experiment metadata to the `WeekendPlannerClient` analytics calls and by changing `apps/ti-web/lib/analytics.ts` so normal visible-page analytics use `fetch` with `keepalive` and only prefer `sendBeacon` when the document is actually hidden, reducing silent event loss during active planner interaction.
- TI Weekend Planner Phase 6 rollout hardening fixed one true release blocker: active experiment sessions could previously switch from treatment to control if the rollout env changed mid-session. The planner session context now carries `experiment_name`, `experiment_variant`, and `feature_flag_state`, the assignment helper honors the locked variant on refresh/auth return, the rollout SQL now uses the validated planner field names and joins planner sessions to hotel search/outbound records, the production-readiness report was expanded with exact enable/rollback procedures, and `CLAUDE.md` now checks experiment continuity in UAT.
...
- TI Weekend Planner Phase 2 follow-up fixes (same local workstream): eliminated the proven tournament CTA hydration mismatch by generating `planner_session_id` server-side before passing it into `TournamentPlanningCtasClient`, fixed signed-out temporary planner persistence so local anonymous events survive refresh instead of being overwritten by seeded context, and replaced stray Explorer upsell copy with sign-in/create-account messaging on signed-out calendar-connection surfaces; `CLAUDE.md` now also notes that the feature flag must live in `apps/ti-web/.env.local` for local TI UAT.
- TI Weekend Planner Phase 2 follow-up fix: completed the hydration repair by moving the full tournament-detail planner CTA href generation to the server render path, so `apps/ti-web/app/tournaments/[slug]/TournamentPlanningCtasClient.tsx` now receives one stable precomputed destination instead of rebuilding planner-session query params during hydration.
- TI Weekend Planner Phase 2 follow-up verification: the remaining tournament CTA multi-click symptom was isolated to the browser automation tool, not the app — synthetic pointer clicks can no-op while direct `element.click()` succeeds immediately on the same stable CTA node with no overlay, no DOM replacement, and no console errors; `CLAUDE.md` now documents this as a UAT/tooling caveat.
- TI Weekend Planner Phase 3 anonymous-claim flow implemented locally: added `apps/ti-web/app/api/planner/anonymous-claim/route.ts`, `apps/ti-web/lib/planner/anonymousClaim.ts`, and `apps/ti-web/lib/planner/anonymousClaim.test.ts`; extended `apps/ti-web/lib/planner/anonymousPlanner.ts`, `apps/ti-web/app/_components/planner/PlannerClient.tsx`, and `apps/ti-web/lib/tiAnalyticsEvents.ts` so authenticated `/weekend-planner` return can detect resumable anonymous state, import non-duplicate temporary manual items into real planner storage, clear local snapshots only after success, and emit claim telemetry; focused test and TI typecheck passed locally.
- TI Weekend Planner Phase 3 follow-up duplicate fix implemented locally: added additive migration `supabase/migrations/20260730_ti_planner_phase3_claim_idempotency.sql` and updated `apps/ti-web/app/api/planner/anonymous-claim/route.ts` so imported claim rows persist `planner_session_id` + `claim_source`, letting the server treat repeat authenticated visits to the same planner session as durable no-op re-entries instead of creating fresh duplicates.
- TI Weekend Planner Phase 3 local browser rerun passed after the idempotency migration was applied: signed-out temporary event creation, post-auth claim, post-auth persistence, repeat revisit duplicate-prevention, and cleanup all verified clean. Follow-up local fix now holds `weekend_planner_loaded` until any in-flight anonymous claim finishes and swaps the brief misleading post-auth `Upcoming` empty state for `Finishing your saved planner items...`; `npx tsc -p apps/ti-web/tsconfig.json --noEmit` passed after the patch.
- TI Weekend Planner Phase 4 activation-clarity pass implemented locally: direct-entry signed-out tournament traffic now gets clearer seeded-context copy (`Tournament added to this planner`), a stronger temporary-state explanation, a primary `Add first event` path, and a post-claim `Planner saved to your account` continuity card; telemetry now adds `weekend_planner_activation_achieved` so first meaningful planner-item creation can be measured separately from generic `weekend_planner_first_action` interactions, and `CLAUDE.md` UAT instructions now explicitly verify temporary-state clarity, first-event affordance, and post-auth continuity messaging; `npx tsc -p apps/ti-web/tsconfig.json --noEmit` passed locally.
- TI Weekend Planner Phase 4 follow-up local fix after Friday, July 31, 2026 browser UAT: anonymous planner persistence is now device-local instead of being trapped on one exact deep-link URL. `apps/ti-web/lib/planner/anonymousPlanner.ts` now writes snapshots to the current `planner_session_id`, a tournament alias, and a device-local active alias; `apps/ti-web/app/weekend-planner/page.tsx` now mounts the real signed-out planner on bare `/weekend-planner` whenever direct-entry mode is enabled, so a temporary manual event can be resumed from the generic planner page or from a fresh same-tournament CTA click on the same device; `CLAUDE.md` UAT now explicitly checks both resume paths; `npx tsc -p apps/ti-web/tsconfig.json --noEmit` passed locally.
- TI Weekend Planner Phase 4 follow-up local fix after a later Friday, July 31, 2026 browser rerun: anonymous claim completion could succeed server-side but leave the default signed-in `Upcoming` view stuck on `Finishing your saved planner items...` indefinitely in local dev. Root cause was the per-effect `cancelled` guard in `apps/ti-web/app/_components/planner/PlannerClient.tsx`, which could be tripped by React dev StrictMode remount behavior and leave `anonymousClaimPending` permanently true. Replaced it with a component-mounted ref so successful claim completion now clears pending state correctly while still suppressing post-unmount updates; `npx tsc -p apps/ti-web/tsconfig.json --noEmit` passed locally.
- TI Weekend Planner Phase 4 follow-up local rerun later on Friday, July 31, 2026 passed cleanly with an in-range August event: signed-out direct entry, temporary planner usage, auth handoff, claimed-event visibility in `Upcoming`, post-auth continuity messaging, `Season` visibility, and hard-reload stability all verified. Current interpretation is that the earlier stuck `Finishing your saved planner items...` local failure is resolved by the mounted-ref fix unless it recurs.
- TI Weekend Planner Phase 5 save-conversion pass implemented locally: signed-out planner entry no longer leads with primary save/auth CTAs before intent is established; the stronger save/account prompt now appears after the first manual planner item exists, `weekend_planner_start_clicked` is wired from the real `Add first event` path, signed-out auth/save CTA analytics now include `prompt_location` (`post_first_manual_event` vs `calendar_gate`), and the post-claim continuity card now gives clearer next steps (`Add another event`, `Connect calendar`); `CLAUDE.md` UAT instructions now explicitly verify save-prompt timing and post-claim next-step visibility; `npx tsc -p apps/ti-web/tsconfig.json --noEmit` passed locally.
- TI Weekend Planner Phase 5 follow-up local rerun on Friday, July 31, 2026 passed cleanly: save/auth prompting was correctly deferred until after the first manual event, the upgraded `Save this planner to keep it` prompt appeared at the intended high-intent moment, auth `returnTo` preserved the same `planner_session_id`, claim succeeded with immediate `Planner saved to your account` messaging, the claimed event remained visible in `Upcoming` and `Season`, and one authenticated follow-up action also worked before cleanup. Current interpretation: the Phase 5 conversion changes are locally passing.
...
- TI Weekend Planner nav discovery + team-travel bridge:
  - Replaced the shared-header Weekend Planner TODO in `apps/ti-web/app/layout.tsx` with a live `Weekend Planner Beta` nav link.
  - Updated the existing logged-out beta intro copy in `apps/ti-web/app/weekend-planner/page.tsx` to the cleaner schedule-first version.
  - Added a compact planner-beta-only `Team hotel blocks` card in `apps/ti-web/app/weekend-planner/WeekendPlannerClient.tsx` that now expands the existing TI-branded team hotel block form inline on `/weekend-planner`.
  - Reused `apps/ti-web/app/book-travel/BookTravelTeamBlockForm.tsx` across both surfaces so planner submissions still flow through the current HotelPlanner-backed `/api/lodging/group-request` route without creating a duplicate implementation.
...
- TI Weekend Planner ICS-note sanitization dedupe:
  ...
  - Added focused tests in `apps/ti-web/lib/planner/icsNoteSanitizer.test.ts` to lock down shared behavior and reduce future drift risk.
...
- TI tournament card pre-hydration dead-tap fix on directory/state hub pages:
  - Verified the main tournament card body on `/tournaments` and `/{sport}/{state}` already used a real `cardOverlayLink` anchor, so the dead-tap issue was not caused by missing primary links.
  ...
  - This restores fallback navigation through the underlying `cardOverlayLink` for empty footer gap regions before hydration, without changing the intended CTA/link behavior or card layout.
...
- TI mobile state-map tap fix on sport/state hub pages:
  ...
  - Kept the existing `data-*` target lookup and analytics/navigation flow intact while routing both click and touch through the same shared state-activation handler.
  - Added a small mobile scroll guard that records the initial touch point and skips activation when touch displacement exceeds `10px`, preventing accidental navigation while users scroll across the map on iOS Safari.
  - Registered `touchend` with `{ passive: false }` so tap-qualified interactions can `preventDefault()` and avoid double-firing the follow-up synthetic click.
  - Updated `CLAUDE.md` mobile UAT guidance to verify that `Explore by State` map taps work on mobile Safari and that scrolling across the SVG does not trigger accidental navigation.
...
- TI Weekend Planner Stage `3.5-1B` private family iCal subscription feed:
  ...
  - Tightened the tokenized feed route cache header to `Cache-Control: private, no-store` so revoke/regenerate take effect immediately in browser UAT instead of being masked by cached `.ics` responses.
...
- TI tournament directory dead-click cleanup on staged filters:
  ...
  - Preserved the staged filter interaction model, existing GET/query-param behavior, and analytics flow while improving accessibility with `aria-live` status messaging and stronger focus treatment on touched controls.
...
- TI Weekend Planner Stage `3.3C-6` compact Season date-range filter:
  ...
  - Followed up on the control placement so `Dates` stays visible in the planner filter row for all schedule views and automatically switches into `Season` when a custom range is applied, instead of forcing users to discover it only after manually selecting `Season`.
  - Tightened the month-view selected-event summary so the Schedule-X agenda card centers its text and time row, while intentionally keeping raw source location out of that compact surface to avoid duplicating the richer venue/location treatment already available in planner list cards and event details.
  - Cleaned up planner list cards in `apps/ti-web/app/_components/planner/PlannerClient.tsx` / `Planner.module.css` by hiding the event-type/source chips from the main card, moving field labels inline with the time row, and collapsing lower-frequency card actions under an `Actions` disclosure while keeping `Map` visible as the primary quick action.
  - Followed up on venue display so list cards now hide redundant `SOURCE LOCATION` rows whenever a linked TI venue exists, while still preserving the raw source location in the edit surface for verification and manual correction.
- TI venue detail map-click clarity:
  - Added a bottom-center instructional overlay inside the venue static-map preview in `apps/ti-web/components/venues/OwlsEyeVenueCard.tsx` / `apps/ti-web/app/tournaments/tournaments.css`, but only when the existing map preview already has valid tournament context and routes into the TI internal planning-map surface.
  - Kept the overlay inside the existing full-map planning link with `pointer-events: none`, preserved the separate external directions buttons, and intentionally hid the overlay on plain static-map states where no TI planning destination exists.
- TI Weekend Planner Stage `3.3C-5` conservative venue matching + venue/map click paths:
  ...
  - Expanded planner venue matching in `apps/ti-web/lib/planner/venueResolution.ts` to safely recognize venue names embedded in source location text and to strip trailing sub-venue text from full street-address strings (for example `... USA - The Warehouse` or `... The Hub Court 1`), then added `scripts/ops/backfill_planner_event_venues.ts` and applied planner-event venue backfills locally for Dwight Merkel, Warehouse, and Hub rows that uniquely resolved.
  - Followed up on source-location parsing so trailing field markers such as `#6`, `Field 3`, or `Court 1` are extracted into `field_label` during ICS normalization in `apps/ti-web/lib/planner/ics-import.ts` and stripped before conservative venue matching, which lets cards show the field inline near the time row while still matching the underlying TI venue when the base venue/address is unique.
...
- TI Weekend Planner Stage 3.2 mobile next-event action hierarchy:
  - Enhanced the first upcoming loaded event card in `apps/ti-web/app/_components/planner/PlannerClient.tsx` with a compact mobile-only `Next up` treatment instead of adding a duplicated command module.
  - Reused existing future-event ordering from the loaded schedule so the promoted card remains bounded to loaded events and keeps Stage 2.6A timezone-safe planner behavior intact.
  - Prioritized the existing `Map` action on the next-up card when available, while preserving linked venue navigation, source-location clarity, duplicate/merge/manual cleanup behavior, feed refresh behavior, and entitlement logic.
...
- TI planner: Stage 2.4C duplicate suggestions + “Keep separate” (no merge yet).
  ...
  - Drafted Stage 2.4F prompt for optional manual-original cleanup after merge (manual-only, user-confirmed, bounded to just-merged IDs).
...
- TI planner: production-safe UAT framework for Stage 2 validation (production-only DB).
  ...
  - Hosting: `apps/ti-web/public/uat-fixtures/planner/*` published as TI static files under `/uat-fixtures/planner/` for production UAT import URLs.
...
- TI planner Stage 2.2: start Season Planner Reliability work (lens toggle + ranged event queries + manual duplicate action).
...
- RI Missing venues — sport + start date in location column: the location `<td>` now shows a second line (font 11px, gray) with `sport • start_date` so reviewers can triage without expanding the row. The `sport` column was added to the RPC return set. Migration: `supabase/migrations/20260511_missing_venues_add_sport.sql`.
- RI Missing venues — venue-only individual deep scan: `DeepScanButton.tsx` (per-tournament) passes `venue_only=1&limit=1` so a single-tournament deep scan only follows venue-keyword links (Fields, Locations, Venues pages) and skips fee/games/parking extraction. Max 4 pages instead of 12. Committed as part of prior session.
- RI Missing venues — inference panel moved below table: `MissingVenueBulkInferencePanel` relocated from above the tournament table to below the pagination div so it doesn't push the list off-screen.

## 2026-05-08
- RI Discovery workbench — Perplexity think-block JSON extraction fix: `sonar-reasoning-pro` wraps its reasoning in `<think>…</think>` before the JSON output. `extractFirstJsonObject` was anchoring on the first `{` inside that block (e.g. the model writing out draft JSON in its reasoning), producing a malformed string that failed `JSON.parse` → `parsedJson = null` → "No tournaments array found". Fix: strip `<think>…</think>` from `rawContent` before passing to `extractFirstJsonObject`. The `rawContent` passed to `parseDebugSnapshot` is unchanged so `think_preview` still works. Previously seen as `extracted_json_chars: 3462, top_level_type: "null"` in debug. File: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`.
- RI Discovery workbench — Perplexity timing note strengthened for far-out dates: prior timing note said "include prior-year events"; model was finding a 2026 placeholder page (e.g. NSC Holiday Classic "schedule coming soon") and stopping rather than also fetching the 2025 version. New note explicitly says: "even if you find a 2026 page that says 'schedule coming soon', you MUST also find and include the 2025 version — use the 2025 source_url and estimate 2026 dates". File: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`.
- RI Discovery workbench — Perplexity platform list reframed: "Search these platforms first" was causing the model to check only the listed sites and stop, missing state-specific hubs (e.g. nscsports.org, minnesotasrc.com for MN soccer). Changed to "use as examples, not an exclusive list" with explicit broad search instruction: run multiple queries, check state association site, state referee committee site, regional hubs, and individual club pages. File: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`.
- RI Discovery workbench — Perplexity timeout and maxDuration: Perplexity `sonar-reasoning-pro` takes 30–90s per call. `PERPLEXITY_TIMEOUT_MS` was 25s (timed out every call); increased to 90s. Both Perplexity routes had no `maxDuration` export (Vercel default 10s): `search` route gets `maxDuration = 120`, `run-all` route gets `maxDuration = 300`. Files: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`, `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/perplexity/search/route.ts`, `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/perplexity/run-all/route.ts`.
- RI Discovery workbench — JSON schema venue required fields relaxed: `venue_address` and `venue_zip` were in schema `required`, causing the model to drop whole tournaments rather than violate the schema when it couldn't find a street address (e.g. named parks, recreation complexes). Changed: `venue_address` type to `["string","null"]`; `venue_zip` removed from required; only `venue_city` + `venue_state` required. Prompt rule updated to match: "venue_city + venue_state required; venue_zip and venue_address are best-effort". Code-level venue filter already accepted named venues without a street address (was fixed earlier); schema now matches. File: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`.
- RI Discovery workbench — queue route accepts re-queue after partial import: "Queue to uploads" on chunk 1 changed run status to `imported`, which blocked chunk 2/3 Perplexity calls (attach check required `draft`). Two fixes: (1) queue route now accepts `draft|imported|imported_partial` (was `draft` only) for the atomic claim; (2) `runPerplexityChunk` attachable-status check expanded to `["draft","imported","imported_partial"]` (was `draft` only). Chunks can be added after a partial queue without needing to create a new run. Files: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`, `apps/referee/app/api/admin/ti/discovery-v2/queue/route.ts`.
- RI Discovery workbench — `think_preview` + `citations` added to debug snapshot: `parseDebugSnapshot` now extracts the first 800 chars of the `<think>…</think>` block and the first 10 Perplexity citations into the debug payload returned on parse failures. This lets you see exactly what the model was reasoning about and which pages it actually searched when diagnosing empty or failed results. File: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`.
- RI Discovery workbench — create-run idempotency: "Create run" was returning a 400 (unique constraint violation) when a run for the same sport/state/date range already existed, leaving `activeRunId` unset and the "Run all chunks (Perplexity)" button permanently disabled. Fixed the POST `/api/admin/ti/discovery-v2/runs` route to detect the `23505` error code and fall back to a SELECT on `search_key`, returning the existing run with `existing: true`. Client now shows "Loaded existing run …" instead of "Created run …" when an existing run is returned. Files: `apps/referee/app/api/admin/ti/discovery-v2/runs/route.ts`, `apps/referee/app/admin/ti/discovery/DiscoveryV2Client.tsx`.
- RI Discovery workbench — run elapsed timer: added a live elapsed timer to the V2.5 Runner. `formatElapsed` helper formats seconds as "Xs" or "Xm Ys". `runElapsed` state ticks every second via `useEffect` keyed on `anyChunkBusy` + `perplexityRunAllStartedAt`. Start time is set by both `runPerplexityAll` and individual `runPerplexity` (single-chunk) so the timer works in both flows. While running: shows "Status: running… 1m 23s" with tabular-nums; after completing: shows "Last run: X succeeded, Y failed. — 2m 5s". Also corrected stale label "Stops on first failure" → "Skips chunks with no results" after the 400-continue fix. File: `apps/referee/app/admin/ti/discovery/DiscoveryV2Client.tsx`.
- RI Discovery workbench — Perplexity prompt + model improvements: five changes to `tiDiscoveryPerplexity.ts` to recover from poor soccer discovery results. (1) Added `SPORT_PLATFORMS` constant (10 sports → dominant platform URLs) — injected as "Search these platforms first: …" at the top of every user prompt so Perplexity targets the right sites before falling back to generic search (basketball→Exposure Events, softball→Softball Connected/USSSA, soccer→usclubsoccer.org/gotsport.com, lacrosse→victoryeventseries.com/uslaxevents.com, hockey→myhockeytournaments.com, etc.). (2) Fixed `venue_address` prompt rule — was still saying "REQUIRED — full street address", contradicting the code-level filter relaxation from earlier today; now says "best-effort, omit rather than guess" so Perplexity doesn't invent fake street addresses for park-name venues. (3) Replaced verbose inline JSON schema block (~30 lines) with a compact two-line field list — structure enforcement already delegated to `response_format.json_schema`, freeing token budget for search context. (4) Added timing guidance: when `dateStart` is >3 months out, prompt tells Perplexity to include well-known recurring annual events even if only prior-year pages are available, with instructions to estimate dates and use prior-year source_url. (5) Switched model from `sonar-pro` to `sonar-reasoning-pro` — reasoning model does multi-step search (plans which sites to query before executing), more appropriate for structured cross-platform discovery. File: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`.
- RI Discovery workbench — run-all chunk error handling: `run-all` route was breaking on first chunk failure regardless of error type, stopping November/December chunks when October returned no results. Fixed: only `break` on non-400 status codes (auth/run-state/server errors); 400 errors (no tournaments found, truncated response, bad JSON) are per-chunk content failures and now skip to the next chunk. File: `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/perplexity/run-all/route.ts`.
- RI Discovery workbench — venue geocoding for named venues (parks/complexes): two changes to improve soccer (and other sports with multi-field park venues). (1) `tiDiscoveryPerplexity.ts` venue filter relaxed: previously required a digit or street suffix in `venue_address`; now accepts a venue as long as it has a non-placeholder `venue_name` + valid `venue_city` + `venue_state` + 5-digit `venue_zip` — a named park without a street address is geocodable. (2) `geocodeTournamentVenues.ts` updated: when `venue.address` is empty, falls back to `venue.name` in the Mapbox query (e.g. "Tenafly Recreation Complex, Tenafly, NJ 07670") so Mapbox POI search finds the exact facility; if geocoding succeeds and address was empty, writes `geo.formatted_address` back to `venues.address` to populate a real street address for display, maps, and Owl's Eye. Files: `apps/referee/lib/admin/tiDiscoveryPerplexity.ts`, `apps/referee/lib/tournaments/geocodeTournamentVenues.ts`.

## 2026-05-07
- RI Sources — sweep yield detection: `upsertTournamentFromSource` now returns `{ id, isNew }` (was plain `id`). All 9 sweep files updated to destructure. Softball Connected and Exposure Basketball sweeps track `new_count`/`existing_count` in their `counts` struct and surface them in `createTournamentFromUrl` returns. `sweepSourceAction` reads those counts and auto-applies `review_status = "low_yield"`, `is_active = false`, `ignore_until = now + 90 days` when a tracked source returns ≥5 total tournaments but <20% new — preventing redundant re-sweeps of exhausted seasonal sources. `importTournamentRecords` similarly returns `newCount`/`existingCount`. Files: `lib/tournaments/upsertFromSource.ts`, `lib/tournaments/importUtils.ts`, `src/server/sweeps/exposureBasketballEvents.ts`, `src/server/sweeps/softballConnectedTournaments.ts`, `src/server/sweeps/myHockeyTournaments.ts`, `src/server/sweeps/usssaBaseballTournaments.ts`, `src/server/sweeps/usssaFastpitchTournaments.ts`, `src/server/sweeps/nsaSoftballTournaments.ts`, `src/server/sweeps/asaAzSanctionedClubTournaments.ts`, `src/server/admin/pasteUrl.ts`, `app/admin/tournaments/sources/actions.ts`.
- RI Discovery workbench — `SPORT_PLATFORMS` calibrated from DB sources page: cross-referenced `SPORT_PLATFORMS` constant against actual top source domains from the admin sources page (`get_admin_tournament_acquisition_tiles` RPC). Major corrections: basketball→Exposure Events (760 tournaments, was missing entirely), softball→Softball Connected (645, was missing) + Advanced Event Systems (URL was wrong: `aesweb.com` → `advancedeventsystems.com`), baseball→added Top Tier Sports (toptiersports.net, 88), lacrosse→replaced US Lacrosse/SportsEngine/PlayMetrics (no DB signal) with Victory Event Series + US Lax Events + My Lacrosse Tournaments (actual top 3), hockey→added My Hockey Tournaments (23, top source for hockey), soccer→US Club Soccer moved to first position (238 in DB). System prompt updated to name dominant platforms by sport so Perplexity weights them correctly before even reading the user prompt. File: `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/perplexity/search/route.ts`.
- RI Discovery workbench — Perplexity prompt improvements: (1) added `SPORT_PLATFORMS` constant mapping 10 sports to their dominant registration/tournament platforms (soccer→GotSport/SoccerWire/US Club Soccer, lacrosse→US Lacrosse/PlayMetrics, softball/baseball→USSSA/Perfect Game, hockey→USA Hockey district sites, volleyball→AES/USAV, basketball/wrestling/swimming/track covered). Platform list injected into the user prompt as "Search first: …" so Perplexity knows where to look rather than guessing. (2) Replaced verbose ~20-line JSON schema block in prompt with two compact field-list lines — structure enforcement delegated to `response_format.json_schema` (already in the API call). (3) Updated system prompt to include search strategy: platforms, association sites, preference for specific event pages over org home pages. (4) Removed "verified" from rules ("real confirmed" only). File: `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/perplexity/search/route.ts`.
- RI Discovery workbench — Perplexity API call tracking: wrapped the Perplexity `fetch` in `trackExternalCall` so every call appears on the API-usage page. Added `perplexity` to `EXTERNAL_API` and `ti_discovery` to `EXTERNAL_API_SURFACE` in `trackExternalCall.ts`. HTTP errors now throw with `httpStatus` attached so the correct status code is preserved in the catch block. Files: `apps/referee/lib/trackExternalCall.ts`, `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/perplexity/search/route.ts`.
- RI Discovery workbench — multi-state + month/quarter date picker: Perplexity search route now accepts comma-separated state codes (e.g. `RI,CT,NH`) in addition to a single state; validates each part as a 2-letter code; builds natural-language label ("RI, CT, and NH") for the prompt instruction and a schema hint for the output field. Workbench UI replaces the From/To date pickers with Mode (Month/Quarter) + Year + Period selectors — Month mode produces 1 Perplexity chunk, Quarter mode produces 3 monthly chunks (one per month, up to 60 results vs 20 for a full quarter). Chunk labels now show "Jul 2026 — Chunk 1" instead of "Chunk 1 prompt". State field accepts comma-separated input and strips spaces on input. Files: `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/perplexity/search/route.ts`, `apps/referee/app/admin/ti/discovery/DiscoveryV2Client.tsx`.
- RI/TI Perplexity cost rollup — SQL-only approach (no schema change): two RPCs read token counts directly from `discovery_batches.raw_paste` (full Perplexity JSON stored on every call). `perplexity_usage_summary(from_ts, to_ts)` returns one aggregate row (total calls, input/output tokens, total cost USD). `perplexity_usage_detail(from_ts, to_ts)` returns one row per call with per-call cost. Pricing baked in: Sonar Pro $3/1M input, $15/1M output. Quick-check: `select * from perplexity_usage_summary('2026-05-01', now());`. Migration: `supabase/migrations/20260507_perplexity_usage_rpc.sql`.
- RI/TI venue deduplication — three-tier fuzzy matching: replaced exact-match venue lookup (name + address + city + state) with a two-step approach in both `upsertVenueAndLinkTournament` (CSV import) and `ensureTournamentVenueLink` (approve flow). When city + state are present, fetches up to 50 candidate venues by city+state, then runs fuzzy matching: Tier 1 address match (normalized — street suffix expansion: St/Ave/Blvd/Dr/Rd/Ln/Ct/Pkwy/Hwy), Tier 2 normalized name match (abbreviation expansion: hs→high school, elem→elementary, ms→middle school, jr/sr; trailing noise words stripped: stadium/complex/arena/center/field/fields/park and compound forms). Falls back to original exact-match when city or state is missing. Retry-on-23505 block (race guard for concurrent inserts) kept intact in both callers. New pure utility: `apps/referee/lib/tournaments/venueNormalization.ts` (`normalizeAddress`, `normalizeName`, `findVenueMatch`). Implemented before Perplexity ingestion to prevent venue table pollution at scale.

## 2026-05-06 (continued)
- RI CSV import — geocode venues + tournaments on import: `importTournamentRecords` now Mapbox-geocodes each unique venue that has no lat/lng at import time (new venue insert or existing venue with missing coords). Existing venues without coords are updated in-place. After the primary venue is linked, its coords are written to the tournament (`geo_source = "venue_import"`) via a coalesce-only update (`.is("latitude", null)`) so existing tournament coords are never overwritten. Multi-venue CSV rows are already deduplicated before the venue loop, so each venue is geocoded exactly once and the tournament is updated exactly once from venue index 0. Import notice now includes "Geocoded N venues." when any were geocoded. `MAPBOX_ACCESS_TOKEN` is read from env at runtime — no geocoding occurs if the token is missing. Files: `apps/referee/lib/tournaments/importUtils.ts`, `apps/referee/app/admin/page.tsx`.
- Ops/TI discovery — Perplexity API integration design: recommendation to replace manual ChatGPT copy-paste with an OpenAI-compatible Perplexity Sonar API call returning structured JSON (no markdown link corruption). Implementation prompt written covering: new `POST /api/admin/ti/discovery-v2/runs/[id]/perplexity/search` route, JSON schema for tournament+venue structure (nested `venues` array per tournament), flattening to `DiscoveryV2CsvRow[]`, geocoding via existing `geocodeAddressMapbox`, and a "Run with Perplexity" button per date-range chunk in the discovery workbench. Estimated cost: ~$10–20 per 6,000 tournaments using Perplexity Sonar Pro ($5/1,000 requests). Not yet implemented.
- TI discovery — state map with multi-select design: recommendation to add an interactive US state choropleth to the discovery page showing tournament density by sport. Click-to-toggle multi-select feeds selected states into the Perplexity query batch. Reuses existing pure-SVG map (`US_STATE_PATHS`/`US_STATE_LABELS` from ti-web) with the same log-scale green gradient; new `DiscoveryStateMap` client component handles multi-select (blue highlight for selected states) and inline sport dropdown. Estimated effort: ~1–2 hours. Not yet implemented.

## 2026-05-06
- TI discovery V2.5 — workbench layout + prompt fixes: switched to two-column layout (left: runner + paste, right: sticky Master CSV panel always visible). Notice banner moved inline below the Attach button with red/green background based on error vs. success. Prompt now includes two additional rules: (1) venue_address must be a full street address; (2) ALL URLs must be plain text — no markdown [text](url) format (root cause of "No valid rows found" — ChatGPT was wrapping official_website_url as a markdown link with commas inside the brackets, misaligning all subsequent columns). File: `apps/referee/app/admin/ti/discovery/DiscoveryV2Client.tsx`.
- TI discovery V2.5 — auto-geocode venues on attach: after "Validate + attach" the attach route now Mapbox-geocodes every accepted row's venue address in parallel and stores `venue_latitude`/`venue_longitude` back into the enriched raw_paste so master CSV rebuilds retain coords. `venue_latitude`/`venue_longitude` added to `DiscoveryV2CsvRow` type, `DISCOVERY_V25_CANONICAL_HEADER`, parser, and `buildMasterCsv` serializer. `importUtils.ts` now reads `venue_latitude`/`venue_longitude` as aliases so coords flow through to venue upserts on queue. Attach notice shows geocoded count (e.g. "12 geocoded"). Master CSV stats line shows "Geocoded: X" alongside row count. Prompt strengthened with rule requiring full street address for `venue_address`. Files: `apps/referee/lib/admin/tiDiscoveryV2Csv.ts`, `apps/referee/app/api/admin/ti/discovery-v2/runs/[id]/paste/attach/route.ts`, `apps/referee/lib/tournaments/importUtils.ts`, `apps/referee/app/admin/ti/discovery/DiscoveryV2Client.tsx`.
- Ops venue + tournament geo backfill: bulk-geocoded 583 of 669 venues (no lat/lng) using `scripts/ops/backfill_venues_geo_mapbox.mjs --apply --limit=2000` (84 skipped — no usable address, 2 errors). Re-ran `supabase/migrations/20260506_backfill_tournaments_location_from_venues.sql` which copied lat/lng + null-filled city/state/zip from confirmed/primary linked venues into 533 no-coords tournaments. Followed up with `backfill_tournaments_geo_mapbox.mjs --apply` to pick up 127 more via direct tournament city/state/zip geocoding. Net result: 689 → 29 remaining no-coords tournaments. The 29 are data quality issues: junk records (city="Fallback to", state="NA", name="G"/"P"), state/zip mismatches (KC basketball pair: zip 66061 is Olathe KS, state says MO), and tournaments with only state-level data (no city/zip).
...
- RI Owl's Eye API cost audit: mapped all external API calls per venue — baseline 9–13 calls (2 Google Nearby + 4 Google sporting goods text + 3–9 Foursquare), worst-case ~25 with geocode/timezone/fallbacks. Google billing at ~$400/month driven primarily by 4 parallel sporting goods text searches (always fires) and food/hotel nearby searches (both Google-primary). Duplicate venue guard confirmed working at route level (HTTP 409 before any API calls); batch scripts bypass the guard.
- RI Owl's Eye cost reduction plan: promote Foursquare to primary for food and lodging (demote Google to fallback); replace sporting goods 4-query Google text search with a single OpenStreetMap Overpass API radius query (free, keyless — `overpass-api.de`); remove big box fallback (Target/Walmart, low value); replace Google Timezone API with timezonedb.com (free tier). Implementation prompt written; pending go-ahead.
- RI API usage page — free tier gauges: added `overpass` and `timezonedb` to tracked API list and alarm dropdown; added `FREE_TIER_LIMITS` constant with monthly caps (Foursquare 100K, Mapbox 100K, Resend 3K, Brave 2K, Bing 1K, SerpAPI 100; Google/Overpass/timezonedb shown as free/credit-based); added "Free tier limits (MTD)" section with per-API progress bars (green < 70%, amber 70–90%, red ≥ 90%) that always show current-month usage regardless of date filter. MTD fetched in parallel with date-range query via second `api_usage_summary` RPC call. File: `apps/referee/app/admin/api-usage/page.tsx`.
...
## 2026-05-02
- RI venue address verify — opt-in Google lookups: timezone (Google Timezone API) and website URL (Google Places) lookups on the bulk address-verify tool are now gated behind opt-in checkboxes (both default unchecked). Driven by cost: a 100-venue dry run showed ~99 pending timezone calls and ~95 Google Places calls that were firing automatically. Normal runs now only hit Mapbox (geocoding). `fillTimezone` and `fillWebsite` payload booleans added to route; two checkboxes added to `VenueAddressVerifyPanel`. Files: `apps/referee/app/api/admin/venues/address-verify/route.ts`, `apps/referee/components/admin/VenueAddressVerifyPanel.tsx`.
- RI venue address verify — per-category stats: replaced flat counters (`parsedAddressCount`, `geocodedCount`, etc.) with structured `CategoryStat { complete, existing, error, skipped }` for all four categories (Address/Mapbox, Geocoding/Mapbox, Timezone/Google, Website URL/Google Places). `complete` = new data written; `existing` = field already populated; `error` = attempted but failed; `skipped` = not attempted (opt-in unchecked or no usable input). UI now shows four stat cards side-by-side with color-coded rows (green complete, red error, gray skipped). Files: `apps/referee/app/api/admin/venues/address-verify/route.ts`, `apps/referee/components/admin/VenueAddressVerifyPanel.tsx`.
- RI venue address verify — fix address parsing for no-zip addresses: `parseAddressBlob` required a zip code at the end of both regex patterns (`\d{5}`), so addresses like "123 Main St, Denver, CO" (no zip) silently counted as errors — confirmed by running the tool and seeing Address: error=100 for 100 venues that already had correct lat/lng (geocode: existing=100). Root cause: both patterns had `\s+(\d{5}(?:-\d{4})?)$` at the end without `?`. Fix: made zip optional in both patterns using `(?:\s+(\d{5}(?:-\d{4})?))?`, changed validity check from `street && city && state && zip` to `street && city && state`, return `zip: zip || null`. Route also guarded `updates.zip` assignment with `parsed.zip` truthiness check to avoid writing null. Zip for these venues will be backfilled from the Mapbox geocode response on the next geocoding pass. File: `apps/referee/app/api/admin/venues/address-verify/route.ts`.
- RI venue address verify — geocode debug panel: added `_debug.geocodeSamples` to the verify API response — captures first 5 geocode attempts with query string, expectedState, and outcome (complete/error/skipped/existing). Rendered as a yellow panel in `VenueAddressVerifyPanel` below the rows list for quick diagnosis without opening DevTools. File: `apps/referee/app/api/admin/venues/address-verify/route.ts`, `apps/referee/components/admin/VenueAddressVerifyPanel.tsx`.

## 2026-04-28 (continued)
- RI Owl's Eye batch + force-run fixes: two bugs addressed. (1) Batch was sending `categories` (targeted run) for venues that have NO prior Owl's Eye run when `activeMissingCategories` is a strict subset of all categories — the route's targeted run path requires an existing run and returns 409 `no_existing_run`, causing every new venue in the batch to silently fail. Fix: batch now gates targeted runs on `venue.categories_fetched` being non-empty; venues with no prior run always get a full run regardless of the category filter. (2) The force re-run path (user confirms re-scan of already-scanned venue) was missing the `scrollIntoView` call after `setRunReport`, so results appeared at the bottom but the page didn't scroll to them. Fixed with same 50ms timeout pattern as the normal success path. API usage page not updating in dev is intentional — `trackExternalCall` skips recording when `NODE_ENV=development`; set `ENABLE_EXTERNAL_API_CALL_TRACKING=true` in `.env.local` to enable. File: `apps/referee/app/admin/owls-eye/OwlsEyePanel.tsx`.

## 2026-04-30 (continued)
- RI Owl's Eye geocoding: switched all venue geocoding from Google Maps Geocoding API to Mapbox. Created `apps/referee/lib/mapbox/geocodeAddress.ts` — a typed TS utility wrapping the Mapbox forward geocoding endpoint with US bbox validation, state-mismatch rejection, and `trackExternalCall` instrumentation (api=`mapbox`, surface=`venue_geocode`). Swapped all three callers: (1) Owl's Eye run route auto-geocodes missing coords via Mapbox before nearby search; (2) `address-verify` batch endpoint uses Mapbox for address→coords resolution; (3) `refresh-coordinates` admin endpoint uses Mapbox. Timezone lookup (Google Timezone API) is unchanged — it takes coords as input, not an address. `geocode_source` written as `"mapbox"` going forward. Files: `apps/referee/lib/mapbox/geocodeAddress.ts`, `apps/referee/app/api/admin/owls-eye/run/route.ts`, `apps/referee/app/api/admin/venues/address-verify/route.ts`, `apps/referee/app/api/admin/venues/[id]/refresh-coordinates/route.ts`.

## 2026-04-30
- RI CSV import pipeline: extended to accept `latitude`/`longitude`/`geocode_source` per venue and `tournament_director`/`tournament_director_email`/`referee_contact`/`referee_contact_email` per tournament. All four contact fields already existed as DB columns on `tournaments`; all three coord fields already existed on `venues` (no new migration needed). Venue coords are written only on new venue insert — existing venues are not updated (backfill script owns that path). Contact fields use conditional spread in the upsert payload so a re-import with empty contact columns does not blank out existing data. `geocode_confidence` from the CSV is silently ignored (no DB column). Files: `apps/referee/lib/types/tournament.ts`, `apps/referee/lib/tournaments/importUtils.ts`, `apps/referee/lib/tournaments/upsertFromSource.ts`.

## 2026-04-29
- RI Owl's Eye batch category targeting: "Run next X" batch now operates on `filteredReadyRows` (venues missing at least one selected category) instead of the full `readyRows`. This means unchecking all categories except e.g. "hangouts" and clicking the batch button will only process venues that are missing hangouts, and will send a targeted hangouts-only request for each — no wasted API calls on venues that don't need it. Batch button disabled state updated to match. File: `apps/referee/app/admin/owls-eye/OwlsEyePanel.tsx`.
- RI Owl's Eye debug view: added raw API result inspection to the Owl's Eye admin panel covering all 6 categories (food, coffee, hotel, sporting_goods, quick_eats, hangouts). `upsertNearbyForRun` now accumulates a `NearbyDebugQuery[]` array at each fetch point (FSQ per-radius, Google fallback, Google food/coffee/hotel, sporting_goods) and returns it as `rawDebug: { queries }` on all three exit paths. `OwlsEyePanel` renders a collapsible "Raw search results" section with per-query cards showing provider badge, radius in miles, kept/rejected/total counts, and per-place rows (green = kept, red = rejected) with FSQ category names, reason tags, and reject reasons. The "Run response" and "Nearby debug" JSON pre-blocks both strip `rawDebug` to keep the page readable. Fixed a bug where the full `rawDebug` payload was being serialized inline in the "Run response" JSON dump, pushing the new section off-screen. After each run (batch or single-venue) the page now auto-scrolls to the results section via a `runReportRef`. Files: `apps/referee/src/owlseye/nearby/upsertNearbyForRun.ts`, `apps/referee/app/admin/owls-eye/OwlsEyePanel.tsx`.

## 2026-04-29 (continued x4)
- TI tournament map — quick eats + hangouts indicators on venue cards: the tournament map page (`/tournaments/[slug]/map`) already had coffee/food/hotels counts on venue cards but silently discarded quick_eats and hangouts into an `other` bucket. Extended `VenueCounts` type to include `quick_eats` and `hangouts`; updated `loadOwlsEyeCountsByVenueId` in `page.tsx` to track them separately instead of bucketing into `other`; sporting_goods/big_box_fallback still dropped (low signal for families). Added `enhancedCounts` helper in `TournamentVenueMapClient.tsx` that renders `🌮 N quick eats • 🎳 N hangouts` as a second line on venue list cards (only when either count > 0) and also in the Weekend Guide section of the detail panel. Files: `apps/ti-web/app/tournaments/[slug]/map/page.tsx`, `apps/ti-web/app/tournaments/[slug]/map/TournamentVenueMapClient.tsx`.

## 2026-04-29 (continued x3)
- RI API usage page — fix PostgREST 1000-row cap: `/admin/api-usage` was silently truncating results at 1000 rows because Supabase's PostgREST server-side `db-max-rows=1000` overrides any client-side `.limit()` call, regardless of plan. The old code fetched up to 50k raw rows and aggregated in JS — but the cap meant the totals were wrong whenever the window contained more than 1000 call records. Fix: replaced both raw queries (`external_api_calls` + `ti_map_events`) with two new RPC functions (`api_usage_summary`, `ti_map_event_summary`) that do `GROUP BY` aggregation in Postgres. Each returns only one row per bucket — never touches the row cap. Vendor summary now derived in-memory from the small aggregated result set instead of re-scanning raw rows. Migration: `supabase/migrations/20260429_api_usage_summary_rpc.sql`. File: `apps/referee/app/admin/api-usage/page.tsx`.

## 2026-04-29 (continued again)
- RI Owl's Eye FSQ filtering bug + budget exhaustion: two root causes for recurring `no_results` on quick_eats/hangouts targeted runs. (1) **Filtering bug**: `tagAndFilterEnhancedPlaces` built the name-keyword haystack `h` from only place name + address, so brand names like "McDonald's", "Jack in the Box", "El Pollo Loco", "Angelo's Burgers", "Taco Jalisco" all failed qualification even though FSQ correctly categorized them. Fix: moved FSQ category name extraction before `h`, and included category names in the haystack (`norm(\`${name} ${address} ${fsqCategoryNames.join(" ")}\``). This way "Jack in the Box" gains "Fast Food Restaurant Burger Joint Fried Chicken Joint" in `h`, and "fast food" matches the positive RE. Also extended `QUICK_EATS_POSITIVE_RE` and `tagsForQuickEats` to cover burger, fried chicken, hot dog, taco, bagel; extended `HANGOUT_POSITIVE_RE`/`tagsForHangouts` for ice cream and beer garden. With fix: 24/25 quick_eats places qualify (Stater Bros grocery correctly excluded), 22/25 hangouts qualify. (2) **Budget exhaustion**: `ENABLE_EXTERNAL_API_CALL_TRACKING=true` was set in `.env.local`, so FSQ calls (default 500/day) and Google fallback calls (default 100/day) both hit their daily limits during batch backfill runs, silently skipping FSQ and printing "Google fallback skipped — budget exceeded". Fix: raised limits in `.env.local` to `FOURSQUARE_DAILY_CALL_LIMIT=5000`, `GOOGLE_FALLBACK_DAILY_CALL_LIMIT=500`. (3) **Silent retry skip**: after a budget-exceeded targeted run, `categories_fetched` in the run record was updated to include those categories (even though no results were saved), so subsequent targeted retries were silently skipped by the `isRunFresh && runHasCategory` guard. Fix: added `!isTargetedRun` to that guard so targeted runs always proceed regardless of cached state. Files: `apps/referee/src/owlseye/nearby/quickEatsHangouts.ts`, `apps/referee/src/owlseye/nearby/upsertNearbyForRun.ts`, `apps/referee/.env.local`.

## 2026-04-29 (continued)
- RI Owl's Eye targeted run results fix: after a targeted (backfill) run completes, the admin panel was showing empty tabs (no food/coffee/hotels listed) even when the run succeeded and saved rows to DB. Root cause: the targeted run path in the route returned only `nearby_meta` (counts) but never queried `owls_eye_nearby_food` to build the `nearby` object (actual place list). The full-run path does both; the targeted path did not. Fix: added a `owls_eye_nearby_food` query after `upsertNearbyForRun` in the targeted run path, building the same `{ food, coffee, hotels }` shape as the full run and including it in the JSON response. Panel already read `runReport.nearby?.food/.coffee/.hotels` correctly — the data just wasn't being returned. File: `apps/referee/app/api/admin/owls-eye/run/route.ts`.
- RI Owl's Eye diagnostic logging: added `console.warn` (with `eslint-disable-next-line no-console`) to three silent failure paths in `upsertNearbyForRun`: (1) FSQ HTTP error catch block; (2) Google fallback missing key; (3) Google fallback budget exceeded. Previously these all failed silently, leaving `rawDebug.queries` empty. File: `apps/referee/src/owlseye/nearby/upsertNearbyForRun.ts`.
- RI Owl's Eye queue filter: admin page now supports a `?queue=initial|backfill|all` URL param and a matching segmented control in the panel. Venues with no prior Owl's Eye run (`categories_fetched` null or empty) are the "initial" queue; venues with a partial run (some categories fetched, not all current) are the "backfill" queue. Server-side (`page.tsx`): the sorted `readyNotRunAll` list is split into `initialRunAll` and `backfillAll`; the selected queue's first 120 are served, so each page-load gives a full batch of the right type. Client-side (`OwlsEyePanel.tsx`): segmented control links (`All / New — initial run / Backfill — partial`) show live counts and highlight the active selection; clicking navigates with `?queue=...`. Counts also surface in the Debug summary panel. Files: `apps/referee/app/admin/owls-eye/page.tsx`, `apps/referee/app/admin/owls-eye/OwlsEyePanel.tsx`.
- TI static map cron hardening: reduced `BATCH_LIMIT` from 200 → 50 (200 items × ~600ms avg was ~120s, over Vercel Pro's 60s limit; 50 items ≈ 30s). Added Mapbox 429 / rate-limit detection in the per-item catch block: on a rate-limit hit, the current item's status is reset to `missing` (not `error`) so it re-queues cleanly, and the loop breaks immediately to stop wasting calls for the rest of the batch. `result.rate_limited` flag surfaced in the cron response JSON. File: `apps/ti-web/app/api/cron/static-map-generator/route.ts`.

## 2026-04-28
- Owl’s Eye (RI) + TI consumption: added Quick Eats + Hangouts scaffolding on the existing Owl’s Eye pipeline. New migration extends `owls_eye_nearby_food` to allow `quick_eats`/`hangouts` and adds provider/fallback metadata + `reason_tags` (+ optional place lat/lng). RI pipeline (`upsertNearbyForRun`) now supports Foursquare primary (category-id based, radius expansion, weak-result gating) with controlled Google fallback (1 call per category) and budgets enforced via `external_api_calls`; categories are only marked fetched when actually attempted to keep `/admin/owls-eye` “Filter by missing” accurate for targeted backfills. Added `scripts/foursquare_category_discovery.ts` and `apps/referee/src/owlseye/foursquareCategories.ts` for staged category ID discovery/lock. TI venue page now excludes `quick_eats`/`hangouts` from generic Food lists, shows them in Weekend Guide (when present), and exposes a read-only API route `GET /api/venues/[venueId]/owls-eye-places`. Files: `supabase/migrations/20260428_owls_eye_nearby_quick_eats_hangouts.sql`, `apps/referee/src/owlseye/categories.ts`, `apps/referee/src/owlseye/nearby/upsertNearbyForRun.ts`, `apps/referee/src/owlseye/nearby/foursquarePlaces.ts`, `apps/referee/src/owlseye/nearby/quickEatsHangouts.ts`, `scripts/foursquare_category_discovery.ts`, `apps/referee/src/owlseye/foursquareCategories.ts`, `apps/referee/lib/trackExternalCall.ts`, `apps/ti-web/app/api/venues/[venueId]/owls-eye-places/route.ts`, `apps/ti-web/app/venues/[venueId]/page.tsx`, `apps/ti-web/components/venues/OwlsEyeVenueCard.tsx`, `apps/ti-web/components/OwlsEyeWeekendGuideAccordion.tsx`, `apps/ti-web/app/tournaments/[slug]/map/page.tsx`.
- External API call tracking: added lightweight fire-and-forget instrumentation for all outbound API calls. New `external_api_calls` table (api, operation, surface, status, latency_ms, error, called_at) with indexes on (api, called_at) and (called_at). `trackExternalCall(api, operation, surface, fn)` helper in each app's lib wraps any async call — records latency and status, re-throws on error, and defaults to skipping in local dev unless `ENABLE_EXTERNAL_API_CALL_TRACKING=true` is set (to avoid noisy prod DB writes from dev). Wrapped callsites: Google Places nearby search + text fallback (owls_eye_batch), gear search (owls_eye_gear), geocode (venue_geocode), timezone (venue_timezone), place-by-id + search-text (venue_places_lookup) in referee; Resend send_email in both apps; Mapbox static_map in TI cron. New RI admin page `/admin/api-usage` shows summary tiles (total calls, errors, error rate) + per-vendor summary + per-api/operation/surface breakdown table with date filter (today/7d/30d); "API Usage" link added to AdminNav. Files: `supabase/migrations/20260428_external_api_calls.sql`, `apps/referee/lib/trackExternalCall.ts`, `apps/ti-web/lib/trackExternalCall.ts`, `apps/referee/src/lib/google/geocodeAddress.ts`, `apps/referee/src/lib/google/timezoneFromCoordinates.ts`, `apps/referee/src/owlseye/nearby/upsertNearbyForRun.ts`, `apps/referee/src/owlseye/nearby/upsertGearNearbyForRun.ts`, `apps/referee/lib/googlePlaces.ts`, `apps/referee/lib/email.ts`, `apps/ti-web/lib/email.ts`, `apps/ti-web/app/api/cron/static-map-generator/route.ts`, `apps/referee/app/admin/api-usage/page.tsx`, `apps/referee/components/admin/AdminNav.tsx`.

## 2026-04-27
- TI static map cron dashboard: added `cron_job_results` table to persist one row per cron execution (scanned, processed, updated, skipped_no_coords, skipped_up_to_date, failures, ms, error). Static map generator cron now inserts a result row best-effort at the end of each run. New RI admin page at `/admin/ti/static-maps` shows: tournament status breakdown (ready/missing/processing/queued/error counts), lifetime totals table (sum across all logged runs), per-run history (last 100), and error tournament list. Nav link added to AdminNav. Files: `supabase/migrations/20260427_cron_job_results.sql`, `apps/ti-web/app/api/cron/static-map-generator/route.ts`, `apps/referee/app/admin/ti/static-maps/page.tsx`, `apps/referee/components/admin/AdminNav.tsx`.
- TI tournament static map columns: added `static_map_path`, `static_map_source_hash`, `static_map_version`, `static_map_status`, `static_map_updated_at`, `static_map_error`, `static_map_processing_started_at` columns to `public.tournaments` for caching pre-generated static map previews (avoids loading Mapbox GL on tournament pages). Status constrained to `missing/queued/processing/ready/error`. `tournaments_public` view recreated with new static map fields. Files: `supabase/migrations/20260427_ti_tournaments_static_map_columns.sql`.
- RI venue merge: fix crash when merging a venue that has `ti_outbound_clicks` rows. The FK on `ti_outbound_clicks.venue_id` uses `ON DELETE SET NULL`, so deleting the source venue would null those rows and immediately violate the `destination_type <> 'hotels' OR venue_id IS NOT NULL` check constraint. Fix: added `ti_outbound_clicks` to the `tryUpdate` block so rows are reassigned to the target venue before the source is deleted. File: `apps/referee/app/api/admin/venues/merge/route.ts`.
- RI Owl's Eye duplicate-suspect cycle fix: three-part fix to stop the batch from endlessly re-presenting the same venues. (1) `page.tsx` now fetches open `owls_eye_venue_duplicate_suspects` pairs at page load and excludes those venues from `readyNotRunVenues` — only excludes when BOTH members of the pair still exist, so merged-away venues never permanently block the survivor. A yellow notice in `OwlsEyePanel` shows how many venues are excluded and links to `/admin/venues?duplicates=1`. (2) `run/route.ts` duplicate check now queries `venue_duplicate_overrides` (keep_both) and non-open `owls_eye_venue_duplicate_suspects` rows before blocking; previously-reviewed pairs are filtered from `activeDuplicateCandidates` so the run proceeds instead of cycling forever. (3) `venues/merge/route.ts` resolves all open suspect pairs involving the source venue after a merge, so the surviving venue immediately re-enters the Owl's Eye queue. Files: `apps/referee/app/admin/owls-eye/page.tsx`, `apps/referee/app/admin/owls-eye/OwlsEyePanel.tsx`, `apps/referee/app/api/admin/owls-eye/run/route.ts`, `apps/referee/app/api/admin/venues/merge/route.ts`.
...
- RI Owl's Eye targeted category runs + admin checkboxes: `upsertNearbyForRun` now accepts `categoriesToFetch` — when specified, only those fetch blocks run and `categories_fetched` is merged (not overwritten) on the run row. The run API route (`/api/admin/owls-eye/run`) accepts a `categories` body param; if present, it finds the most recent existing run for the venue and runs only the missing categories (skips `runVenueScan`). Admin panel (`OwlsEyePanel`) now has category filter checkboxes (one per `CURRENT_OWL_CATEGORIES` entry), shows per-venue "Missing:" badges (highlighted for selected categories), and adds a "Run missing" button per venue that triggers a targeted run for only the selected missing categories. Batch runs also pass targeted categories per venue. `OWL_CATEGORY_LABELS` added to `categories.ts` for display names: `apps/referee/src/owlseye/categories.ts`, `apps/referee/src/owlseye/nearby/upsertNearbyForRun.ts`, `apps/referee/app/api/admin/owls-eye/run/route.ts`, `apps/referee/app/admin/owls-eye/page.tsx`, `apps/referee/app/admin/owls-eye/OwlsEyePanel.tsx`.
- RI Owl's Eye category versioning (hybrid Option B/C): added `categories_fetched text[]` column to `owls_eye_runs` so the system tracks which search categories each venue has had fetched. Enables targeted backfills when new categories are added (quick eats, hangouts, campgrounds, vacation rentals) without re-fetching existing categories. `upsertNearbyForRun` now stamps `categories_fetched` on every successful run. Admin page now surfaces venues missing any current category (not just venues with no run at all) using `completeVenueIds` vs `runVenueIds`. Canonical category list lives in `apps/referee/src/owlseye/categories.ts` — add new categories there when ready to ship. One-time backfill script stamps all legacy runs as complete: `apps/referee/scripts/backfill_owls_eye_categories.ts` (run after applying migration, before deploying code). Migration: `supabase/migrations/20260427_owls_eye_categories_fetched.sql`. No impact on TI-facing venue cards or any existing Owl's Eye display logic.
...
- TI Booking.com (Awin) hotel redirects: added `/go/hotels` server-side redirect with Booking URL build → Awin `cread.php` wrap → best-effort click logging → 302 with `no-store` headers (local dev can fall back to direct Booking if Awin env vars are missing): `apps/ti-web/app/go/hotels/route.ts`.
- TI outbound click logging (hotels): extended `public.ti_outbound_clicks` so venue-level hotel clicks can be logged (nullable tournament fields, added `destination_type`, `partner`, `source_surface`, `venue_id`, constraints, and indexes): `supabase/migrations/20260420_ti_outbound_clicks_hotels.sql`.
- TI admin reporting (RI): added “Hotels (Booking)” totals + top-venues table (RPC-backed) to `/admin/ti/outbound`: `supabase/migrations/20260420_ti_outbound_clicks_hotels_admin_rpcs.sql`, `apps/referee/app/admin/ti/outbound/page.tsx`.
- TI Booking CTA UX: “Check hotel availability” CTA with urgency microcopy and non-blocking analytics beacon (`venue_hotels_cta_clicked`): `apps/ti-web/components/venues/HotelBookingCta.tsx`, `apps/ti-web/components/venues/OwlsEyeVenueCard.tsx`, `apps/ti-web/app/tournaments/tournaments.css`.
- TI Booking visibility broadened beyond Owl’s Eye: show the Booking CTA for any venue card surface with a valid 5-digit US ZIP (keeps existing tracking/redirect flow via `/go/hotels`): `apps/ti-web/components/venues/VenueCard.tsx`, `apps/ti-web/components/venues/VenueCard.module.css`.
- TI Booking search-string helper: centralized Booking `ss` computation and updated `/go/hotels` to use Booking’s preferred order (`City+State+ZIP` → `City+State` → `ZIP` last resort), while continuing to avoid region labels like “Front Range”: `apps/ti-web/lib/booking/venueBooking.ts`, `apps/ti-web/app/go/hotels/route.ts`, `apps/ti-web/lib/booking/venueBooking.test.ts`.
...
- TI Mobile UX: remove the homepage hero “Browse tournaments” CTA (header already includes Tournament Directory) to avoid iPhone/mobile-simulator horizontal overflow; also set explicit `viewport` width/scale and add coarse-pointer stacking guard: `apps/ti-web/app/page.tsx`, `apps/ti-web/app/layout.tsx`, `apps/ti-web/app/home.css`.
...
- RI Admin: batch venue-linking for draft uploads (`apps/referee/app/admin/page.tsx`)
  ...
  - Configurable limit (default 50, max 200); processes up to 500 drafts to count total missing
  - Added shared `apps/referee/lib/pageScanner.ts` with `fetchPageText` (6s timeout, strips HTML) and `extractLocationFromPageText` (city/state/zip regex, valid-state guard, zip-only fallback)
...
- 2026-08-03: RI Phase 5 SEO and sitemap separation implemented locally.
  - Updated RI title builders in `apps/referee/lib/seo/buildTitle.ts` so tournament, venue, and hub titles read as RI event/venue guides instead of generic or TI-adjacent labels.
  - Tightened RI metadata copy for `apps/referee/app/tournaments/page.tsx`, `apps/referee/app/tournaments/[slug]/page.tsx`, `apps/referee/app/tournaments/hubs/[sport]/page.tsx`, `apps/referee/app/tournaments/hubs/[sport]/[state]/page.tsx`, `apps/referee/app/tournaments/hubs/soccer/{arizona,california,florida,north-carolina}/page.tsx`, `apps/referee/app/tournaments/map/page.tsx`, `apps/referee/app/venues/page.tsx`, and `apps/referee/app/venues/[venueId]/page.tsx` to reflect referee-oriented discovery/travel framing while keeping canonicals on RI routes.
  - Extended RI sitemap coverage by adding venue sitemap pagination alongside tournament sitemap pagination in `apps/referee/app/sitemap.xml/route.ts` and `apps/referee/app/sitemaps/[name]/route.ts`; RI sitemap index now emits static, hub, tournament, and venue sitemap pages using RI URLs only.
  - Added an RI Phase 5 metadata/sitemap verification checklist to `CLAUDE.md`.

- 2026-08-03: RI Phase 6 analytics and outbound attribution implemented locally.
  - Added a shared RI analytics helper in `apps/referee/lib/riAnalytics.ts` so RI action events now carry a consistent base contract: `source_app=refereeinsights`, `page_type`, `page_path`, `device_type`, and `traffic_source`.
  - Normalized RI action instrumentation across the shipped directory, hub, map, and venue flows by updating `apps/referee/components/analytics/RiTournamentDirectoryAnalytics.tsx`, `apps/referee/components/analytics/RiTournamentCardLink.tsx`, `apps/referee/components/analytics/RiVenueAnalytics.tsx`, `apps/referee/components/analytics/RiTrackedLink.tsx`, `apps/referee/app/tournaments/map/TournamentMapPageClient.tsx`, `apps/referee/components/venues/OwlsEyeVenueCard.tsx`, `apps/referee/components/venues/VenueCard.tsx`, `apps/referee/app/tournaments/page.tsx`, `apps/referee/app/tournaments/hubs/_components/SportHubPage.tsx`, and `apps/referee/app/venues/page.tsx`.
  - The minimal RI event contract now covers directory view/filtering, tournament result opens, map opens, map marker/list selection, venue directory/detail views, venue-result opens, hotels-module views, nearby-module views, hotel CTA clicks, and nearby place clicks.
  - RI HotelPlanner handoffs from `apps/referee/app/venues/[venueId]/page.tsx` and `apps/referee/app/tournaments/[slug]/page.tsx` now append RI-specific source markers (`source=referee_*`, `pageType=referee`, RI placement labels, and `custom8=app:refereeinsights`) so shared TI `/go/hotels` attribution can distinguish RI-origin traffic without changing TI defaults.
  - Added an RI Phase 6 browser UAT checklist to `CLAUDE.md`; reporting/admin-email integration was intentionally left unchanged in this phase because no safe existing RI summary pipeline was identified during the audit.
  - Follow-up: `apps/referee/lib/riAnalytics.ts` now lazily initializes PostHog only in production when `NEXT_PUBLIC_POSTHOG_KEY` is present, so the RI Phase 6 action events can actually send in production without restoring the old layout-level analytics mounts. Local/dev remains silent because the helper still exits outside production.

- 2026-08-04: RI Phase 3A venue map cleanup passed local re-UAT.
...
- 2026-08-13: TI Tournament Hotels Phase 1 browser-UAT follow-up.
  - Fixed HotelPlanner result normalization so nested `position.distanceFromSearch` and `contactInfo.address1/city/state/country` reach tournament hotel cards.
  - Fixed nullable numeric normalization so missing distance/rating/review/price values stay null instead of becoming misleading zeroes.
...
- 2026-09-03: Tournament Enrichment Approval workflow added to RI admin.
  - Migration (applied 2026-09-03 via Supabase SQL editor): `supabase/migrations/20260903_tournament_enrichment_proposals.sql` — `tournament_enrichment_proposals` table with status/action_type/confidence check constraints, jsonb current_value/proposed_value, proposed_by (text), reviewed_by/applied_by (UUID FK to auth.users), auto-updated_at trigger.
