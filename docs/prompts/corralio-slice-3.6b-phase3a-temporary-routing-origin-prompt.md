...
* **No trip, hotel, or booking record exists anywhere in Corralio.** Confirmed by direct repository search. HotelPlanner integration exists elsewhere in this monorepo (TI/referee apps only) as an outbound affiliate-style handoff with a separate, admin-only, once-daily batch reconciliation job (`apps/referee/lib/hotelPlannerBookingSync.ts` → `ti_hotel_bookings`, service-role only, ~24h latency, no real-time callback/webhook exists anywhere in the monorepo). This confirms the founder's caution: there is no way today, for Corralio or anything else in this codebase, to automatically know a hotel booking completed in real time. Note this pattern's existence for whoever eventually scopes Phase 3B — it's a reusable reconciliation shape, not a reason to build anything here.
