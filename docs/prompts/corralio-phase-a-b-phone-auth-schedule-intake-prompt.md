...
**Founder-reviewed and accepted, 2026-08-31.** The review cycle is closed — per the founder's own framing, "the architecture is converging; the next leverage is implementation and real-feed evidence," not another investigation. This prompt is ready to send to Codex once the ICS calendar-metadata micro-slice below has been filed/run (a soft prerequisite, not a hard blocker — Section 6.4 degrades gracefully if it hasn't landed yet) and once 3.6B Stage 1 ships, per the confirmed critical-path dispatch order below. Sending it to Codex remains an action outside this session's toolset (no dispatch mechanism exists here, per prior discussion this session) — that step is the founder's or an external process's to take.
...
>
> **Fourth same-day follow-up, 2026-08-31 — email vendor domains confirmed.** Section 3 (item 4) and Section 6.1 amended: Resend is confirmed (no longer "or whichever vendor Task 0 confirms"), with `mail.corralio.com` already live for outbound send and `inbound.corralio.com` as the planned receiving domain, DNS/MX setup in progress. Resend's own documentation (verified this session) requires the inbound MX record to be the domain's lowest-priority MX record or receiving fails entirely, and delivers parsed inbound email as a signed `POST` webhook — the exact signing/authenticity mechanism still needs direct confirmation against the live account before Section 6.7/8's authorization logic is built. The webhook itself can't be pointed at a real endpoint until Section 6.1 exists, so domain/DNS setup proceeds independently of this prompt shipping, but full end-to-end verification is naturally gated on it.
...
4. **Confirm what authenticated-message evidence is actually available for inbound email.** **Vendor confirmed as Resend, 2026-08-31** (no longer open per prior wording) — domain setup is in progress: `mail.corralio.com` is live for outbound send; `inbound.corralio.com` is the planned receiving domain, DNS/MX setup in progress as of this writing. Resend's own documentation (verified this session) states inbound receiving requires an MX record obtained from the Resend dashboard, that this record must be the domain's lowest-priority MX record or inbound mail will not route to Resend at all, and that once verified, Resend parses the inbound message (headers, body, attachments) and delivers it as a `POST` webhook to an application-configured endpoint — **the webhook itself cannot be created/pointed at a real endpoint until Section 6.1's handler exists**, so DNS/domain setup can complete independently of this build prompt, but the last step (pointing Resend's webhook at a live Corralio endpoint) is naturally gated on this task shipping. **What Resend's fetched documentation did not specify, and this spike must still confirm directly against the live account: the exact webhook-payload signature/authenticity mechanism** (needed for Section 8's raw-body signature verification) and the precision of any SPF/DKIM/DMARC-alignment data Resend passes through in that payload — this remains the hard prerequisite for Section 6.7 (email `From` is not authentication); do not build that logic until confirmed against the live webhook payload, not assumed from documentation.
...
**6.1 Inbound email webhook.** Stand up the webhook at Resend's already-provisioned receiving domain, `inbound.corralio.com` (vendor and domain confirmed 2026-08-31 — no longer open; see Section 3, item 4, for current DNS/setup status). The endpoint receives Resend's already-parsed `POST` payload (headers, body, attachment metadata) — parse *that* payload for a calendar/subscription URL only, not raw MIME; no attachment parsing regardless of what Resend's payload includes (Section 2's non-goal). See Section 7.4 and Section 8 for the webhook-security requirements this endpoint must meet regardless of channel, including confirming Resend's specific payload-signing mechanism (Section 3, item 4) before trusting the payload's authenticity.
