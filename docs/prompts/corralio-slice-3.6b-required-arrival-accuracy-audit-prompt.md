...
* **New, unrelated but concrete finding: an unhandled placeholder-location pattern.** Three events with an unscheduled site emit `LOCATION: (unknown) (unknown), (unknown) (unknown)  (unknown)` verbatim — a compound placeholder, not a blank field. Checked directly: the existing `NON_PLACE` exclusion regex in `apps/corralio/lib/provisionalVenues.ts` only matches an exact whole-string value like `unknown`, `tbd`, `home`, etc., and would **not** catch this compound form. If any ArbiterSports feed shaped like this one is ever ingested, this literal string would currently be treated as a real address rather than "no location" — worth a one-line addition to that exclusion pattern whenever Arbiter (or any vendor emitting this placeholder shape) is actually wired into ingestion. Not in scope to fix as part of this Stage 1's Task 1/Task 2 — noted here so it isn't lost, not requested as new work.
* Net effect on Section 1's finding: this doesn't just fail to close the group-identity question, it changes what kind of problem it is. The evidence increasingly points toward "ArsbiterSports has no single consistent export contract to parse against" rather than "one format exists and we haven't found the right field yet." That's a materially different conclusion for whether to keep investing in automatic detection at all versus leaning entirely on Task 1's manual source-level control as the durable answer — flag this explicitly in the Stage 1 report rather than defaulting to "needs more samples" without saying so.
