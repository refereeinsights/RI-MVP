# Corralio Slice 4.4B — Provisional Venue Foundation
...
This is an audit-first implementation prompt. Repository reality wins over assumptions here. If the audit reveals a material conflict involving privacy, public exposure, source trust, identity semantics, shared-data ownership, or irreversible migration risk, stop and report it before implementation.
...
### Mandatory source-eligibility stop gate

Determine whether existing trusted, non-secret repository/database metadata can distinguish at least one qualifying sports schedule source class from arbitrary or user-controlled ICS sources.

If the audit cannot derive at least one such source class:

> **STOP after the audit.**

Do not implement automatic provisional venue creation. Do not infer trust from:

- user-entered schedule or calendar display names;
- arbitrary URL patterns;
- hostnames extracted from credential-bearing URLs unless an existing trusted classification explicitly supports that derivation;
- event text or sports terminology;
- user-selected sport;
- the desire to make automatic creation operational.

Report exactly:

`SOURCE CLASSIFICATION PREREQUISITE NOT MET`

Identify the smallest prerequisite required to establish trustworthy, server-derived source classification. Do not broaden 4.4B into a universal provider-integration framework merely to pass this gate.
...
- determine whether the schedule source is eligible to contribute shared evidence;
- require an identifiable facility or school name;
...
- remain bounded, idempotent, non-blocking, and default-disabled in production;
...
## 3. Source eligibility and safe provenance

A parsed event is not trusted shared evidence. Implement the smallest explicit V1 policy supported by repository evidence.

Do not automatically create shared venues from manual events, arbitrary pasted addresses, generic/private calendars, unclassified user-controlled ICS feeds, credential-bearing feeds merely because they parse, or user notes/location text alone. Those events remain usable for planning and leave-by.
...
1. qualifying trusted source class;
2. no privacy/non-venue exclusion;
3. successful usable existing event geocode;
4. identifiable facility/school name;
5. credible public sports/school/facility signal in that parsed name;
...
### Facility-name heuristic

Evaluate indicators only inside an identified facility-name component, never across arbitrary address or logistical text.

Potential V1 indicators include fields, ballfields, ballparks, parks, sports/athletic complexes or centers, gyms, arenas, rinks, stadiums, courts, fieldhouses, recreation/community/aquatic centers, pools, golf courses, country clubs, fairgrounds, and sportsplexes.

Schools explicitly qualify when accompanied by otherwise valid evidence: elementary or middle schools, junior highs, high schools, academies, preparatory schools, colleges, universities, and campuses. A school need not contain a sports-specific term.
...
Create or reuse a dedicated shared provisional identity containing only required fields: UUID, facility name, normalized facility name/address, latitude/longitude, `active` lifecycle state, timestamps, validator version, and minimal safe provenance.
...
- grant nothing to `public`, `anon`, or `authenticated`;
...
> **Production automatic provisional creation remains disabled until either a minimal trusted service-only withdrawal/suppression mechanism exists or Slice 4.4C is complete.**
...
No retroactive sweep, bulk backfill, production-wide creation, or historical replay is authorized. Any initial enablement or backfill requires a separate explicit decision after source trust, creation precision, correction capability, and coverage are understood.
...
Report considered geocodes, private/non-venue exclusions, source-ineligible events, eligible events, canonical associations, provisional associations, unresolved eligible events, ambiguity blocks where available, and canonical/provisional/combined rates. Keep denominator semantics explicit and expose no sensitive raw data.
...
- qualifying, manual, and untrusted source eligibility;
...
- A non-qualifying source, address-only event, and private/home event create no provisional identity or shared evidence while events remain usable.
...
Update `apps/corralio/notes.md` and `docs/notes.md`. Record audit findings, source-gate result, storage and identity decisions, public isolation, privacy/facility rules, truthful geocode provenance, atomic duplicate strategy, security boundary, abuse bounds, coverage report, production gate, 4.4C/4.5 deferrals, ADR actions, verifier names, builds/checks, usage, and cleanup-zero. Never record private fixture details or credentials.

Before requesting migration application, report:

1. prerequisite and source-classification findings;
...
If the source gate fails, do not implement merely to complete this report.
...
- Trusted source classification is a hard prerequisite; never infer it.
- Prefer isolated provisional storage and typed identities.
- Privacy first; named facility/school signal required; address-only deferred.
- Existing canonical first, active provisional second, atomic creation third.
- No credential-bearing provenance, new external calls, Overture, canonical promotion, bulk backfill, retroactive sweep, public exposure, push, deployment, cron, or production enablement.
