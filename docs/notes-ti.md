...
- **Past-tournament venue maps now recover through explicit hotel dates.** When a selected venue returns the existing `no_dates` outcome, the hotel panel exposes blank check-in/check-out controls instead of hiding `Change dates` behind a missing resolved-date label or describing the state as limited inventory. No replacement dates are inferred and the initial completed-tournament resolution makes no HotelPlanner call; valid explicit dates refresh the existing rates, cards, pins, property/team-block dates, and attributed broad-results link. Client and trusted-server checks now reject past ranges before provider access and preserve the 730-day horizon. Bounded local UAT used exactly two HotelPlanner searches: Earl Johnson Park recovered with Sep 18–20 (115 results/10 pins), and the upcoming Autumn Ambush regression retained Sep 26–28 (71 results/10 pins). The `/venues/{slug}/hotels` SEO pilot remained separate and unchanged; its pre-existing broken stylesheet import from `7d5fb800` was corrected after the production build exposed it. Focused tests passed 19/19, TI TypeScript and zero-warning lint passed, the TI production build passed with only its existing displayed warning backlog, and diff validation passed. Report: `docs/reports/ti-past-tournament-hotel-date-recovery-2026-09-04.md`.

- **Venue-hotel SEO pilot implemented.** Added `/venues/{seo_slug}/hotels` as a new indexable route. Pilot cohort is the deterministic top-75 venues ranked by linked-tournament count in the last 24 months (tie-broken by `seo_slug` asc), with guards against address-pattern slugs and garbage venue names. Key implementation decisions: (1) UUID→slug permanent redirect on the hotels sub-route using the same `resolveSharedVenueByParam` resolver as the parent page; (2) `computeFallbackDates()` is never triggered — the page collects explicit check-in/check-out via a client-side date picker before navigating to `/go/hotels?source=venue_hotel_seo`; (3) non-pilot-eligible venues (below tournament-count threshold) get a `robots: { index: false }` hotels page rather than a hard 404, so future cohort expansion is non-breaking; (4) `venue_hotel_page_viewed` and `venue_hotel_cta_clicked` added to analytics allowlist as TI-internal-only events with no Custom1-8 change; (5) `venue-hotels-1.xml` pilot sitemap shard added to the sitemap index; (6) internal "Browse hotel options near this venue →" link added to the parent venue page for eligible venues. Revenue routing, HotelPlanner attribution, Custom1-8, and `/go/hotels` behavior are unchanged. TypeScript validation and lint are clean.

## 2026-09-04

- **Selected-venue HotelPlanner searches now remain centered on the venue.** Browser verification showed that sending both precise Dwight Merkel coordinates and `city=Spokane, WA 99205` caused HotelPlanner to substitute its Spokane city centroid (`47.658889,-117.425`) and prioritize downtown inventory. Removing `city` preserved the intended venue area near 5701 N Assembly St (`47.7100434,-117.4776833`). `/go/hotels` now omits only HotelPlanner's `city` parameter when a selected venue has coordinates. Coordinate destination, dates, attribution/Custom fields, Hotel Program routing, and the city fallback for searches without coordinates remain intact. Direct venue, tournament venue-card, and selected tournament-map `near this venue` links share the fix; tournament-wide and generic searches are unchanged. The 137-test focused hotel suite, explicit TI TypeScript validation, zero-warning TI lint, all four production builds, and diff validation passed; the full TI/RI builds displayed only their pre-existing warning backlogs. No database/provider write, booking, push, or deployment occurred.

- **Venue travel actions are now grouped around the venue map.** The existing hotel action moved from the venue-page hero into the venue card immediately below the map; directions follow it, and the existing camping/RV action now sits beneath directions. Direct SEO entry retains the undated attributed hotel search, while explicit tournament context retains the dated tournament-map action. No travel destination, attribution, eligibility, provider, or tournament-date behavior changed. Focused tests and local port-3001 no-booking UAT passed.

- **Direct SEO venue entry no longer silently switches into tournament context.** A venue page without an explicit valid `?tournament=` selection now keeps its static location preview neutral instead of linking the preview overlay to the first upcoming or merely linked tournament map. Its attributed `Find hotels near this venue` handoff also omits inferred tournament IDs and dates. This prevents field-address/SEO visitors from inheriting unrelated tournament dates and dated hotel pins. Explicit tournament context still enables the tournament planning map and dated hotel behavior, while named upcoming tournaments retain their own links. Directions remain unchanged. Focused tests and local port-3001 no-booking UAT passed. Report: `docs/reports/ti-direct-venue-map-context-correction-2026-09-04.md`.
...
- **TI venue hotel entry now prefers the populated, date-aware venue map.** Venue detail already resolves an explicit linked tournament or otherwise the earliest upcoming linked tournament. When that context has a slug and usable current/upcoming dates, the primary hotel CTA now opens the existing tournament venue map in the same tab with the venue preselected; the map then performs its existing single-venue HotelPlanner search and displays nearby hotel names, ratings, and rates. The map retains its existing per-property and “View all nearby hotels” HotelPlanner handoffs. A venue without usable tournament context retains the existing attributed `/go/hotels` fallback rather than presenting invented event rates. Multi-venue maps still load only the selected venue's hotel pool, never one provider search per venue. No schema, provider adapter, HotelPlanner attribution/commercial routing, date resolver, or RI behavior changed. A pre-change production diagnostic made one dated search for Fremont Central Park (`09/04/2026`–`09/07/2026`), returned 117 hotels with 10 priced pins, and completed no booking. Focused coverage passed 3/3, TI TypeScript and zero-warning lint passed, the TI production build passed with only the pre-existing warning backlog, and `git diff --check` passed.

## 2026-09-01

- **TI HotelPlanner backfill boundary bug fixed locally (not committed, dry run not yet executed).** The August dry run returned 35 of 40 rows because `buildHotelPlannerBackfillChunks` set each `chunkEnd` to midnight of the intended last day (e.g. `2026-08-21T00:00:00Z`). HP treats `purchasedDateEnd` as an exclusive boundary — the start of that calendar day — so bookings made later on Aug 21 (1) and Aug 28 (4) were excluded. Fix: `chunkEnd` is now set to the first instant of the calendar day after the intended last day (half-open window). The Aug 15–21 chunk sends end=Aug 22, Aug 22–28 sends end=Aug 29, Aug 29–31 sends end=Sep 1. Adjacent chunk starts equal prior chunk ends so no day is lost between chunks. The 31-day/5-call/7-day-per-chunk operator bounds are unchanged. Evidence for HP's exclusive-end semantics is the observed dry-run omissions; no undocumented semantics were assumed. Four new focused tests added: exclusive-boundary coverage of Aug 21/28/31, adjacent-chunk no-gap, and bounds-preserved assertions. All 8 safety tests, 6 sync tests, 7 reconciliation tests, 9 hotel-program tests pass. Referee TypeScript clean. Revenue path, attribution semantics, and all customer-facing HotelPlanner flows unchanged. Corrected August dry run not yet executed — pending separate authorization.

- **TI hotel booking disclosure updated.** Default text in `apps/ti-web/components/AffiliateDisclosure.tsx` replaced with a HotelPlanner-specific disclosure: names HotelPlanner as the booking provider, states TI may receive a marketing fee, and attributes reservation fulfillment and customer service to HotelPlanner. The "at no additional cost to you" representation was intentionally excluded because some HP programs may include configured per-night fees. No styling, layout, props, placement, calling components, or handoff logic changed. Callers that pass custom `children` (Fanatics gear pages) are unaffected. Lint passed with zero warnings.

- **TI HotelPlanner August 2026 backfill applied to production.** Authorization: founder, 2026-09-01, Aug 1–30 window only (--end 2026-08-30), after a boundary-fix dry run with --end 2026-08-31 returned 46 rows (6 Sep 1 bleed-through due to the half-open boundary sending end=Sep 1 for the final chunk) and the narrowed --end 2026-08-30 dry run returned exactly 40 rows (matching the baseline cohort). Production apply: 5 provider calls, 5 errors=0, parsed=40, persisted=40 to ti_hotel_bookings (upsert by itinerary_number). Post-apply verification: 40 rows total / confirmed=29 / cancelled=11 — exactly matching acceptance criteria. Financial: booking value all-status $19,192.14; confirmed-only $13,923.71; expected commission (all-status) $1,264.89. Sub-period: Aug 1–23 = 8 rows (4c/4x), confirmed-only $1,270.70; Aug 24–30 = 32 rows (25c/7x), confirmed-only $12,653.01. Attribution (29 confirmed): 7 MATCHED (have attr: token linking to ti_outbound_clicks via outbound_attribution_id — hex prefix stripped), 0 ORPHANED_VALID_TOKEN, 22 MISSING_TOKEN (no custom3; went through a non-TI-click-tracked path). RI attribution: 2 rows (custom8=app:refereeinsights, H20007372 + H19459840). Source normalization: all 40 rows source=tournamentinsights (lowercase). Aug 31 bookings: 0 (confirmed by dry run). Sep 1+ will be captured by the daily rolling cron. No customer-facing change, no push, no deployment. Backfill boundary fix and tests were committed locally in cbaaf61f (alongside Corralio SMS safety work) prior to this apply; not yet pushed.

## 2026-08-31

- **TI HotelPlanner revenue-safe synchronization and reporting hardened and post-implementation validated locally.** One explicitly authorized seven-day, read-only `getReport` cancellation diagnostic made exactly one provider request and returned 8 aggregate rows, all exact `Cancelled`, all normalized TournamentInsights Source, and all with cancellation date, purchase date, and itinerary key; it made no provider/database write and retained or printed no payload, PII, booking identifier, or signed URL. The existing sync now performs an independently isolated seven-day cancellation refresh, updating only lifecycle fields so cancellation results cannot erase stored Source/economics. TI commercial reporting uses exact trimmed/case-insensitive `Source=TournamentInsights`, conservative exact Confirmed/Cancelled mappings, confirmed-only booking value and expected commission, separately labeled provider-paid commission across all TI-source statuses, four-way confirmed-cohort attribution, and unavailable attribution on lookup failure while preserving commercial totals. Room nights remain `UNPROVEN`; HotelPlanner arrival remains `UNOBSERVABLE`. A dry-run-first operator backfill is bounded to five sequential chunks covering August 1–31, with no retries. Report downloads enforce audited HTTPS hosts, redirect rejection, 20/30-second timeouts, 20 MB compressed and 50 MB expanded limits, 64 ZIP entries, safe expected worksheet paths, and 10,000 parsed rows. Audit found explicit Custom3 on search/property exits but checkout still relies on opaque provider bundle/session continuity; that gap was documented and not changed under the revenue freeze. Offline post-implementation verification passed 134 HotelPlanner/revenue-path regression tests, Referee TypeScript/lint/build, and diff checks. On 2026-09-01, the separately authorized August dry run completed five sequential provider calls, parsed 35 aggregate rows, persisted zero, and returned zero errors; an initial CommonJS wrapper failure made zero calls and was corrected with an async `main()`. The 35 rows differ from the captured 40-row baseline, so persistence remains held pending aggregate variance review. The existing daily rolling-seven-day September scorecard is ready; a separate weekly artifact was not added. No customer-facing handoff, schema, cron, production write, deployment, push, or unrelated worktree change occurred. Detailed report: `docs/reports/ti-hotel-monetization-reporting-2026-08-31.md`.

## 2026-08-28

- **RI Travel shared HotelPlanner support completed locally.** TI's generic lodging endpoint now accepts the exact additional source `referee_travel` through a tested three-value allowlist while continuing to deny missing, arbitrary, and look-alike sources. RI searches still use TI's existing provider, lifecycle, rate limiting, date-horizon, Mapbox generic-geocoding, and HotelPlanner property-handoff boundaries. Property handoffs carry `source/request_source/flow_type=referee_travel`, canonical source page `referee`, and `Custom8=app:refereeinsights`; no direct provider link or new attribution identifier was introduced. Deterministic booking reconciliation was restored in the shared RI-housed TI report: only a valid `Custom3` token that joins to persisted `ti_outbound_clicks` is matched, lookup failure suppresses conversion without changing booking totals. Focused tests, both workspace TypeScript/lint/build checks, bounded three-request browser UAT, and diff checks passed. No migration, production configuration change, booking, push, or deployment occurred. Detailed report: `docs/reports/refereeinsights-travel-mvp-2026-08-28.md`.

## 2026-08-27

- **Hotel Booking Attribution Reconciliation completed locally.** The branch audit found all six named gaps still present and repaired them narrowly: property clicks now carry their existing outbound/search IDs, tournament-map hotel events share canonical source/placement context and the pin/handoff events persist, Weekend hotel clicks have a canonical placement, and generic Book Travel handoffs retain the lodging-search ID. Group-request source values are validated against the unchanged canonical set. Confirmed bookings now use a bounded `Custom3` → persisted-outbound lookup and report `MATCHED`, `ORPHANED_VALID_TOKEN`, `MISSING_TOKEN`, and `INVALID_TOKEN`; lookup failures preserve booking totals and suppress matched conversion instead of inventing orphan counts. Verification passed 19 focused tests, both workspace TypeScript/lint/build checks, and `git diff --check`. No migration, provider/database call, new ID or source type, Clarity/commercial/privacy redesign, push, or deployment occurred.
- **TI HotelPlanner Stage 2 UAT completed and a local date-horizon guardrail was added.** Exactly three authorized search calls were made: contextual Starfire 2026 and generic Spokane 2026 returned recognized HTTP-200 hotel/availability shapes with 183 and 57 normalized results; a controlled Starfire 2028 search approximately 739 days ahead returned valid HTTP-200 JSON without any recognized success or result container, which the unchanged adapter correctly classified as `UPSTREAM_REQUEST_FAILURE` / `upstream_rejected_response`. No payload, hotel list, credential, secret-bearing URL, customer data, or booking was retained or printed. Because the repository contains no authoritative HotelPlanner booking-search maximum, TI now enforces an explicitly non-contractual 730-day UTC safety boundary for both check-in and checkout. Later dates return the distinct bounded `unsupported_date_horizon` classification before generic geocoding, provider construction/invocation, or outbound persistence; tournament context stays visible, Book Travel preserves its destination, and neither surface offers a fallback that repeats the rejected request. The calm copy is “Hotels aren’t available this far in advance yet. Check back closer to your trip.” Provider response handling, attribution, commercial routing, fees, beneficiaries, zero-inventory behavior, RI, and Corralio are unchanged. Focused hotel coverage passed 94/94, plus TI TypeScript, zero-warning lint, the TI production build, and `git diff --check`; build-time warnings are pre-existing and outside this task. One reproduced cause of `upstream_rejected_response` is unsupported far-future date context; historical failures may include it, but existing diagnostics cannot prove all prior cases had the same trigger. No additional provider call, push, or deployment occurred.
- **TI hotel reliability and conversion Stage 1 implemented locally.** The canonical task prompt is `docs/prompts/ti-hotel-reliability-conversion.md`, including the authorized bounded production read, provider-free browser verification, notes, and local-only commit boundary. One aggregate-only read inspected the 500 most recent `/api/lodging/search` sessions from 2026-07-28 through 2026-08-27 using safe lifecycle/category fields only: 492 succeeded with results, zero were successful zero-result sessions, eight failed, four successful sessions requested low-inventory fallback, and seven failures carried error code `200`. The safe evidence supports HTTP-success semantic rejection but cannot identify the provider payload cause or source-page ownership without separately approved UAT. No prohibited/private field, row-level record, provider payload, write, mutating RPC, configuration change, HotelPlanner call, or booking attempt was involved.
- The proven UI-state defect is fixed: contextual tournament hotel failure/empty states now lead with a positive `Find Hotels Near the Venue` recovery through the existing attributed `/go/hotels` handoff instead of unavailable copy. Generic/insufficient context remains `/book-travel`; 5+ rooms remain a distinct `Request Team Hotel Options` flow. Search failure telemetry now stores bounded classifications instead of provider-derived messages/response snapshots. The homepage adds one secondary sports-travel module, and `/book-travel` clarifies hotels/rentals plus the 1–4 versus 5+ room split. Attribution vocabulary, Custom fields, commercial routing, Hotel Program logic, free access, navigation label, SEO architecture, providers, caching, schemas, RI, and Corralio are unchanged.
- Validation passed 99/99 focused hotel tests (including a deterministic clock repair for one pre-existing Team Hotel reporting fixture), TI TypeScript, zero-warning TI lint, the TI production build, and desktop/mobile browser verification at 1440×1000 and 390×844. Browser UAT installed a deterministic lodging-search interception before contextual navigation, captured the local `/go/hotels` URL without following it, confirmed tournament/venue/dates/source/placement/keyword/job-code/Custom attribution, found no overflow or application errors, and recorded zero HotelPlanner/provider requests. Detailed evidence: `docs/reports/ti-hotel-reliability-conversion-2026-08-27.md`. The Stage 1 verdict at that checkpoint required approved provider UAT; the completed Stage 2 result and follow-up guardrail are recorded immediately above. No push or deployment occurred; all four production workspace builds remain required before any eventual push.
...
- TI public tournament hot-path optimization now shares one validated, versioned one-hour public slug loader between tournament metadata and detail rendering. Successful records alone enter the cross-request cache; confirmed not-found exceptions are translated outside it and therefore do not become cached 404s, while invalid rows and Supabase failures surface as retryable errors. Viewer-specific claims, entitlements, saved state, and private fields remain separate. The directory now requests 50 visible tournaments plus one lookahead row instead of looping through as many as 600, applies normal state/sport/month/upcoming/AYSO/league filters in PostgreSQL, orders deterministically, and exposes filter-preserving Previous/Next controls. ZIP/radius RPC requests are also capped at 51 rows in-database. Controlled default or single-state/single-sport shapes and the heatmap state-count aggregate use five-minute caches; free text, radius, combinatorial filters, and deep pages remain uncached. Production-safe non-executing plans plus a separately guarded low-traffic/staging ANALYZE script were added under `scripts/analysis/`; no index or migration was added without production catalog and plan evidence. Remaining TI opportunities are metro hub catalog batching, map/hub reads, Weekend Planner/weekend routes, lodging redirects, and lower-volume public APIs. Measure combined sitemap/admin/directory effects after 24 hours, 72 hours, and seven days using Supabase CPU/I/O, timeout volume, query calls/time, cache refreshes, transferred rows, and page latency. Focused tests, TI typecheck, lint, and diff checks passed; the production-backed build was skipped during Supabase recovery.
...
- TI Weekend Planner first-game activation Phase 1:
  ...
  - Server-side venue resolution verifies any URL venue against `tournament_venues`, auto-resolves only one trusted non-inferred venue, and never silently chooses among multiple venues. Anonymous snapshots and authenticated/claim APIs now preserve the minimum `field_label` plumbing plus exact tournament and trusted venue context.
  - Analytics carry `activation_flow="first_game_inline_v1"` through the typed client contract and server sanitizer. View toggles no longer count as first actions. Both the inline flow and existing generic manual-event flow now emit `weekend_planner_first_meaningful_action` and `weekend_planner_activation_achieved` only after the local-storage or API write succeeds—not on submit intent.
  - Final prompt: `docs/prompts/ti-weekend-planner-activation-phase1-first-game-inline-v1.md`.
  - Validation passed: focused tests (8/8), TI typecheck, TI production build, and desktop/mobile Playwright success/failure UAT. Success UAT proved submit → persisted → activation ordering and stored `tournament_id`/`field_label`; forced storage failure proved no persisted/meaningful/activation events and retained the retryable form. Final browser check had no failed requests or console errors. The broader planner test glob retains four unrelated pre-existing failures in calendar-feed/guest-share setup and ICS sanitizer assertions.
...
- TI Weekend Planner Phase 6 analytics audit follow-up:
  ...
  - Follow-up operator visibility fix: `apps/ti-web/app/api/cron/admin-dashboard-email/route.ts` now includes the new planner rollout metrics in the daily admin email summary:
    - `weekend_planner_ready`
    - `weekend_planner_activation_achieved`
    - `weekend_planner_save_prompt_viewed`
    - `weekend_planner_anonymous_claim_started`
...
- TI Weekend Planner Stage `3.5-1C` entitlement/privacy/calendar-nav closeout:
  ...
  - Confirmed clean:
    - explorer / insider / weekend_pro entitlement behavior
    - ICS edit-modal privacy sanitization
    - month/week Schedule-X navigation stability
    - planner API mutation enforcement on `children`, `teams`, `events`, and `sources`
  - Remaining note:
...
- 2026-07-13: Venue map "View all nearby hotels" switched back to /Search/ URL with lat/lng for immediate results.
  ...
  - Fix: venue map path (when venueId present) now uses `buildHotelPlannerSearchUrl` with `latitude`/`longitude` so HP's `/Search/` page auto-loads hotel results immediately. Book-travel path (no venueId) stays on `buildHotelPlannerRootUrl` with `city=` + `CheckIn`/`CheckOut` since no lat/lng is available there.
  - Trade-off: venue map "View all" shows populated results immediately by location; HP may not honor the date params it receives. Book-travel still requires one search click but pre-fills correct tournament dates.
  - Map pins and individual hotel card clicks (→ `HotelRoomTypes.htm`) are entirely unaffected.
