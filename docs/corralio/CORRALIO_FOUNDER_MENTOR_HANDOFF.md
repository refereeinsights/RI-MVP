...
- **Know required arrival** — done for the accepted Phase 1 hierarchy (`34d83cf4`). One shared resolver now applies `ics_explicit → source_preference → team_preference → corralio_default` to assigned and household/unassigned sources, and What Fits plus This Weekend/Leave-by consume the same result. Arbiter group-specific automatic inference remains separately inconclusive/non-blocking (II.7–II.9).
...
- **Slice 3.6B Phase 1 — Required-Arrival Accuracy** (`8db597a2`, verifier repair `03296bd2`, closeout `34d83cf4`) — complete locally and database/UAT verified. The shared resolver and existing narrow source-preference writer are the only authorized arrival foundation for downstream work.
...
| Gate | Status |
|---|---|
| 3.6B Phase 1 — required arrival for household/unassigned events | Complete locally; dependency satisfied by `34d83cf4` |
| 3.6B Phase 2 — Arbiter Officials group-identity audit | Inconclusive on current fixtures; parallel and non-blocking |
| 3.6B Phase 3A — temporary routing origin | Filed, not built; Phase 1 dependency satisfied |
| 3.6B Phase 3B — HotelPlanner booking → lodging → routing origin | Un-deferred 2026-08-30; evidence diagnostic filed, not run |
| 3.6B Phase 4 — Mapbox traffic-aware leave-by | Design accepted, not built; required-arrival dependency satisfied |
...
```
3.6B Phase 1 ✓ (`34d83cf4`)
  → { Phase 2 (Arbiter identity evidence, parallel/non-blocking)
      || Channel/Auth Phase A+B
      || HotelPlanner Phase 3B evidence diagnostic }
...
- **Phase 1 — Required arrival, household/unassigned events.** Complete locally at `34d83cf4`. The implementation adds the bounded nullable schedule-source preference and one shared pure resolver with typed provenance: `ics_explicit → source_preference → team_preference → corralio_default`. What Fits and This Weekend/Leave-by consume it identically. The narrow owner-authorized source writer is already implemented and verified. Downstream work reuses this foundation and must not add arrival schema, preference tiers, or resolution logic.
- **Phase 2 — Arbiter Officials group-identity audit.** Parallel, non-blocking, and inconclusive on the two current sanitized fixtures. See II.9. No group parser or group preference tier was authorized.
- **Phase 3A — Temporary routing origin.** Filed: `docs/prompts/corralio-slice-3.6b-phase3a-temporary-routing-origin-prompt.md`. Home / current-location / choose-another-location, independent of any hotel concept. Not built. Its Phase 1 sequencing dependency is satisfied; it consumes required-arrival output rather than recalculating it.
...
- **Phase 4 — Mapbox traffic-aware leave-by.** Design accepted (II.13). Its trustworthy `required_arrival` dependency is satisfied by `34d83cf4`; it must consume that output without a parallel arrival model.
...
Completed hierarchy (`34d83cf4`): **valid ICS/event explicit arrival → schedule-source preference → team preference → Corralio 30-minute default**, with typed in-memory provenance `ics_explicit`, `source_preference`, `team_preference`, or `corralio_default`. The source preference is nullable, bounded to 0–120 minutes in five-minute increments, and changed only through the narrow owner-authorized writer. What Fits and This Weekend/Leave-by call the same resolver; Leave-by subtracts drive duration from its resolved required-arrival timestamp. This applies equally to assigned and household/unassigned sources. No persisted provenance column, group tier, SMS-specific field, or second resolver exists or is authorized.

## II.9 Arbiter Officials — Evidence Gap

The founder's proving case: one official working football, soccer, and basketball through different Arbiter groups, each needing a different real arrival time. Two real (sanitized) Arbiter feed exports have been inspected directly (`docs/audits/corralio-arbiter-multi-sport-sample-sanitized.ics` and `-sample-2-team-calendar-sanitized.ics`). Finding: ArbiterSports does not export one consistent format. Sample 1 (an officiating-assignment feed) has a labeled `Sport:`/`Level:`/`Team:`/`Site:` block inside `DESCRIPTION` — real structure, not fuzzy text-matching — but `Sport:` reads the literal value "Tournament" (not the actual sport) for every tournament-type assignment. Sample 2 (a different export path — a team/program schedule, not an officiating feed) has none of those labels at all, only `Site: <value>`. Neither sample actually contains two different real sports for the same official, so the founder's specific proving case remains untested either way. Phase 2 re-inspected both fixtures and closed **inconclusive** rather than fabricating evidence. No group parser or group-specific preference tier was authorized; Phase 1's verified manual source preference remains the durable current answer.
...
Founder's four required evidence items before Phase 3B converts from design to an executable Stage 1 prompt — **items 1 and 2 corrected 2026-09-03; both were satisfied by work completed after this section was written, but neither closure had made it back into this document.** A pre-implementation audit of the filed diagnostic prompt (`6fd64ffb`) caught this before the prompt was dispatched; CPO independently re-verified against the repository before correcting anything here — see `docs/corralio/cpo/2026-09-03-cpo-audit-hotelplanner-phase3b-diagnostic-overtaken.md`.

1. **Textual status contract — closed.** `getReport`'s `Status` field is a string; the locked design's reconciliation rule (`status === 1`) was empirically derived from `getClientSummary`'s *numeric* field and did not apply. **Corrected in the design doc** (`docs/reference/corralio-hotelplanner-attribution-design.md` Section 7, 2026-09-03): exact normalized `"confirmed"`/`"cancelled"` match, everything else `other`, blank `unknown` — implemented in `hotelBookingReconciliation.ts`'s `classifyHotelPlannerStatus` and exercised in production. The filed SQL-enumeration diagnostic (`ti_hotel_bookings` grouped by `status`/`cancel_date`) has still not been run — it remains available, optionally, as a low-cost vocabulary-confirmation step, but it no longer gates anything: the implementation already fails closed to `other`/`unknown` for any unrecognized value rather than guessing.
2. **Cancellation-window query-shape evidence — closed.** One authorized, read-only `getReport` call (2026-08-25 through 2026-08-31, `cancelledDateStart`/`cancelledDateEnd`/`includeCancelled`) returned 8 rows, all exact `Cancelled`, all with cancellation date, purchase date, and itinerary key present — confirming the query filters and shapes data as this design assumed (`docs/reports/ti-hotel-monetization-reporting-2026-08-31.md`, "Cancellation diagnostic"). The production sync now runs this as a separate, isolated seven-day cancellation refresh (`hotelPlannerBookingSync.ts`) — not a diagnostic anymore, shipped behavior.
...
4. **HotelPlanner status-contract bug — resolved 2026-09-03.** The locked attribution design's Section 7 stated the reconciliation rule as `status === 1` — correct for `getClientSummary`, wrong for `getReport`, which the design elsewhere selects as the primary mechanism and which returns `Status` as a string. Corrected in place once the underlying evidence gap (II.12 item 2) actually closed, not before: `docs/reference/corralio-hotelplanner-attribution-design.md` Section 7 now states the proven textual rule (exact `confirmed`/`cancelled`, `other`/`unknown` fallback) and explicitly keeps the numeric `getClientSummary` mapping separate rather than conflating the two again. No longer an open item.
...
**Founder decision, same day as II.22, refining and confirming the priority-channels investigation's recommended slot.** The fork is now active because 3.6B Phase 1 completed locally at `34d83cf4`:

**Phase 1 ✓ -> { Channel/Auth Phase A+B (phone-first identity + deterministic email/SMS intake)  ||  3.6B Phase 2 (parallel/non-blocking/inconclusive on current evidence)  ||  HotelPlanner Phase 3B evidence diagnostic } -> resume 3A -> 3B -> 4 -> 5.**
...
**Binding scope guardrail on the Phase A+B build prompt (not a recommendation — a founder instruction):** establish phone auth/channel identity and deterministic schedule intake cleanly; explicitly do not build the full SMS onboarding conversation yet. Arrival handling is now settled: reuse the completed shared resolver and narrow source-preference writer. A bounded parent answer may populate that existing source preference; it must not become a pending-intake-only arrival field, new schema, new resolver tier, or parallel computation. Inference maturity, secure-origin capture (Phase 3A), and source evidence remain separate capabilities.

**SMS-first implementation order is binding.** Phase B builds and proves the SMS path before the email path. A2P/10DLC lead time controls production activation, not implementation order. Email may stop independently if Resend sender-authentication evidence is insufficient.
...
**Net effect: the Phase A+B review cycle is closed.** Founder's own words: *"I would move forward with Phase A+B rather than opening another investigation cycle. The architecture is converging; the next leverage is implementation and real-feed evidence."* Phase A+B (v2, corrected) is founder-reviewed and accepted. Its 3.6B Phase 1 dependency is satisfied by `34d83cf4`; implementation reuses the completed arrival foundation.

**Execution checkpoint, 2026-08-31:** repository/provider-contract Gates 1–2 completed, but Gate 3 could not run. No local or Corralio Vercel environment has the required Telnyx sender/profile/key/public-key configuration, CAPTCHA, dedicated channel-HMAC key, isolated Supabase test setup, hard SMS cap, or disposable test-phone contract. The prompt makes that capped live spike sequential, so no product code or migration was started. See `docs/corralio/cpo/2026-08-31-cpo-audit-phase-a-b-execution-gates.md` for the exact unblock contract.

## II.25 What Happens Next

See `CORRALIO_CPO_EXECUTION_STATE.md`, "NEXT 5 ACTIONS." In one line: Phase 1 is complete at `34d83cf4`; provision the isolated capped Phase A+B SMS test contract before resuming that prompt, proceed with the filed Phase 3A prompt as selected, run the independent HotelPlanner diagnostic and CALNAME work, keep Arbiter evidence non-blocking, and close the remaining scheduling/cost gates before Phase 4/5 implementation.
