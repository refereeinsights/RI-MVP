...
7. **Email/SMS reclassified as priority channels (2026-08-30, founder directive)** — accepted; see `docs/corralio/cpo/2026-08-30-cpo-investigation-email-sms-priority-channels.md` for the resulting phased architecture (channel identity → deterministic ingestion → notification delivery → true send-before-account, in that order). Recommends slotting the first three phases after 3.6B Phase 1 ships, ahead of Phase 3B/4/5 — a recommendation only; the locked critical path below is unchanged until this slot is confirmed. Nothing here is authorized to build yet.
8. **Phone-first authentication (2026-08-30, founder decision)** — email must not be required for account creation, authentication, household access, or the full web experience. Audited and designed (not built) at `docs/corralio/cpo/2026-08-30-cpo-investigation-phone-first-authentication.md`: household creation is already keyed purely on `auth.uid()` with no email-specific step, so enabling Supabase's native phone-OTP auth (delivered via Telnyx through Supabase's Send SMS Hook, decoupling the auth-provider choice from the SMS-vendor choice) is additive, not a parallel identity system. Slots into the priority-channels investigation's Phase A (item 7) as the same underlying capability — recommend one build prompt, not two. Nothing here is authorized to build yet.
