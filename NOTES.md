...
- Admin (tournament uploads): exposed inline “Edit fields” panel under each pending tournament row in `/admin?tab=tournament-uploads` so admins can tweak values in-place before approving; sport dropdown now includes all supported sports (e.g. futsal/softball/volleyball/wrestling/hockey/other).
