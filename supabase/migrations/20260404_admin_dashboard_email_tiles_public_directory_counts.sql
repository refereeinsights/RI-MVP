-- Admin Dashboard Email tiles RPC: include DB + public directory counts (v3)
-- Adds:
-- - `tournaments_db.total`: total tournaments in DB (all statuses)
-- - `public_directory.total/new_yesterday/by_sport`: matches the TI public directory default view
--   (published canonical, upcoming-only unless demo; includes AYSO and non-AYSO)
...
declare
  ...
  v_public_new_yesterday integer;
...
begin
  ...
  select count(*)::int into v_public_new_yesterday
  from public.tournaments t
  where t.status = 'published'
    and t.is_canonical = true
    and t.created_at >= v_yesterday_start
  ...
  return jsonb_build_object(
    ...
    'public_directory', jsonb_build_object(
      ...
      'new_yesterday', v_public_new_yesterday,
