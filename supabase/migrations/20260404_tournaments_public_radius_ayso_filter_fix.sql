-- Tournaments directory: radius search AYSO filter fix (v2)
-- Previously, `p_ayso_only=false` excluded AYSO tournaments entirely.
-- New behavior:
-- - `p_ayso_only=true` => only AYSO tournaments
-- - `p_ayso_only=false` => include all tournaments (AYSO + non-AYSO)
...
as $$
  ...
  base as (
    ...
    where v.latitude is not null
      ...
      and (
        p.ayso_only is distinct from true
        or upper(coalesce(tp.tournament_association, '')) = 'AYSO'
      )
  ),
  scored as (
    select
