-- Venue inference for published tournaments missing confirmed venue links.
-- Mirrors apply_inferred_venue_candidates_for_drafts but targets status='published'.
-- Training data remains confirmed-only via city_state_sport_venue_clusters.
-- Writes are always is_inferred=true and never mutate confirmed rows.

create or replace function public.get_inferred_venue_candidates_for_published(
...
revoke all on function public.get_inferred_venue_candidates_for_published(integer) from public;
grant execute on function public.get_inferred_venue_candidates_for_published(integer) to service_role;


create or replace function public.apply_inferred_venue_candidates_for_published(
  limit_per_tournament integer default 3,
...
begin
  if coalesce(dry_run, true) then
    return query
      with c as (
        select * from public.get_inferred_venue_candidates_for_published(limit_per_tournament)
      ),
      e as (
        select
          c.*,
  ...
  select a.attnum::smallint into v_tid_attnum
  from pg_attribute a
  where a.attrelid = 'public.tournament_venues'::regclass
    and a.attname = 'tournament_id' and a.attisdropped = false;

  select a.attnum::smallint into v_vid_attnum
  from pg_attribute a
  where a.attrelid = 'public.tournament_venues'::regclass
    and a.attname = 'venue_id' and a.attisdropped = false;
  ...
  select c.conname into v_pair_constraint
  ...
  v_sql := format($fmt$
    with c as (
      select * from public.get_inferred_venue_candidates_for_published($1)
    ),
    upserted as (
      insert into public.tournament_venues (
        tournament_id, venue_id, is_inferred, is_primary,
        inference_confidence, inference_method, inferred_at,
        inference_run_id, venue_sport_profile_id
      )
      select
        c.tournament_id, c.venue_id,
        true, false,
        c.confidence_score, $2::text, now(), $3::uuid, null::uuid
      from c
    ...
    select
      c.tournament_id, c.venue_id, c.confidence_score,
      $2::text as inference_method, c.rank_inference,
      true as wrote, 'inferred' as existing_link_type
    from c
    join upserted u on u.up_tournament_id = c.tournament_id and u.up_venue_id = c.venue_id
...
revoke all on function public.apply_inferred_venue_candidates_for_published(integer, boolean) from public;
grant execute on function public.apply_inferred_venue_candidates_for_published(integer, boolean) to service_role;
