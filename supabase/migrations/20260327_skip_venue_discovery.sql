-- Allow admins to opt a tournament out of venue discovery/deep scans.
-- Also exclude these from the missing-venues paging RPC.

alter table public.tournaments
  add column if not exists skip_venue_discovery boolean not null default false;

create index if not exists tournaments_skip_venue_discovery_true_idx
  on public.tournaments (skip_venue_discovery)
  where skip_venue_discovery = true;

-- Update missing-venues chunking helper to exclude skipped tournaments.
