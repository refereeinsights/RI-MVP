-- Missing venues: include drafts toggle (v1)
-- Extends the missing-venues admin tool to optionally list DRAFT tournaments (uploads queue)
-- that are missing CONFIRMED venue links (no tournament_venues rows with is_inferred=false).
...
revoke all on function public.list_missing_venue_link_tournaments_v2(integer, integer, text, text, text) from public;
grant execute on function public.list_missing_venue_link_tournaments_v2(integer, integer, text, text, text) to service_role;
