...
async function hashExternalId(input: string) {
...
Deno.serve(async (req) => {
  ...
  const withExternalMap = new Map<string, any>();
  ...
  for (const item of records) {
    ...
    let externalIdValue = externalId ? String(externalId) : "";
    if (!externalIdValue) {
      const fallbackKey = [
        sourceId,
        name ?? "",
        ...
        .join("|");
      const hash = await hashExternalId(fallbackKey);
      externalIdValue = `fallback_${hash}`;
    }
    ...
    if (!withExternalMap.has(row.external_id)) {
      withExternalMap.set(row.external_id, row);
    }
  }

  const withExternal = Array.from(withExternalMap.values());
  if (withExternal.length) {
    const externalIds = withExternal.map((row) => row.external_id);
    ...
    if (existingError) {
      const detail = existingError.message ?? "lookup failed";
      await failRun(`Failed to check existing records: ${detail}`);
      return jsonResponse(500, { error: `Failed to check existing records: ${detail}` });
    }
    const existingSet = new Set((existingRows ?? []).map((row: any) => row.external_id).filter(Boolean));
    const toInsert = withExternal.filter((row) => !existingSet.has(row.external_id));
    if (toInsert.length) {
      ...
      if (insertError) {
        ...
        await failRun(`Failed to insert records: ${detail}`);
        return jsonResponse(500, { error: `Failed to insert records: ${detail}` });
      }
    }
  }
    ...
    const inserted = withExternal.length;
    ...
    console.error("assignor-crawl-cnra unhandled error", err);
    return jsonResponse(500, { error: `Unhandled error: ${err?.message ?? String(err)}` });
  }
});
